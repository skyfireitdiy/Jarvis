...
**🏢 事实三：中兴通讯内部深度使用验证（工具集成 + 用户协作）**

Jarvis 已在中兴通讯内部获得深度使用验证：一个约 20 人的研发协作单元通过 Jarvis 进行日常开发协作，累计完成代码开发超 5 万行；同时超过 5 个内部工具将 Jarvis 作为 SDK 嵌入（如自主评审系统、故障定位、代码自动化修改、自动化办公等）。这验证了 Jarvis 作为研发协作平台在实际企业环境中被深度使用的可用性，以及其「作为 SDK 被集成」的开放能力。
...
- **节点与集群管理**：`list_nodes` 查看节点配置与运行状态、`get_node_secret` 获取节点连接私钥、`update_nodes_code` 更新所有节点代码、`restart_nodes` 一键重启节点服务、`list_model_groups` 查看模型组配置、`exec_command` 在指定节点执行 shell 命令、`eval_js` 向前端浏览器下发并执行 JS 代码
- **群组与聊天管理**：`create_group` / `list_groups` / `get_group` / `join_group` / `leave_group` / `send_group_message` 管理群组协作，`chat_list_rooms` / `chat_get_online_clients` / `chat_get_room_members` / `chat_send_room_message` / `chat_send_private_message` 管理聊天室，`list_sessions` 查看活跃连接及其对应用户
...
Jarvis 的核心能力可概括为两大支柱：**独立工作能力**与**协作共事能力**。本章以「人 × Agent」四象限为骨架，逐一展开每种协作模式的技术支撑与差异化优势；3.5 节进一步展示支撑协作的底层能力——插件机制、工具自扩展、分层记忆、知识沉淀、规则加载、MCP 集成、多模型调度、专项能力、宠物大厅与浏览器扩展，它们不是独立的协作模式，而是让每种协作模式都更强大的基础设施。
...
这是 Jarvis 单兵作战能力的集中体现，也是协作共事的根基——一个连独立工作都做不好的 Agent，谈不上与人协作、与 Agent 共事。单个 CodeAgent 的能力可归纳为四个维度：**代码理解**——不是「看到」代码，而是「看懂」代码的依赖关系、影响范围、调用链，并预判改动波及范围；**自主执行**——无人值守、定时调度，从被动响应走向主动安排工作；**质量保障**——用独立审查 Agent 兜底质量，用 fork 子 Agent 零丢失传递上下文；**环境交互与入口**——集成终端与集成编辑器让 Agent 直接操作真实开发环境，CLI 提供轻量入口。此外，方法论系统、三层记忆系统、规则系统等支撑能力贯穿上述全部维度，让 Agent 越用越懂你。

本节八个子章节围绕「独当一面」这一目标，从四个**并列的能力维度**展开——四个维度回答的是不同侧面的问题，不存在先后依赖：

| 能力维度           | 子章节                                         | 核心问题                     | 一句话概括                                              |
| ------------------ | ---------------------------------------------- | ---------------------------- | ------------------------------------------------------- |
| **代码理解**       | 3.1.1 符号数据库、3.1.2 影响分析、3.1.3 多语言 | Agent 如何「看懂」代码？     | 用符号图替代文本匹配，精确回答「改这个函数会影响哪里」  |
| **自主执行**       | 3.1.4 无人值守、3.1.5 定时任务                 | Agent 如何「自主干活」？     | 从「被动响应」到「主动调度」，覆盖半自动到全自动        |
| **质量保障**       | 3.1.6 交叉验证、3.1.7 Fork 子 Agent            | Agent 如何「保证产出质量」？ | 独立审查 Agent 消除局限，fork 子 Agent 零丢失传递上下文 |
| **环境交互与入口** | 3.1.8 CLI                                      | 开发者如何「轻量使用」？     | 一条命令秒级响应，小需求无需启动完整架构                |

需要说明三点：其一，**代码理解是其余三个维度的共同基础**——看不懂代码，自主执行与质量保障都无从谈起；但「基础」不等于「层次」，代码理解与自主执行、质量保障之间是**支撑关系而非递进关系**，四者并列构成 Agent 的完整能力面。其二，**质量保障维度描述的是单个 Agent 内部的质量机制**（独立审查 Agent、fork 子 Agent），与 3.2 节「单人多 Agent」的跨 Agent 协作不是同一概念。其三，CLI 作为**环境交互与入口**维度，背后复用的正是代码理解维度的符号数据库与影响分析引擎——它是同一套能力的另一种触达方式，而非独立于代码理解之外的新能力。此外，方法论系统、三层记忆、规则系统、集成终端等能力不属于上述任一维度，而是**贯穿全部维度**的支撑能力，详见本节末尾。
...
**差异化**：主流 AI 编程工具（Claude Code、CodeX 等）每次查询通常重新读取文件（grep 实时搜索或模型上下文读取），不维护持久化的符号索引；Jarvis 的符号数据库以「mtime 增量检测 + 单文件粒度更新」实现索引与代码的实时同步，既避免全量重建的开销，又保证查询结果的准确性。这正是「双轨并行」中第二轨相对主流方案的增量价值所在。
...
AI 编程场景与 IDE 场景有本质区别——Agent 修改的往往是**代码片段**，而非完整的、可编译通过的代码。LSP（Language Server Protocol）的设计前提是「代码处于可编译状态」，它需要完整的项目上下文、正确的构建配置、可解析的类型信息，一旦代码处于中间态（函数写了一半、import 缺失、语法不完整），LSP 就会失效或返回错误结果。Tree-sitter 则完全不同：**容错解析**——即使代码片段语法不完整也能恢复出语法树，提取可用符号信息（这对 AI 编程至关重要，Agent 修改过程中代码几乎总处于「不完整」状态）；**零依赖、零启动开销**——纯解析库直接内嵌在进程中，无需为每种语言启动独立的语言服务器进程；**无需构建配置**——只需语法规则，对任意代码片段都能立即解析，不依赖项目是否可编译。

Jarvis 同时提供 `jarvis-lsp`（命令行 LSP 客户端）作为补充手段：当代码处于完整可编译状态、且需要编译时类型信息时，Agent 可选用它作为 Tree-sitter 符号提取的补充。
...
至此，**代码理解**维度三节已完整——符号数据库提供结构化理解，影响分析提供改动预判，多语言保证覆盖面。但「看懂代码」只是手段，**「自主干活」才是目的**。接下来进入**自主执行**维度：Agent 如何无人值守、如何定时调度。
...
**差异化**：主流工具提供自动确认模式（如 Claude Code 的 `--dangerously-skip-permissions`），但多为「一次授权、全程放行」的单一开关；Jarvis 将无人值守拆为任务级与进程级两个层次，可按场景选择自主程度，粒度更细。
...
**自主执行**维度两节已完整——Agent 既能无人值守，又能定时调度。但**自主执行越强，质量风险越大**：Agent 自主写的代码，谁来把关？接下来进入**质量保障**维度：用独立审查 Agent 与 fork 子 Agent 为自主执行兜底。
...
**质量保障**维度两节已完整——独立审查 Agent 消除局限，fork 子 Agent 零丢失传递上下文。但**所有这些能力，开发者如何触达？** 如果每次使用都要启动完整的多节点架构，那「独当一面」就失去了轻量性。最后进入**环境交互与入口**维度：CLI 这一轻量入口。
...
至此，3.1 节以**代码理解、自主执行、质量保障、环境交互与入口**四个能力维度，完整回答了「独立如何要强」——符号数据库让 Agent 看懂代码，无人值守与定时任务让 Agent 自主干活，独立审查与 fork 子 Agent 为自主执行兜底，CLI 让所有能力轻量触达。

除上述四个维度外，Jarvis 的 CodeAgent 还内置一套**贯穿全部维度**的支撑能力，让「独当一面」不止于「会写代码」：
...
> 注：上下文感知与集成编辑器（`read_code` 的结构化编辑上下文、`edit_file` 的精确编辑与自动影响分析）是上述能力在具体工具上的体现，已分别在 3.1.1 与 3.1.2 节展开，此处不再重复。
...
7. **上下文压缩**：多人共享同一会话意味着上下文增长更快——多个用户的消息、Agent 的回复、工具调用结果都累积在同一份历史中，长会话极易触及模型的输入长度上限。Jarvis 采用**滑动窗口压缩 + 后台预压缩**两级机制：当上下文接近模型输入上限时，将窗口之外的历史消息交给模型生成结构化摘要（保留任务目标、已完成事项、关键决策与待办），再以「系统消息 + 压缩摘要 + 最近消息」重建历史；同时，在上下文用量达到约 75% 时即**在后台线程提前生成摘要**，待真正触发压缩时直接复用，避免压缩本身阻塞对话。压缩摘要以可识别的消息格式写入历史，会话恢复时能正确还原压缩状态，保证多人协作的长会话不因上下文溢出而中断

这七点机制共同构成多人共享 Agent 的上下文管理闭环：**标识**解决「谁在说话」，**序号与队列**解决「顺序一致」，**权限**解决「谁能说话」，**会话共享**解决「上下文连贯」，**冲突检测**解决「修改安全」，**上下文压缩**解决「长会话不溢出」。
...
![节点与 Agent 拓扑视图](images/节点Agent拓扑.png)

上图为 Jarvis 管理界面中的节点与 Agent 拓扑视图，直观展示各节点上运行的 Agent 分布与连接关系。
...
**拓扑感知**：节点级运维的前提是「看得见网络」。Jarvis 前端提供**实时网络拓扑可视化**，将节点与 Agent 的运行状态直接呈现为一张可交互的网络图：
...
本节十个能力分六组：**扩展机制**（3.5.1 插件机制、3.5.2 工具自扩展）让内核能力按需扩展、让工具集随需求增长；**知识管理**（3.5.3 分层记忆、3.5.4 知识沉淀、3.5.5 规则加载）让经验可沉淀、可复用；**生态与调度**（3.5.6 MCP 集成、3.5.7 多模型调度）让 Jarvis 融入开放生态并优化成本；**工程深水区**（3.5.8 专项能力）覆盖主流工具未触及的迁移、审计、跨环境操作；**交互体验**（3.5.9 宠物大厅）让多 Agent 的运行状态从「列表与日志」变为「一眼可见、可交互的协作现场」；**真实环境接入**（3.5.10 浏览器扩展）让 Agent 的操作对象从服务端模拟环境延伸到用户的真实工作现场。
...
- **浏览器自动化**：Playwright 驱动，HTML 转 Markdown（服务端独立浏览器；操作用户真实浏览器见 3.5.10 节）
...
#### 3.5.9 宠物大厅：Agent 协作状态的可视化与趣味化

当 Agent 数量增长到多面板、多节点规模后，「谁在运行、谁在等待、谁已停止」这类状态若只能从列表与日志中读取，协作现场便失去了直观性。**宠物大厅（PetLobby）** 是 Jarvis 前端的 Agent 可视化视图——它把每个 Agent 抽象为一只在「大厅」中自由游走的**宠物精灵**，让多 Agent 的运行状态一眼可见、可直接交互。

**宠物即 Agent**：大厅中的每只宠物对应一个正在运行的 Agent。宠物形象为纯 CSS 自绘（头部、耳朵、眼睛、尾巴、影子），无需任何图片资源；宠物下方标注 Agent 名称，并按 Agent 类型区分图标（`💻` 表示 CodeAgent，`🤖` 表示普通 Agent）。**已停止（stopped）的 Agent 不显示宠物**，因此大厅中出现的每一只精灵都代表一个活跃的协作单元。

**状态配色**：宠物与节点沿用与大屏网络拓扑（见 3.4.4）一致的配色方案，保证两处视觉语言统一：

- **运行中（running）**：`#20c8ff`（蓝）
- **等待输入（waiting）**：`#ffb347`（橙）
- **空闲（idle）**：`#8a9bb0`（灰）
- **已停止（stopped）**：`#ff5d6c`（红）

**大厅即拓扑**：宠物大厅的地板之上叠绘了一层与网络拓扑同款的**节点机箱与连线**——master 节点以金色机箱居中，其余节点按状态着色；每个 Agent 宠物与其所属节点之间以虚线相连（颜色取该 Agent 的状态色），连线随宠物移动**实时更新**。于是大厅同时呈现了两层信息：**宠物层**表达「有哪些 Agent、各自什么状态」，**拓扑层**表达「这些 Agent 分布在哪些节点上」。

**地板 Slogan**：大厅地板印有做旧、斑驳质感的 Slogan——**「独当一面，与众共事」**，正是 Jarvis 四象限协作模式的主张（见 1.2 节）。宠物在 Slogan 之上游走，将产品理念直接嵌入协作现场。

**交互能力**：

- **自由游走**：宠物默认在大厅内自由游走（`requestAnimationFrame` 驱动），右上角开关可一键切换「游走中 / 已静止」；宠物被激活、输入或拖拽时自动暂停移动
- **拖拽与选中**：可拖动任意宠物调整位置；单击宠物展开其输入面板，点击大厅空白处取消选中，宠物恢复飘动
- **输出气泡**：宠物下方常驻显示该 Agent 的最新输出（Markdown 渲染），支持一键复制；「显示全部输出 / 隐藏全部输出」可批量控制，单个输出的隐藏状态持久化到 `localStorage`
- **就地交互**：单击宠物展开输入面板（多行/单行按状态自动选择，`Ctrl+Enter` 或 `Ctrl+D` 发送）；需要确认时，确认控件直接显示在宠物下方，无需跳转
- **快捷入口**：双击宠物打开命令面板；右键宠物唤出环形菜单（复用命令面板「当前 Agent」组动作）；点击宠物旁的迷你拓扑图可展开网络拓扑大图（见 3.4.4）
- **批量显隐**：「全部显示 / 全部隐藏」一键控制所有精灵的显示，状态持久化到 `localStorage`

**技术深度**：这是**协作状态可视化在 AI 工具领域的应用**。多 Agent 系统的核心难点之一是「状态可观测性」——当数十个 Agent 并发运行时，传统的列表视图难以让人快速判断整体态势。宠物大厅将每个 Agent 的状态编码为**位置 + 颜色 + 气泡**三重视觉信号，并叠加节点拓扑层，使「哪些 Agent 在跑、分布在哪些节点、各自的输出是什么」在一个视图内同时可得。其实现上采用纯 SVG + CSS 自绘、`requestAnimationFrame` 驱动动画，且精灵隐藏时跳过全部渲染与运动计算以降低资源消耗，兼顾表现力与性能。

**差异化**：主流 AI 开发工具对多 Agent 的呈现以**列表、标签页或终端日志**为主，状态信息抽象、缺乏空间感；即便提供可视化，也多停留在静态的关系图。Jarvis 的宠物大厅把多 Agent 协作现场「具象化」为一场可游走、可拖拽、可交互的精灵聚会，并与网络拓扑、命令面板、输出气泡深度联动——它既是**状态监控面板**，也是**轻量操作入口**，让「管 Agent」这件事从「读日志」变为「看现场」。

下图展示了宠物大厅的实际运行界面——大厅中每只宠物精灵对应一个正在运行的 Agent，精灵下方标注 Agent 名称与最新输出，并按状态着色（运行中/等待输入/空闲）；地板之上叠绘了节点机箱与连线，因此「有哪些 Agent、各自什么状态、分布在哪些节点上」在一个视图内即可一目了然。

![宠物大厅](images/多Agent界面2.png)

#### 3.5.10 浏览器扩展：让 Agent 操作用户真实浏览器

Jarvis 原有的浏览器能力由 `jarvis-browser`（CLI 名 `jb`）提供，它在**服务端**用 Playwright 启动一个全新的独立 Chromium 实例——没有 `user_data_dir`、没有 `storage_state`，与用户桌面上的浏览器毫无关系。这意味着 `jb` 操作的是「服务器上凭空开的一个浏览器」：不共享用户的登录态、Cookie、书签与已打开标签页，因此**无法完成「操作用户自己已登录的网页」这类任务**。

**浏览器扩展（Jarvis Browser Bridge）** 把执行体从「服务端 Playwright」换成「**用户浏览器内部的扩展**」，让 Agent 直接操作用户本机真实浏览器中的任意标签页：

- **操作对象**：用户本地浏览器中当前打开的任意 `http(s)` 标签页，而非服务端新建的空白浏览器
- **登录态复用**：直接复用用户已有的 Cookie 与 Session，无需重新登录、无需用户手动填写 Token
- **连接方式**：扩展通过 WebSocket **主动连出**到 Jarvis 网关，因此**无需公网 IP、无需开放端口**，内网环境同样可用
- **多网关支持**：同一浏览器可同时连接多个 Jarvis 网关（如本机 `http://127.0.0.1:8000` 与线上网关），每个网关维护独立的 WebSocket 连接、独立的 `session_id` 与独立的登录态 Token；网关列表持久化在 `chrome.storage.local.gateways`，**Token 仅存内存、不落盘**，冷启动时自动为所有已配置网关重连

**登录态复用机制**：扩展不要求用户单独登录，而是**自动复用浏览器中已登录的 Jarvis 网页登录态**——前端暴露只读接口 `window.__jarvisAuthBridge.getToken()`，content script 注入页面主世界读取 Token 并转发给 background，background 以 `jarvis-token.<token>` 子协议连接网关 WebSocket。由此带来三点性质：**身份天然一致**（扩展使用的 Token 就是网页登录用户的 Token，不存在「扩展账号与网页账号不一致」的问题）、**Token 变化自动跟随**（用户在网页切换账号或登出，扩展自动重连或断开）、**冷启动自动获取**（扩展启动时主动向已打开的 Jarvis 页面索取 Token）。

**能力清单**（工具名 `browser_ext`，每次调用只能执行一个 action）：

- **会话与标签页**：`list_sessions` 列出在线会话、`list_tabs` 列出标签页、`activate_tab` / `close_tab` / `new_tab` / `reload` / `back` / `forward` 管理标签页与导航
- **DOM 操作**：`get_text` / `click` / `type` / `query` / `get_html` / `hover` / `select` / `wait_for` / `press_key` / `scroll` / `upload_file`，覆盖常规网页交互
- **截图**：`screenshot` 抓取标签页画面
- **JS 执行三件套**：`execute_script` 在页面执行任意 JS（默认 MAIN 世界，以复用页面自身 JS 环境与登录态）、`evaluate` 经 CDP `Runtime.evaluate` 求值表达式（**不受页面 CSP 限制**，相当于在 F12 Console 直接敲表达式）、`send_cdp_command` 透传任意 CDP 命令（如 `Page.addScriptToEvaluateOnNewDocument` 在文档创建前注入脚本）
- **调试类（对标 F12 面板）**：`get_computed_style`（Computed 面板）、`get_page_info`（页面概览）、`get_console_logs`（Console 面板）、`get_network_requests`（Network 面板）
- **类油猴脚本管理**：`script_list` / `script_get` / `script_run` 盘点与调用已安装脚本；脚本源码在页面**主世界**求值，可访问页面自身的 JS 对象，从而绕过 CSP、iframe 隔离与坐标点击的脆弱性，把某个站点的复杂交互封装成可复用的 action

**安全边界**：扩展须由用户**显式安装并授权**（开发者模式加载，不做静默安装）；指令可审计；`execute_script` 等高危操作在 popup 中设有确认；不操作 `chrome://*`、扩展商店等浏览器受保护页面。

**风险提示与免责声明**：能力越强，边界越需要被明确告知。因此在安装环节，前端会先弹出风险提示与免责声明，用户确认后才会开始下载插件包：

![浏览器插件风险提示与免责声明](images/浏览器插件风险提示与免责声明.png)

_安装前弹出的风险提示与免责声明：逐项说明高敏感权限的含义与不可逆操作，用户确认后方可下载插件包。_

**风险操作示例**：上述高敏感能力并非停留在纸面。以「扩展管理」为例，Agent 通过 `extmgr_list` 即可列出用户浏览器中已安装的全部扩展及其启用状态：

![Agent 列出浏览器中已安装的扩展](images/风险操作示例-插件列表.png)

_Agent 调用 `extmgr_list`，列出浏览器中已安装的扩展及其启用状态——无需用户手动打开扩展管理页。_

进一步，Agent 通过 `extmgr_uninstall` 可直接卸载指定扩展：

![Agent 卸载指定浏览器扩展](images/风险操作示例-插件卸载.png)

_Agent 调用 `extmgr_uninstall` 卸载指定扩展；此类操作**不可逆**，一旦执行无法恢复。_

这两个示例直观说明了「把浏览器完全交给 Agent」的实际含义：**能力即风险**——`extmgr_list` / `extmgr_uninstall` 只是 28 项权限中的一小部分，Cookie、网络请求、代理、浏览数据等高敏感能力同样对已连接的 Agent 开放。因此本方案把风险告知前置到安装环节，并明确免责声明：扩展按「现状」提供，仅限用户本人授权范围内使用，因授权、误操作或第三方滥用导致的后果由使用者自行承担。

用户浏览器、扩展与 Jarvis 网关之间的完整链路如下：
...
participant "Agent" as agent
participant "Jarvis 网关" as gw
participant "扩展\n(background)" as bg
participant "页面\n(主世界)" as page
participant "用户浏览器\n真实标签页" as tab

agent -> gw : 调用 browser_ext
note right of agent
  与调用普通工具
  无差别
end note
gw -> bg : WebSocket 下发指令
note right of gw
  扩展主动连出
  无需公网 IP
end note
...
图中展示了浏览器扩展操作真实浏览器的完整链路：**Agent** 调用 `browser_ext` 工具时与调用普通工具**无差别**；**Jarvis 网关**经 **WebSocket** 将指令下发至用户浏览器中的**扩展 background**——由于是扩展**主动连出**，用户侧无需公网 IP、无需开放端口；background 通过注入脚本或 CDP 在页面**主世界**执行操作，从而**复用用户已有的登录态与 Cookie**，直接作用于用户浏览器的**真实标签页**；执行结果沿原路回传至 Agent。整个过程中，扩展的 Token 来自用户已登录的 Jarvis 网页，且**仅存内存、不落盘**。

**技术深度**：这是**执行体位置迁移在 AI 工具领域的应用**。传统浏览器自动化（含 `jb`）的执行体在服务端，Agent 与「用户所见」之间隔着一层由服务端重建的浏览器环境，登录态、Cookie、已打开标签页都无法复用；Jarvis 把执行体下沉到用户浏览器内部，用扩展作为 Agent 与真实页面之间的桥梁，使「Agent 操作的就是用户正在看的那个页面」成为可能。其难点在于**登录态的安全传递**——扩展不引入第二套账号体系，而是通过页面主世界注入读取前端已登录的 Token，并以 `jarvis-token.<token>` 子协议完成网关鉴权，做到身份天然一致、Token 不落盘、变化自动跟随。

**差异化**：主流 AI 编程工具的浏览器能力多为「服务端启动一个无登录态的浏览器」（如 Playwright 方案），或通过 MCP 接入外部浏览器服务，均难以直接操作用户**已登录**的网页；即便支持，也往往需要用户重新登录或手工导出 Cookie。Jarvis 的浏览器扩展让 Agent 直接驱动用户本机真实浏览器，**登录态零成本复用**、**无需公网 IP 与端口映射**、**支持同时连接多个网关**，并内置类油猴脚本机制把站点复杂交互沉淀为可复用 action——让「操作环境」这一能力从服务端的模拟环境，延伸到用户真实的工作现场。

至此，3.5 节十个底层能力已全部展开：**扩展机制**让内核与工具集按需生长，**知识管理**让经验可沉淀可复用，**生态与调度**让 Jarvis 融入开放生态并优化成本，**工程深水区**覆盖主流工具未触及的迁移与审计场景，**交互体验**让多 Agent 的运行状态一眼可见，**真实环境接入**则把 Agent 的操作对象延伸到用户的真实工作现场。它们共同构成 Jarvis 从「协作平台」走向「可自我进化的 AI 开发生态」的基础设施——与第四章的能力对比、第五章的场景落地相互印证。
...
**壁垒的本质**：上述五层壁垒的**组合**才是真正的护城河——单一能力（如符号数据库）可被复制，但「符号级理解 + 多 Agent 协作 + Agent 自举 + 多人共享 + 知识沉淀」的系统级整合需要跨越多层技术栈的深度工程投入。Jarvis 的 158,181 行代码、1064 个测试用例、319 个 Python 文件即是此壁垒的实证。
...
> **说明**：5.1-5.4 节展示的是 Jarvis 能力在典型开发场景中的**工作流设计**（基于实际功能机制推演的理想工作流），5.5-5.6 节展示的是**已实际落地验证**的成果（安全扫描规则库、Agent 自举提交、中兴通讯内部深度使用验证、开放原子大赛）。
...
2. **实证内容的范围**：5.5-5.6 节展示的是**已实际执行并记录结果**的验证——安全扫描（62 条 C/C++ 规则 + 34 条 Rust 规则，154 漏洞样本 0 漏报，110 安全样本 0 误报）、Agent 自举（超 1 万次提交）、中兴通讯内部深度使用验证、开放原子大赛一等奖
...
**验证方式**：jsec 案例采用**基准测试集验证**——154 个漏洞样本（覆盖 12 类 CWE 漏洞类别）与 110 个安全样本构成固定基准集，每次规则迭代后通过 pytest 自动回归，确保 0 漏报、0 误报。其严谨性体现在：样本来源多样（CVE 真实漏洞、开源仓库扫描发现、人工构造的边界用例）；基准集固定不可变，后续规则迭代必须通过全部基准测试以防回归；量化指标可复现（检出率 154/154 = 100%、误报率 0/110 = 0%）。与开放原子大赛赛题要求（检出率 ≥90%、unsafe <5%）对标，jsec 的 100% 检出率、0% 误报率显著优于赛题基准。

**关于多 Agent 协作的对照实验**：当前 jsec 案例验证的是「对抗式双 Agent 协作」的最终成果（规则库质量），尚未设计「单 Agent vs 双 Agent 对抗」的对照实验来量化协作带来的增量收益。该实验已在规划中：自变量为协作模式（单 Agent 独立开发 / 双 Agent 对抗迭代 / 三 Agent 编排协作），因变量为规则库质量（检出率、误报率、规则覆盖率）、开发效率（单位时间产出规则数）与资源消耗（token 用量、运行时长），控制变量为相同时间预算、相同基准测试集、相同初始规则库与模型配置；三组各重复 3 次取中位数，以「检出率提升 ≥5% 或误报率降低 ≥2%」作为多 Agent 协作增量收益的判定标准。该实验的可重复性与结论可信度由固定基准集保障，将在 v4.x 版本中落地执行。当前阶段以成果验证为主（规则库已达生产可用标准）。

**文档对抗优化实证**：对抗式优化不仅适用于安全规则沉淀，同样适用于文档质量提升。本文档的编写过程本身即是一次对抗式优化的实证——通过「大赛评委 Agent」与「文档编写 Agent」两个角色的对抗迭代，依据大赛评分标准（技术创新 30%、场景落地 30%、开源治理 20%、长期发展 20%）逐项评审、回应修改、重新打分，直至达到参赛水准。
...
Jarvis 已在中兴通讯内部获得深度使用验证，核心使用方式为**研发人员的日常开发协作**，同时具备**作为 SDK 被集成**的开放能力：

- **用户日常协作**（主要方式）：一个约 20 人的研发协作单元通过 Jarvis 进行日常开发协作，利用其代码分析、修改与生成能力提升开发效率；其中典型模式为**结对编程**（平台同事 + 业务同事 2 人协作），已持续使用约一年
- **作为 SDK 嵌入内部工具**（开放能力）：超过 5 个内部工具将 Jarvis 的 CodeAgent 作为 SDK 嵌入，涵盖**代码评审**（自主评审系统）、**故障定位**、**代码自动化修改**、**自动化办公**等场景，利用其代码理解与生成能力驱动特定业务流程

以下量化数据经脱敏处理后可供披露（数据截至 2026 年 9 月）：

- **集成规模**：SDK 嵌入内部工具超 5 个，覆盖代码评审、故障定位、代码自动化修改、自动化办公四类场景
- **使用规模**：日常协作用户约 20 人，使用时长约一年
- **代码产出**：累计完成代码开发超 5 万行；其中基于 Jarvis 的 C2Rust 工具已完成**上万行代码迁移**
- **评审能力**：自主评审系统覆盖 C、Rust、Go 等语言，评审维度包括**逻辑错误、上下文关联错误、安全编码**等

约一年的持续使用表明，Jarvis 已深度融入该团队的日常研发流程——从代码编写、评审到跨语言迁移，形成了稳定的协作模式。

这验证了 Jarvis 作为**研发协作平台**在实际企业环境中被深度使用的可用性——从 Jarvis 开发者自身的深度使用，到中兴通讯约 20 人的研发协作单元，Jarvis 的协作模式已完成了**从个人到中型团队的扩展验证**；同时其「作为 SDK 被集成」的开放能力与跨语言代码评审、迁移的工程能力也得到了充分验证。
...
**可披露与不可披露的边界说明**：本案例已披露**集成规模**（SDK 嵌入工具超 5 个）、**使用规模**（约 20 人、约一年）、**代码产出**（超 5 万行、C2Rust 迁移上万行）、**评审能力**（C/Rust/Go、逻辑错误/上下文关联/安全编码）等量化数据。仍不可披露的信息包括——**具体业务流程**（工具所服务的业务系统细节）、**精确效率数据**（提效百分比、节省工时）、**用户身份**（具体人员与团队信息）。此边界由中兴通讯的企业保密政策决定——中兴通讯对内部效率数据实行统一管控，涉及核心业务逻辑与人员信息的部分仍属企业机密。Jarvis 团队在可披露范围内提供了**量化实证数据**（集成规模、使用规模、代码产出），辅以**可公开验证的量化数据**（大赛成绩、安全扫描指标、自举提交次数）作为技术实力的佐证。

**内部使用实证**：除企业合作外，Jarvis 的开发者自身即是 Jarvis 的**深度日常用户**——日常开发工作（代码编写、测试、文档、Issue 处理）均通过 Jarvis 的 CodeAgent 完成，此即 6.5 节所述「Agent 自举」的实践基础。超 1 万次 Agent 自主提交（2025 年 5795 次 + 2026 年 4524 次）不仅是技术能力的展示，更是**真实用户使用记录**——每次提交都经过「自然语言描述需求 → Agent 编写代码 → 测试验证 → 人类审查合并」的完整协作流程。此内部使用实证的价值在于：

1. **真实使用场景**：日常开发覆盖代码编写、Bug 修复、功能迭代、文档维护、测试补充等完整开发活动，非演示场景
2. **长期持续使用**：自 2025 年至今持续使用，非短期试用——长期使用暴露并修复了大量真实问题（Issue 47 个全部关闭即为此过程的记录）
3. **多 Agent 协作验证**：开发者通过 Jarvis 的多 Agent 编排功能（3.2 节）驱动多个 Agent 并行协作，验证了多 Agent 协作场景的可用性
4. **可公开验证**：所有提交记录、Issue 处理记录均在 GitHub 公开仓库可查，非内部不可验证数据

此内部使用实证与中兴通讯的企业落地形成互补：内部使用证明「Jarvis 开发者自己用 Jarvis 开发 Jarvis」的闭环可行性，企业落地证明「外部组织也能用 Jarvis」的开放可用性。
...
**协作平台界面实证**（以下为 Jarvis 当前版本的界面截图，用于展示上述协作能力的实际运行效果；大赛参赛时 Jarvis 尚处早期 CLI 形态，无前端界面，故非大赛现场截图）：

下图展示了 Agent 修改文件后自动进行**影响分析**的过程（对应 3.1.2 节）——Agent 基于符号数据库精确识别修改所涉及的影响范围，并给出风险等级评估，帮助开发者理解每次代码变更的潜在影响。

![变更影响](images/变更影响.png)

下图展示了利用**编排文件自动批量生成 Agent** 的过程（对应 3.2.1 节）——通过 YAML 声明式编排文件，一键批量创建多个 Agent，截图中红框框起的是创建 Agent 的结果表格，清晰呈现每个 Agent 的创建状态与配置信息。

![编排](images/编排.png)

下图展示了 Jarvis 前端的**多分区界面**（对应 3.2.5 节）——可以同时查看多个 Agent 的运行状态与输出，每个 panel 中还集成了终端、聊天室、编辑器等开发工具（编辑器在图中未直接体现），实现一站式多 Agent 监控与交互。图中可见三个 Agent 分别创建在不同的节点上，体现了跨节点分布式部署的能力。
...
- **代码规模**：319 个 Python 源文件，158181 行代码
- **测试覆盖**：84 个测试文件，1064 个测试用例
- **内置工具**：21 个内置工具
...
- **全量测试**：`pytest` 一键运行 1064 个测试用例，覆盖核心功能，保证可复现验证
...
**社区贡献实际案例**（PR 6 个全部合并，其中 4 个来自 3 位社区贡献者）：
...
**Agent 提交的识别机制**：为便于区分人类提交与 Agent 自主提交，项目约定在 Agent 生成的提交信息中附加特殊标记（`[Jarvis Code Agent提交]` 前缀），人类提交则无此标记。该约定使 git 历史中 Agent 提交可被程序化识别与统计，为「99% 以上提交由 Agent 自主生成」提供可验证的量化依据。

**识别机制的工程保障**：`[Jarvis Code Agent提交]` 标记并非仅靠约定，而是有工程机制保障——Jarvis 的 CodeAgent 在自动提交时，其提交信息生成逻辑内置了标记附加规则（Agent 的提交模板强制包含 `[Jarvis Code Agent提交]` 前缀），人类手动提交则不会触发此模板。此机制使标记的附加是**程序化强制**而非人工自觉，保证了统计数据的可靠性。同时，git 提交历史本身是**不可篡改的公开记录**——任何人可通过 `git log` 验证标记的真实性与一致性，不存在事后标注或选择性标注的可能。

**自举数据的定位**：Agent 自举数据（超 1 万次提交、99% 以上由 Agent 生成）的定位是**工程可行性的实证**，而非**产品质量的独立验证**。自举数据证明的核心命题是「Jarvis 的 Agent 具备自主完成真实开发任务的能力」——每次提交都经过代码编写、测试执行、构建验证的完整流程，且由人类审查后合并（见 6.3 节贡献机制）。但自举数据不替代独立的产品质量验证——Jarvis 的代码质量由 1064 个测试用例（6.1 节）、CI/CD 四条流水线（6.6 节）、以及大赛与企业的独立验证（5.6 节）共同保障。自举与独立验证是互补关系：自举证明「Agent 能干活」，独立验证证明「活干得好」。
...
- 项目已在中兴通讯内部获得深度使用验证（工具集成 + 用户协作）
- 获第三届开放原子大赛总冠军
- 真实场景验证先于社区热度，体现了「先落地、后推广」的务实路线

**社区数据**（GitHub 官方统计，2026-09-06）：

- Star 133、Fork 30、贡献者 4（核心开发者 1 人 + 社区贡献者 3 人）
- Issue 47（**已关闭 47，关闭率 100%**）、PR 6（全部合并）
- Issue 全部关闭的原因：项目处于早期快速迭代阶段，Issue 多为功能建议与改进需求，核心开发者在迭代中逐项落实并关闭；社区反馈的每个 Issue 均得到响应与处理，体现了对社区反馈的重视
- 社区规模尚在早期，但项目以「先落地、后推广」为路线——真实场景验证（中兴通讯内部深度使用验证、开放原子大赛）先于社区热度，符合「开源向实」的赛事主题
...
**社区规模与项目阶段的匹配性**：Star 133 的规模需放在项目阶段背景下理解——Jarvis 当前处于**早期快速迭代阶段**（v5.0.1，2025 年启动），此阶段的社区特征与成熟项目不同：

1. **真实场景验证先于社区热度**：Jarvis 的路线是「先落地、后推广」——中兴通讯内部深度使用验证、开放原子大赛总冠军等真实场景验证先于社区推广。此路线的逻辑是：先证明产品价值，再扩大用户基础，避免「有热度无价值」的空转
2. **社区规模与项目成熟度的匹配**：133 Star 对应的是「核心功能已完成、真实场景已验证、正在扩大用户基础」的阶段——此阶段的核心任务是完善文档、降低门槛、建立反馈闭环（见 7.1 节路线图），而非追求 Star 数量
3. **社区质量重于数量**：贡献者 4 人（核心 1 + 社区 3）、PR 6 个全部合并、Issue 47 个全部关闭——社区规模虽小，但每个 Issue 都得到响应、每个 PR 都被认真审查合并，体现了「小而精」的社区质量
...
1. **Agent 负责执行，人类负责决策**：Agent 完成代码编写、测试、构建验证后自动提交，但**合并权始终在人类手中**——核心开发者审查每个 Agent 提交，确认质量后才合并（见 6.3 节贡献机制）。Agent 是「执行者」，人类是「决策者」
...
3. **开源治理的透明度**：git 提交历史公开可查，`[Jarvis Code Agent提交]` 标记使 Agent 提交与人类提交清晰可辨（见 6.5 节），社区可自行验证「哪些代码由 Agent 生成、哪些由人类贡献」——治理过程完全透明
4. **自举降低贡献门槛**：Agent 自举开发使外部贡献者可用自然语言描述需求，由 Agent 生成代码并提交 PR——这降低了开源贡献的技术门槛，反而**促进**而非阻碍社区参与

**Issue 关闭率的可验证性**：Issue 47 个全部关闭（关闭率 100%）的数据可通过 GitHub 公开页面直接验证——任何人可访问项目仓库的 Issues 页面，查看每个 Issue 的状态、关闭时间与处理记录。关闭率 100% 的原因已在 6.8 节说明：项目处于早期快速迭代阶段，Issue 多为功能建议与改进需求，核心开发者在迭代中逐项落实并关闭。此数据反映的是**项目早期阶段的真实状态**——Issue 数量有限（47 个）、响应及时，而非刻意追求关闭率。随着社区规模扩大、Issue 数量增长，关闭率将自然回落至正常水平（7.1 节路线图已将 2027 年 Issue 关闭率目标设定为 ≥60%，反映此预期）。
...
**量化 KPI**：跨节点通信延迟降低 ≥30%；高频路径执行效率提升 ≥20%；全量测试用例保持 ≥1064 个且全部通过
...
| 层级         | 职责                                   | 维护方         | 原则           |
| ------------ | -------------------------------------- | -------------- | -------------- |
| **内核**     | Agent 基类、网关、记忆、规则等核心能力 | 核心开发者     | 稳定性优先     |
| **插件包**   | 代码理解、安全分析、C2Rust 等扩展能力  | 社区贡献       | 灵活性优先     |
| **贡献机制** | Fork → PR → CI → 审查 → 合并           | 核心维护者把关 | 先测试、后合并 |
| **版本管理** | 语义化版本（SemVer）                   | 核心开发者     | 兼容性保证     |

**治理路径说明**：内核由核心开发者维护，保证稳定性；插件包由社区贡献，保证灵活性。所有贡献（无论内核还是插件包）都经过统一的「Fork → PR → CI → 审查 → 合并」流程，由核心维护者把关。重大特性走 RFC 流程，安全漏洞 48 小时内响应。

**当前单人开发的简化执行方式**：上述治理框架是面向社区壮大后的完整设计，当前单人开发以**轻量简化**方式执行：

- **RFC 流程简化**：重大特性由核心开发者直接决策，以 Issue 记录决策过程与结论，替代正式的 RFC 文档流程；待社区贡献者增多后再引入完整 RFC 机制
- **安全响应**：安全漏洞响应由核心开发者直接处理，通过 Issue 跟踪与修复提交闭环，48 小时响应承诺在当前规模下可即时达成
- **代码审查**：所有提交均经过核心开发者的人工审查后合并，配合 CI/CD 自动化测试与 Agent 自举的符号级影响分析，保证「先测试、后合并」原则
- **CI/CD 保障**：四条 CI/CD 流水线（test/publish/docker-publish/deploy-docs）自动化执行测试与发布，弥补人力的不足

这种「当前简化执行、未来平滑升级」的路径，确保治理框架从单人开发到社区化治理的过渡是渐进的、可落地的。

**单人开发的产能杠杆**：单人开发能支撑 158,181 行代码、1064 个测试用例、超 1 万次提交的项目，核心在于**多层产能杠杆**：

1. **Agent 自举**（6.5 节）：99% 以上的提交由 Agent 自主生成——开发者以自然语言描述需求，Agent 完成代码编写、测试、构建验证后自动提交，人类负责审查与合并。Agent 将单人产能放大了一个数量级
2. **CI/CD 自动化**（6.6 节）：四条流水线（test/publish/docker-publish/deploy-docs）自动化执行测试、发布与文档部署，替代了传统团队中需要专人负责的工程基础设施工作
3. **符号数据库与影响分析**（3.1 节）：Agent 在修改代码时基于符号图精确识别影响范围，减少人工审查的认知负担——单人开发无需逐行审查所有代码，只需审查 Agent 提交的关键变更
4. **分层记忆与知识沉淀**（3.5 节）：项目长期记忆与全局长期记忆使 Agent 的经验可跨会话复用，避免重复劳动——Agent 每次开发都基于历史经验，而非从零开始
5. **测试保障**（6.1 节）：1064 个测试用例构成安全网，Agent 提交的代码必须通过全量测试才能合并，降低了人工回归测试的负担

**可持续性保障**：单人开发的可持续性由以下因素保障——核心开发者具备完整的架构认知与决策能力（治理结构 7.2 节）；Agent 自举使产能不随人力规模线性受限；社区贡献（4 人贡献者、6 个 PR 全合并）开始形成外部输入；路线图（7.1 节）规划了从 1 人到社区化的渐进路径。单人开发是**起点**而非**终点**——治理框架已为社区壮大预留了平滑升级路径。

**决策机制**：

- **日常决策**：核心维护者负责日常 PR 审查与合并，遵循「先测试、后合并」原则，所有合并必须通过 CI 全量测试
- **重大特性**：引入 RFC（Request for Comments）流程，重大特性先提交设计文档，经社区讨论达成共识后再实现
- **冲突解决**：技术分歧以「可运行、可测试、可维护」为判断标准，必要时由核心维护者决策并记录于 Issue
...
- **CI/CD**：四条流水线保证代码质量
- **测试体系**：84 个测试文件，1064 个测试用例，覆盖核心功能，保证稳定性
- **版本迭代**：已迭代至 5.0.1，持续活跃
...
- 宠物大厅 + 实时网络拓扑，多 Agent 运行状态一图可见、可直接交互
...
- **真实场景验证**：中兴通讯内部深度使用验证（工具集成 + 用户协作）、开放原子大赛总冠军（单兵能力竞赛验证）
...
| **开源治理** | 20%  | 6.1 项目规模（319 文件/158181 行/1064 测试）、6.3 贡献机制（PR 6 全合并）、6.5 Agent 自举识别机制、6.8 社区数据（Star 133/Issue 47 全关闭）          | MIT 开源、CI/CD 四条流水线、Agent 自举与开源治理协同、社区阶段匹配性   |
| **长期发展** | 20%  | 7.1 路线图（量化 KPI、保守 Star 目标）、7.2 治理结构（单人开发产能杠杆）、7.3 社区支撑（网络效应路径、获客渠道）、8.5 与巨头差异化生存空间           | 可验证的量化目标、多层产能杠杆、四层获客渠道、差异化生态位             |
...
从第一章提出的四象限协作模式，到第二章的技术架构支撑，再到第三章的能力展开、第四章的对比验证、第五章的实证落地——Jarvis 的答案始终如一：**让 AI 从独自工作走向与众共事**。单兵能力是起点，协作是路径，生态是归宿。Jarvis 不仅是一个工具，更是一个可自我进化的协作平台——它用自身能力开发自身（超 1 万次 Agent 自主提交），在真实场景中验证价值（中兴通讯落地、开放原子大赛总冠军），以开放姿态拥抱社区（MIT 开源、插件生态）。

**独当一面，与众共事**——这是 Jarvis 的定位，也是 Jarvis 的承诺。

---

_文档版本：5.0.1（本轮：同步 180 个后续提交带来的事实数据变更——版本号、代码行数、测试文件/用例数、内置工具数、gateway_manager 操作数；补充节点与 Agent 拓扑视图、宠物大厅、协作平台界面等运行截图）_
_最后更新：2026-09-13_
