...
Jarvis 的核心能力可概括为两大支柱：**独立工作能力**与**协作共事能力**。本章以「人 × Agent」四象限为骨架，逐一展开每种协作模式的技术支撑与差异化优势；3.5 节进一步展示支撑协作的底层能力——插件机制、工具自扩展、分层记忆、知识沉淀、规则加载、MCP 集成、多模型调度、专项能力与宠物大厅，它们不是独立的协作模式，而是让每种协作模式都更强大的基础设施。
...
本节九个能力分五组：**扩展机制**（3.5.1 插件机制、3.5.2 工具自扩展）让内核能力按需扩展、让工具集随需求增长；**知识管理**（3.5.3 分层记忆、3.5.4 知识沉淀、3.5.5 规则加载）让经验可沉淀、可复用；**生态与调度**（3.5.6 MCP 集成、3.5.7 多模型调度）让 Jarvis 融入开放生态并优化成本；**工程深水区**（3.5.8 专项能力）覆盖主流工具未触及的迁移、审计、跨环境操作；**交互体验**（3.5.9 宠物大厅）让多 Agent 的运行状态从「列表与日志」变为「一眼可见、可交互的协作现场」。
