...
| `JARVIS_LLM_GROUPS`             | `[]`                                      | 预定义的LLM配置组列表。每个组可以指定 `JARVIS_PLATFORM`, `JARVIS_MODEL`, `JARVIS_MAX_INPUT_TOKEN_COUNT`, `JARVIS_CHEAP_PLATFORM`, `JARVIS_CHEAP_MODEL`, `JARVIS_SMART_PLATFORM`, `JARVIS_SMART_MODEL`，以及一个可选的 `ENV` 字典用于覆盖当前会话的环境变量。 |
