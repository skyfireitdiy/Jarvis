...
1. `rag_groups` 中不再支持直接定义 `embedding_model`、`embedding_type`、`rerank_model`、`reranker_type` 等参数，必须通过 `embedding` 引用 `embeddings` 中定义的配置（必需），可选通过 `reranker` 引用 `rerankers` 中定义的配置。
2. 顶层 `rag` 配置项已被移除，所有RAG相关配置必须在 `rag_groups` 中进行定义。
...
| `rag_groups` | `{"text": {"embedding": "default-text-embedding", "reranker": "default-text-reranker", "use_bm25": true, "use_rerank": true}, "code": {"embedding": "default-code-embedding", "use_bm25": false, "use_rerank": false}}` | 预定义的RAG配置组，使用组名作为 key（对象格式）。每个配置组必须通过 `embedding` 引用 `embeddings` 中定义的配置（必需），可选通过 `reranker` 引用 `rerankers` 中定义的配置，不再支持直接定义 `embedding_model`、`embedding_type` 等参数。每个配置组可包含 `embedding`（引用 embeddings 中的配置名称，必需）、`reranker`（引用 rerankers 中的配置名称，可选）、`use_bm25`（是否使用BM25）、`use_rerank`（是否使用重排）等配置项。 |
...
| `conversation_turn_threshold` | `200` | 对话轮次阈值，用于触发总结。当对话轮次超过此值时，系统会自动触发对话总结，清理历史记录并生成摘要。 |
| `enable_tmux` | `true` | 是否启用tmux功能。当设置为 `false` 时，系统将禁用所有tmux相关功能，包括自动启动tmux会话、任务派发到tmux窗口、批量任务并行执行等。 |
