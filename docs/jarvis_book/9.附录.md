...
**注意**：`rag_groups` 中不再支持直接定义 `embedding_model`、`embedding_type`、`rerank_model`、`reranker_type` 等参数，必须通过 `embedding` 引用 `embeddings` 中定义的配置（必需），可选通过 `reranker` 引用 `rerankers` 中定义的配置。
