...
| `JARVIS_LLM_GROUPS`             | `[]`                                      | 预定义的LLM配置组列表。每个组可以指定 `JARVIS_PLATFORM`, `JARVIS_MODEL`, `JARVIS_MAX_INPUT_TOKEN_COUNT`, `JARVIS_CHEAP_PLATFORM`, `JARVIS_CHEAP_MODEL`, `JARVIS_CHEAP_MAX_INPUT_TOKEN_COUNT`, `JARVIS_SMART_PLATFORM`, `JARVIS_SMART_MODEL`, `JARVIS_SMART_MAX_INPUT_TOKEN_COUNT`，以及一个可选的 `ENV` 字典用于覆盖当前会话的环境变量。 |
| `JARVIS_LLM_GROUP`              | `""`                                      | 选择要激活的LLM组名称。                                     |
| `JARVIS_MAX_INPUT_TOKEN_COUNT`    | `128000`                                   | 输入给模型的最大token数量 (可被模型组覆盖)。                   |
| `JARVIS_PLATFORM`                 | `openai`                                  | 默认AI平台 (可被模型组覆盖)。                                |
| `JARVIS_MODEL`                    | `gpt-5`                                   | 默认模型 (可被模型组覆盖)。                                  |
| `JARVIS_CHEAP_PLATFORM`           | `""`                                      | 廉价操作的平台名称。如未配置，则使用 `JARVIS_PLATFORM` 的值 (可被模型组覆盖)。 |
| `JARVIS_CHEAP_MODEL`              | `""`                                      | 廉价操作的模型名称。如未配置，则使用 `JARVIS_MODEL` 的值 (可被模型组覆盖)。 |
| `JARVIS_CHEAP_MAX_INPUT_TOKEN_COUNT` | `0`                                      | 廉价模型允许的最大输入token数量。如未配置（值为0），则使用 `JARVIS_MAX_INPUT_TOKEN_COUNT` 的值 (可被模型组覆盖)。 |
| `JARVIS_SMART_PLATFORM`           | `""`                                      | 智能操作的平台名称。如未配置，则使用 `JARVIS_PLATFORM` 的值 (可被模型组覆盖)。 |
| `JARVIS_SMART_MODEL`              | `""`                                      | 智能操作的模型名称。如未配置，则使用 `JARVIS_MODEL` 的值 (可被模型组覆盖)。 |
| `JARVIS_SMART_MAX_INPUT_TOKEN_COUNT` | `0`                                      | 智能模型允许的最大输入token数量。如未配置（值为0），则使用 `JARVIS_MAX_INPUT_TOKEN_COUNT` 的值 (可被模型组覆盖)。 |
...
| `JARVIS_ENABLE_IMPACT_ANALYSIS`  | `true`                                    | 是否启用编辑影响范围分析。分析代码编辑的影响范围，识别可能受影响的文件、函数、测试等，帮助评估编辑风险。 |
...
| `JARVIS_ENABLE_INTENT_RECOGNITION` | `true`                                    | 是否启用意图识别功能。用于智能上下文推荐中的LLM意图提取和语义分析。默认开启。 |
| `JARVIS_ENABLE_MEMORY_ORGANIZER` | `false`                                   | 是否启用自动记忆整理功能。在任务完成后自动检测记忆库状态，当记忆数量超过200条且存在重叠时提示用户整理。默认关闭。 |
| `JARVIS_CONVERSATION_TURN_THRESHOLD` | `50`                                      | 对话轮次阈值，用于触发总结。当对话轮次超过此值时，系统会自动触发对话总结，清理历史记录并生成摘要。 |
