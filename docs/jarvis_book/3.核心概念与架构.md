...
2. **项目规则**: 存储在项目 `.jarvis/rules/rule.md` 文件，仅适用于当前项目。
...
package "可插拔能力" #Orange {
    ...
    package "记忆系统" {
        ...
        [全局长期记忆\n(~/.jarvis/memory/global_long_term)] as GlobalMem
...
- **多源规则加载**：
  ...
  - 项目规则：存储在项目 `.jarvis/rules/rule.md` 文件
...
**职责**：实现复杂任务的模块化执行，支持任务依赖、状态跟踪、会话记忆增强。
...
- **职责**：提供 tmux 自动启动、任务派发到 tmux 窗格等核心功能（需配置 `enable_tmux: true`）
- **核心功能**：
...
1. **任务派发**：将独立任务派发到新窗格执行，主会话可继续其他操作
2. **代码修改与监控**：一边执行代码修改，一边监控输出
3. **多项目协作**：在不同窗格中处理多个项目任务
