...
#### LSP 客户端工具（CodeAgent 专用）

`lsp_client` 是一个专为 CodeAgent 设计的代码分析工具，它通过 Language Server Protocol (LSP) 提供强大的代码符号分析能力。

**核心特性**：

- **基于符号名称的操作**：所有操作都基于符号名称（函数名、类名、变量名等），无需手动指定行列号，极大简化了代码分析流程
- **多语言支持**：支持 Python、TypeScript、JavaScript、C/C++、Rust、Go、Java 等主流语言的 LSP 服务器
- **智能后备方案**：当 LSP 服务器不可用时，自动降级到 Tree-sitter 进行符号提取，确保功能的可用性
- **自动工作目录检测**：自动检测 LSP 服务器所需的工作目录（如 `Cargo.toml`、`pyproject.toml`、`package.json` 等），确保 LSP 服务器能够正确初始化
...
- **LSPClient 类**：管理与 LSP 服务器的连接，处理 LSP 协议的通信细节（JSON-RPC、Content-Length 等）
- **TreeSitterFallback 类**：当 LSP 服务器不可用时的后备方案，使用 Tree-sitter 进行符号提取
- **客户端缓存机制**：按项目根目录和语言类型缓存 LSP 客户端连接，提高性能并防止资源泄露
- **自动降级策略**：检测 LSP 服务器可用性，不可用时自动切换到 Tree-sitter 后备方案

**在 CodeAgent 中的应用**：

- CodeAgent 的系统提示词中明确指导使用 `lsp_client` 进行代码分析
- 优先使用 `lsp_client` 的 `search_symbol` 定位关联文件
- 使用 `document_symbols`、`get_symbol_info`、`definition`、`references` 进行深度代码分析
- 在修改前使用 `references` 查询待修改符号的所有引用场景，评估影响范围

**可用性检查**：

- 工具通过 `check()` 方法检查是否在 CodeAgent 模式下可用
- 仅在 CodeAgent 模式下加载和使用，避免在通用 Agent 中误用
