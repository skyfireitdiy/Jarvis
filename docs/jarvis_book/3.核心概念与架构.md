...
- 代理（Agent）
  ...
  - 广播关键事件（task_started/before_model_call/after_model_call/before_tool_call/after_tool_call/before_history_clear/after_history_clear/after_summary/task_completed 等）

- 运行循环（AgentRunLoop）
  - 承载主循环逻辑：调用模型、解析并执行工具、处理中断、驱动下一步
  - 使用 LoopAction 表达跳过当前轮（skip_turn）、继续（continue）、完成（complete）
  - 处理自动完成：当开启 auto_complete 且检测到 !!!COMPLETE!!! 标记时直接收尾
  - 在工具调用与模型调用等关键节点通过 EventBus 广播事件
...
- 输出处理器（Output Handler）
  - 一个可扩展的处理器链，默认包含 ToolRegistry；Agent 将模型响应委派给能处理的处理器，由处理器解析并执行
...
#### 关联组件与内部模块
- EventBus（事件总线）: 提供简单的发布/订阅机制（before_model_call/after_model_call、before_tool_call/after_tool_call、before_history_clear/after_history_clear、task_started/task_completed 等），以旁路方式集成能力，回调异常隔离不影响主流程。
...
- ToolExecutor（工具执行器）: 通过 `execute_tool_call` 统一解析与执行模型返回的工具调用，支持执行前确认和错误处理。
