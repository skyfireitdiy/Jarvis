...
package "用户交互层 (User Interaction Layer)" #AliceBlue {
  [<font-awesome/users> 用户] as User
  User -right-> [<font-awesome/terminal>\nCLI Tools\n(jvs, jca, jgc, ...)] as CLI
}
...
package "目标系统 (Target System)" #LightGrey {
  [<DEVICONS/git>\n代码库] as Codebase
  [操作系统 (OS)] as OS
  [Web 服务] as WebServices
  [本地知识库] as KnowledgeBase
}

CLI -down-> Agent : "用户指令"
Agent -right-> ToolRegistry

ToolRegistry --> CodeTools
ToolRegistry --> SystemTools
ToolRegistry --> WebTools
ToolRegistry --> RAGTools

CodeTools --> Codebase : "读/写/分析"
SystemTools --> OS : "执行命令"
WebTools --> WebServices : "API 请求"
RAGTools --> KnowledgeBase : "查询/构建"
...
-   **Jarvis 核心框架**: 这是系统的大脑。
    ...
    -   **方法论**: 一个独特的学习系统，使 Jarvis 能够从交互中总结经验，优化未来的行为模式。
    -   **MCP**: 支持多智能体协作，使 Jarvis 可以与其他智能体或外部服务协同工作。
-   **工具生态系统**: 一系列即插即用的工具，负责执行具体任务，如代码操作、系统命令执行等。这个生态系统是可扩展的。
-   **目标系统**: Jarvis 操作的实际环境，包括本地的代码库、操作系统、Web服务和知识库。
