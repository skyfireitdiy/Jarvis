...
```bash
jarvis-service --node-mode master --host 0.0.0.0 --port 8000 --gateway-password your_password
```
...
Environment="JARVIS_NODE_SECRET=your_secret"
ExecStart=/usr/local/bin/jarvis-service --node-mode master --host 0.0.0.0 --port 8000
Restart=always
