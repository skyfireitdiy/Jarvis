...
llm_groups:
  # LLM组一：使用强大的模型组合
  power_group:
    normal_llm: openai-power
    cheap_llm: ollama-speed
    # 可选：配置备用模型组，非交互模式下本组模型连续失败时自动切换
    fallback_group: speed_group
  # LLM组二：使用快速经济的模型组合
  speed_group:
    normal_llm: ollama-speed
    # 如果不指定思考模型, 将自动使用常规模型
llms:
  openai-power:
    platform: openai
    model: gpt-4o
    max_input_token_count: 1280
  ollama-speed:
    platform: ollama
    model: llama3
# 2. 选择要激活的LLM组
llm_group: power_group
```

### 备用模型组（fallback_group）

每个 LLM 组可通过可选的 `fallback_group` 指定一个备用模型组（取值必须是 `llm_groups` 中已定义的组名）。当当前组的模型**连续失败**（重试耗尽仍返回空结果）时，Jarvis 会自动切换到备用组并重试一次。

- **仅在非交互模式下生效**：交互模式（如人工对话）下不会自动切换，避免打断用户的模型选择。
- **未配置 `fallback_group` 时不切换**：无备用组时保持原有失败行为。
- **只切换一次**：切换后若仍失败，不会继续切换到其他组，避免无限循环。
- 切换仅影响全局 `llm_group` 并重建模型实例（保留当前对话消息），后续对话将继续使用备用组。
...
您可以在配置文件 `~/.jarvis/config.yaml` 中通过 `replace_map` 定义自己的快捷指令。
...
# 默认行为由全局配置 `force_save_memory` (默认为 true) 控制。
# 您可以在这里覆盖该设置：
...
与 `ja` (自定义代理) 专注于执行复杂任务不同，有时您可能只是想快速与一个具有特定“人设”和知识领域的 AI 进行纯粹的对话。`jpm role` 功能就是为此设计的。
...
### jpm 其他命令：info / chat / service

平台管理器还提供列出平台、直接聊天以及启动 OpenAI 兼容 API 服务的能力（由 jarvis_platform_manager.main 提供）：

- info：列出模型组对应的平台与模型
- chat：与指定模型组进行交互式聊天
- service：启动 OpenAI 兼容的 HTTP 服务
...
- 语法：
  - jpm info
  - jpm info -g `llm_group`
- 行为说明：
  - 未指定 `-g` 时，使用默认模型组对应的平台，调用 get_model_list() 展示模型与描述。
  - 指定 `-g` 时，展示该模型组对应平台的模型列表。
- 示例：
  - jpm info
  - jpm info -g power_group

#### 2) jpm chat

- 用途：使用指定模型组进行交互式聊天，支持多种会话管理命令。
- 语法：
  - jpm chat [-g/--llm-group `group`]
- 参数优先级：
  1. 命令行参数 -g/--llm-group
  2. 全局默认配置（未指定 llm_group 时使用 get_normal_platform_name() / get_normal_model_name()）
- 交互式命令：
  - /bye：退出
  - /clear：清空平台会话（reset）并重置模型；清空本地对话历史
  - /upload `path`：上传文件（需平台支持；调用 platform.support_upload_files 与 upload_files）
  - /save `file`：保存最后一条助手回复到文件
  - /saveall `file`：保存完整对话到文件（role: content）
  - /save_session `file`：调用 platform.save 持久化当前会话
  - /load_session `file`：调用 platform.restore 恢复会话，本地历史将被清空
  - /shell `command`：在本地 shell 中执行命令（慎用）
- 示例：
  - jpm chat
  - jpm chat -g power_group
- 注意：
  - 发生异常时会进行错误提示；会话结束前尝试调用 platform.reset 进行清理。

#### 3) jpm service（OpenAI 兼容 API）

- 用途：启动 OpenAI 兼容的 REST 服务，支持 /v1/models 与 /v1/chat/completions。
- 语法：
  - jpm service [--host 0.0.0.0] [--port 8000]
- 端点与行为：
  - GET /v1/models
    - 对当前默认平台（由配置中的默认模型组决定）列出模型，返回 OpenAI 风格的模型清单（id 为 platform/model）。
  - POST /v1/chat/completions
    - 请求体（主要字段）：model、messages（[{role, content}]）、stream（可选）
    - model 可为 "platform/model"；若仅提供 "model" 则使用默认平台，否则返回 400。
    - 非流式：返回标准 OpenAI chat.completion JSON。
    - 流式：返回 text/event-stream（SSE），以 chat.completion.chunk 片段逐步输出，并以 [DONE] 结束。
- 日志：
  - 优先写入环境变量 `JARVIS_LOG_DIR` 指定目录（环境变量仍使用大写）；否则写入 ~/.jarvis/logs；若失败则退回到 ./logs。
  - 每次会话以纯文本记录，包含消息与模型信息。
- 跨域：
  - 已启用 CORS：允许任意来源（适用于开发/测试）。
- 示例：
  - 启动服务：
    - jpm service --host 0.0.0.0 --port 8000
  - 非流式请求：
  ...
  - 流式请求：
    ...
    curl -N -X POST http://127.0.0.1:8000/v1/chat/completions \
      -H "Content-Type: application/json" \
      -d '{"model":"openai/gpt-4o","stream":true,"messages":[{"role":"user","content":"流式测试"}]}'
    ```
...
2. **外部方法论目录**: 由 `methodology_dirs` 配置指定的目录列表。

**通过 Git 共享方法论**:

`methodology_dirs` 的设计初衷就是为了方便团队协作和知识共享。您可以：

1. 创建一个 Git 仓库，专门用于存放团队的最佳实践和方法论 (`.json` 文件)。
2. 团队成员将此仓库克隆到本地。
3. 设置配置 `methodology_dirs` 指向该仓库的本地路径。
...
通过设置 `central_methodology_repo` 配置指定 Git 仓库地址：
...
在您的全局配置文件 `~/.jarvis/config.yaml` 中，您可以定义 `tool_groups` 列表，并用 `tool_group` 来选择当前激活的组。
...
2. **通过配置设置**：
   ...
   ```bash
   # 临时切换到安全模式（在 ~/.jarvis/config.yaml 中设置）
...
## 6.9 工具调用后回调扩展（after_tool_call_cb）
...
- 代码注册：在运行时通过 `agent.event_bus.subscribe(AFTER_TOOL_CALL, cb)` 订阅回调（相同回调不会重复添加）
- 配置注册：在配置文件中指定 `after_tool_call_cb_dirs`，Agent 启动时自动扫描并注册
...
系统会扫描 `after_tool_call_cb_dirs` 指定目录下的所有 .py 文件（忽略 **init**.py），并按以下约定依次尝试注册：

1. 模块级可调用对象

- 名称：after_tool_call_cb
- 签名：接收关键字参数（含 agent），即 `def after_tool_call_cb(**kwargs)`

1. 工厂函数 get_after_tool_call_cb()

- 返回单个回调或回调列表
- 签名：() -> Callable 或 List[Callable]

1. 工厂函数 register_after_tool_call_cb()
...
- 相同回调不会重复添加
- 回调文件头部可用 `# requirements: pkg1 pkg2` 声明依赖，加载时会自动安装

### 6.9.3 执行时机与顺序

- 执行时机：每次工具调用完成后，先依次调用已订阅的回调，再广播 AFTER_TOOL_CALL 事件
...
agent.event_bus.subscribe(AFTER_TOOL_CALL, my_cb)
```
...
| 配置项 (Key)               | 描述                                                                                                                                                                                      | 默认值  |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `llm_group`                | 当前激活的模型组名称。                                                                                                                                                                    | `""`    |
| `llms`                     | 可复用的 LLM 配置定义（平台、模型、最大输入 token 等），可在 `llm_groups` 中按名称引用。                                                                                                  | `{}`    |
| `llm_groups`               | 预定义的模型配置组，通过 `normal_llm`/`cheap_llm`/`smart_llm`/`eval_llm` 引用 `llms` 中的配置。每个组可选用 `fallback_group` 指定备用组（非交互模式下连续失败时自动切换）。                                                                                           | `{}`    |
| `execute_tool_confirm`     | 在执行任何工具前是否需要用户二次确认。                                                                                                                                                    | `false` |
| `methodology_dirs`         | 额外的方法论加载目录列表。                                                                                                                                                                | `[]`    |
| `central_methodology_repo` | 中心方法论 Git 仓库地址，用于团队共享方法论。                                                                                                                                             | `""`    |
| `central_tool_repo`        | 中心工具仓库地址，用于团队共享自定义工具。                                                                                                                                                | `""`    |
| `central_rules_repo`       | 中心规则仓库地址，用于团队共享规则。                                                                                                                                                      | `""`    |
| `print_prompt`             | 是否在控制台打印发送给 LLM 的完整提示，用于调试。                                                                                                                                         | `false` |
| `git_commit_prompt`        | `jgc` 命令生成 Git Commit Message 时使用的自定义模板。                                                                                                                                    | `""`    |
| `replace_map`              | 定义在交互中使用的自定义快捷命令替换规则。                                                                                                                                                | `{}`    |
| `mcp`                      | 一个列表，用于定义所有模型上下文协议（MCP）的端点配置。支持三种类型：`stdio`（本地命令行程序）、`sse`（Server-Sent Events 远程服务）、`streamable`（流式 HTTP 服务）。详见 MCP 配置章节。 | `[]`    |
| `tool_group`               | 当前激活的工具组名称（详见工具组配置章节）。                                                                                                                                              | `""`    |
| `tool_groups`              | 预定义的工具组列表（详见工具组配置章节）。                                                                                                                                                | `[]`    |
| `tool_load_dirs`           | 自定义工具的加载目录列表。                                                                                                                                                                | `[]`    |
| `after_tool_call_cb_dirs`  | 工具调用后回调的扫描目录列表（详见 6.10 节）。                                                                                                                                            | `[]`    |
| `enable_tmux`              | 是否启用 tmux 功能。当设置为 `true` 时，系统将启用 tmux 相关功能，包括自动启动 tmux 会话、任务派发到 tmux 窗口等。                                                                        | `false` |
| `enable_native_tool_calls` | 是否启用原生工具调用（function calling）协议。                                                                                                                                            | `true`  |
| `save_session_history`     | 是否保存会话记录。                                                                                                                                                                        | `false` |
| `auto_resume_session`      | 是否自动恢复上次会话。                                                                                                                                                                    | `false` |
| `ENV`                      | 一个字典，用于为 Jarvis 运行环境设置临时的环境变量。                                                                                                                                      | `{}`    |

> 完整且始终最新的配置项清单请参考 `src/jarvis/jarvis_data/config_schema.json`。
...
1. **服务发现**: Jarvis 启动时，`ToolRegistry` 会根据配置（`mcp`）连接到指定的 MCP 端点（Endpoint）。
...
mcp:
  - type: "stdio"
    ...
    args: ["--json-rpc", "--verbose"]
    env:
      CUSTOM_VAR: "value"
    enable: true
```

**2. sse 类型（Server-Sent Events 远程服务）**:
...
2. **相似性检测**
   - 使用组合数学计算所有记忆对的标签重叠度
   - 采用贪心算法扩展记忆组，确保组内每对记忆都有至少 2 个重叠标签（当前实现固定为 2）
...
- **固定阈值**：当前实现固定使用 2 个重叠标签作为合并门槛，避免过度合并
...
Jarvis 支持分布式网关部署模式，允许你在多台机器上部署 Web Gateway，并通过统一的入口访问所有节点上的 Agent。这种模式特别适合需要利用多机资源、异地协作或负载分担的场景。

### 6.14.1 架构概述

分布式网关采用主从架构：

- **主节点 (Master)**：对外提供统一的访问入口，维护所有子节点的注册信息
- **子节点 (Child)**：通过 WebSocket 长连接接入主节点，实际承载 Agent 的运行
- **客户端**：只需连接主节点，主节点会自动将请求转发到对应的子节点
...
```bash
jarvis-service run --node-mode master --gateway-host 0.0.0.0 --gateway-port 8000
```
...
- **节点模式 (`--node-mode`)**：`master` 表示主节点，`child` 表示子节点
- **节点 ID (`--node-id`)**：子节点的唯一标识，创建 Agent 时用于指定目标节点
- **主节点地址 (`--master-url`)**：子节点连接主节点的 WebSocket 地址
- **节点密钥 (`--node-secret`)**：用于节点间通信的安全校验

### 6.14.4 使用场景

1. **多机资源利用**：将计算密集型任务分散到多台高性能机器上
2. **团队协作**：团队成员在不同地点访问统一的 Jarvis 服务入口
3. **环境隔离**：不同子节点可以配置不同的运行环境（如不同的 Python 版本、工具集）
4. **负载分担**：根据机器性能分配不同数量的 Agent

### 6.14.5 生产环境部署建议

对于生产环境，建议使用 systemd 或 Docker Compose 进行部署：

#### 使用 install 命令自动安装 systemd 服务

Jarvis 提供了便捷的 `install` 命令来自动创建和安装 systemd 用户服务：
...
# 启用服务（开机自启）
systemctl --user enable jarvis-master.service
...
services:
  jarvis-master:
    image: your-jarvis-image
    command: run --node-mode master --gateway-host 0.0.0.0 --gateway-port 8000
    environment:
...
更多详细部署方法请参考 [分布式网关部署方法](../用户手册/04_web_界面与网关/分布式网关部署方法.md)。
