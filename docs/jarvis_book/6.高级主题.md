...
在 `~/.jarvis/config.yaml` 中，您可以定义 `rag_groups`（对象格式），并用 `rag_group` 来选择当前激活的组。

```yaml
# 1. 定义可用的 RAG 配置组（使用对象格式）
rag_groups:
  # 默认的文本RAG组
  text: # "text" 是组名
...
# 2. 定义 embeddings 和 rerankers（供 rag_groups 引用）
embeddings:
...
1. **顶层 `rag` 配置**: 在 `rag` 键下直接设置的 `embedding_model`, `use_bm25` 等，优先级最高，会覆盖所有组配置。
2. **RAG 组配置**: 由 `rag_group` 选中的配置。
...
- 示例：
  - 启动服务：
    - jpm service --host 0.0.0.0 --port 8000 -p kimi -m moonshot-v1-8k
  - 非流式请求：

    ```bash
    curl -X POST http://127.0.0.1:8000/v1/chat/completions \
      -H "Content-Type: application/json" \
      -d '{"model":"kimi/moonshot-v1-8k","messages":[{"role":"user","content":"你好"}]}'
    ```
  ...
  - 流式请求：
    ...
    curl -N -X POST http://127.0.0.1:8000/v1/chat/completions \
      -H "Content-Type: application/json" \
      -d '{"model":"kimi/moonshot-v1-8k","stream":true,"messages":[{"role":"user","content":"流式测试"}]}'
    ```
...
| `platform`   | 默认使用的 AI 平台名称。         | `openai` |
| `model`      | 默认使用的模型名称。             | `gpt-5`  |
...
| `enable_tmux` | 是否启用tmux功能。当设置为 `false` 时，系统将禁用所有tmux相关功能，包括自动启动tmux会话、任务派发到tmux窗口、批量任务并行执行等。 | `true` |
| `rag_group` | 当前激活的 RAG 配置组名称。 | `""` |
| `rag_groups` | 预定义的 RAG 配置组（对象格式）。 | 见默认配置 |
| `rag` | RAG框架的顶层配置，会覆盖组配置。 | `{}` |
| `ENV` | 一个字典，用于为 Jarvis 运行环境设置临时的环境变量。 | `{}` |

### 6.10.3 补丁格式控制（patch_format）
