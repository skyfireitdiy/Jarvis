...
### Q14: 如何设置 OpenAI 的推理努力程度（Reasoning Effort）？

**A14:** OpenAI 的 o1 系列模型支持通过 `reasoning_effort` 参数控制推理努力程度，影响模型的思考和回答质量。
...
### Q15: 如何使用自定义的 Anthropic API 地址？

**A15:** 类似于 OpenAI 的 `OPENAI_API_BASE`，您可以通过设置环境变量 `ANTHROPIC_BASE_URL` 来指定自定义的 Anthropic API 地址。
...
### Q16: 如何配置 Web Gateway 的密码认证？

**A16:** Web Gateway 支持通过配置文件或环境变量设置密码认证，保护您的 Jarvis Web 界面。
...
```bash
# Linux/macOS
export JARVIS_GATEWAY_PASSWORD="your-secure-password"

# Windows PowerShell
$env:JARVIS_GATEWAY_PASSWORD = "your-secure-password"
```
...
gateway_auth:
  ...
  allow_unset: false # 禁止未设置密码时访问
```

**说明:**

- 环境变量的优先级高于配置文件
- 设置密码后，访问 Web Gateway 时需要输入密码
- 建议使用强密码并定期更换

---

### Q17: 如何配置多个 API Key 实现轮询使用？

**A17:** API Key 现在支持配置多个 Key，请求时自动轮询使用，适用于需要分散请求压力或避免单个 Key 限流的场景。
