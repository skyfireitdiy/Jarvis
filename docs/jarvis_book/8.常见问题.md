...
### Q14: 如何配置和使用 RAG（检索增强生成）功能？

**A14:** RAG 功能允许您构建本地知识库并基于文档回答问题。
...
2. **查询知识库**
   ...
   ```bash
   # 提问
   jrg query "如何在项目中配置 RAG？"
   ```

3. **仅检索（不生成答案）**

   ```bash
   jrg retrieve "RAG 配置方法"
   ```

**注意事项:**

- 首次构建知识库可能需要较长时间
- 支持 `.ragignore` 文件来排除不需要索引的文件
- 默认使用 `jarvis_rag_collection` 作为主知识库名称
