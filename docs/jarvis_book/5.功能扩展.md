...
3. **额外工具目录**: 由 `tool_load_dirs` 配置指定的目录列表。

#### 通过 Git 共享和同步工具

与方法论类似，`tool_load_dirs` 的设计也考虑到了团队协作和版本控制。您可以将一个或多个包含Jarvis工具的Git仓库作为共享的工具库。

1. **创建工具仓库**: 创建一个 Git 仓库，用于存放团队共享的工具 (`.py` 文件)。
2. **克隆仓库**: 团队成员将此仓库克隆到本地。
3. **设置配置**: 设置 `tool_load_dirs` 指向该仓库的本地路径。可以指定多个路径，使用列表格式。
...
通过设置 `central_tool_repo` 配置来指定中心工具库的 Git 仓库地址：

```yaml
# 在 ~/.jarvis/config.yaml 中设置
central_tool_repo: "https://github.com/your-org/central-tools.git"
```
...
```bash
# 1. 配置中心工具库（在 ~/.jarvis/config.yaml 中设置）
# central_tool_repo: "https://github.com/team/jarvis-tools.git"
...
MCP 工具通过 `config.yaml` 文件中的 `mcp` 列表进行配置。每个列表项都定义了一个MCP客户端。
...
- `auth_token`: 用于认证的令牌 (可选)
- `headers`: 额外的 HTTP 请求头 (可选，对象格式)
...
- `auth_token`: 用于认证的令牌 (可选)
- `headers`: 额外的 HTTP 请求头 (可选，对象格式)
- `endpoint_path`: MCP 服务器端点路径 (可选，默认为 `"mcp"`)。如果 `base_url` 已包含端点路径，可设置为空字符串
- `timeout`: 请求超时时间数组 `[连接超时, 读取超时]` (可选，默认 `[10, 300]`，单位：秒)
- `enable`: 是否启用 (可选, 默认为 `true`)
...
class MyLLMPlatform(BasePlatform):
    ...
    def platform_name(cls) -> str:
        # 这个名称将用于 config.yaml 中的 platform
        return "my_llm"
...
# 在 ~/.jarvis/config.yaml 中设置
platform: my_llm
model: pro-model # 可选，根据你的实现
ENV:
  MYLLM_API_KEY: "YOUR_API_KEY"
```

1. **启动Jarvis**: 重新启动 `jarvis`，它会自动加载并使用你的新平台。
...
2. **方法论加载来源**：
   - 项目级方法论目录（若存在，优先级最高）
   - 全局方法论目录：`~/.jarvis/methodologies`
   - 额外配置目录：`methodology_dirs` 指定的目录
   - 中心方法论仓库：`~/.jarvis/central_methodology_repo`（若配置了 `central_methodology_repo`）

3. **重复检测**：
   - 当存在同名方法论时，以最终加载到的版本为准
   - 分享功能会自动检测并排除已存在的方法论
...
Jarvis 的规则系统允许您创建和管理自定义规则，用于指导 AI 的行为和代码生成。规则可以以多种方式组织和共享，支持团队协作和知识复用。

### 规则类型

Jarvis 的规则以 Markdown 文件（`.md`）的形式组织，每个文件即一条规则，文件名（可含子目录相对路径）即为规则名：

- 全局规则：`~/.jarvis/rules/<规则名>.md`（如 `~/.jarvis/rules/rule.md`）
- 项目规则：`.jarvis/rules/<规则名>.md`（如 `.jarvis/rules/rule.md`）

规则文件可以在开头使用 YAML Front Matter 声明元信息（如 `description`），Jarvis 会解析该头部用于生成规则索引与自动选择；Front Matter 之后的内容即为规则正文。例如：
...
> 注意：Jarvis **不支持** `rules.yaml` 这类“一个 YAML 文件定义多条命名规则”的格式，规则一律以目录下的 `.md` 文件形式存在。

### 规则加载位置和优先级

Jarvis 会从以下位置按优先级加载规则（从高到低）：

1. **中心规则仓库**（最高优先级）
   - 配置项：`central_rules_repo`
   - 存储位置：`~/.jarvis/central_rules_repo`（若仓库内含 `rules/` 子目录则使用该子目录）
   - 支持自动克隆和每日更新

2. **项目规则目录**
   - 位置：`.jarvis/rules/` 目录
   - 每个文件一个规则，文件名（含子目录相对路径）即为规则名

3. **全局规则目录**
   - 位置：`~/.jarvis/rules/` 目录

4. **配置的规则目录**
   - 配置项：`rules_load_dirs`（列表格式）
   - 支持多个目录，按配置顺序加载

5. **内置规则**（最低优先级）
   - Jarvis 系统内置的规则（`builtin/rules/`），作为后备
...
1. 自动克隆中心规则仓库到 `~/.jarvis/central_rules_repo`
2. 每日自动更新中心规则仓库（执行 `git pull`）
3. 从中心规则仓库加载所有规则
...
在项目 `.jarvis/rules/` 目录或全局 `~/.jarvis/rules/` 目录下创建规则文件：

```bash
# 创建项目规则
mkdir -p .jarvis/rules
echo "# TDD 规则
- 所有新功能必须先写测试
- 测试必须覆盖主要功能路径和边界情况
- 禁止使用 todo!() 宏" > .jarvis/rules/tdd

# 创建全局规则
mkdir -p ~/.jarvis/rules
echo "# 代码风格规则
- Python 代码必须遵循 PEP 8 规范
- 所有函数必须包含类型注解和 docstring" > ~/.jarvis/rules/python_style
```

#### 方式二：使用子目录组织与 YAML Front Matter 描述

规则目录支持递归遍历子目录，因此可以用子目录对规则进行分类；规则名即为其相对路径。例如：

```bash
mkdir -p .jarvis/rules/deployment
cat > .jarvis/rules/deployment/version_release.md <<'EOF'
---
description: 版本发布流程规范
---
- 发布前必须通过全部测试
- 版本号遵循语义化版本规范
EOF
...
此时该规则的名称即为 `deployment/version_release.md`（在项目规则中可写作 `project:deployment/version_release.md`）。

Front Matter 中的 `description` 会被写入规则索引，供 AI 自动选择规则时参考；正文内容则是实际注入给 AI 的规则文本。
...
在用户输入中使用 `'<rule:rule_name>'` 标记动态注入规则：
...
您可以将本地开发的优秀规则分享到中心规则仓库，供团队其他成员使用。规则分享功能通过 `RuleShareManager` 实现，并已提供 CLI 命令：
...
**注意**：可分享的规则来源包括项目规则目录（`.jarvis/rules/`）、`rules_load_dirs` 配置的规则目录，以及内置规则目录（`builtin/rules/`）；中心规则仓库自身的规则会被排除。

规则分享流程：

1. 更新中心仓库到最新版本
2. 检测并显示可分享的规则列表（排除中心仓库中已存在的）
3. 支持选择单个或多个规则（格式：`1,2,3` 或 `4-9` 或 `all`）
4. **注意**：规则文件会被**复制**到中心库（原文件保留）
5. 自动提交并推送到中心规则仓库
...
```text
中心规则仓库: tdd, clean_code
项目 .jarvis/rules/: tdd, project_style
全局 ~/.jarvis/rules/: python_style, clean_code
配置的规则目录（rules_load_dirs）: security
```

当使用 `--rule-names tdd,clean_code` 时：

- `tdd` 规则：从中心规则仓库加载（最高优先级）
- `clean_code` 规则：从中心规则仓库加载（如果存在），否则从全局 `~/.jarvis/rules/` 加载

### 规则最佳实践

1. **命名规范**：使用清晰、描述性的规则名称，使用下划线分隔（如 `python_style`、`tdd_rules`）

2. **内容组织**：
   - 保持规则简洁明了，使用条目式清单
   - 每个规则聚焦一个主题（代码风格、测试、安全等）
   - 避免冗长的背景描述
...
5. **规则复用**：
   - 在多个规则中引用通用规则
   - 使用子目录对规则分类，并借助 YAML Front Matter 补充描述，便于管理和维护
   - 定期审查和更新规则内容
...
# 使用 uv 安装项目（推荐）
# 如果未安装 uv，请先执行：curl -LsSf https://astral.sh/uv/install.sh | sh
...
3. **代码风格**：
   - 使用 Python 3.12 的特性（项目要求 `requires-python = "==3.12.*"`）
   - 遵循 PEP 8 代码风格指南
   - 使用类型注解提高代码可读性
...
4. **提交规范**：
   ...
   git commit -m "fix(platform): 修复 OpenAI 平台的流式响应问题"
