...
1. **纯文本规则文件**：简单的文本文件，包含规则内容
   - 全局规则：`~/.jarvis/rules/rule.md` 或 `~/.jarvis/rules/<规则名>`
   - 项目规则：`.jarvis/rules/rule.md` 或 `.jarvis/rules/<规则名>`

2. **命名规则文件（YAML）**：结构化的 YAML 文件，可以定义多个命名规则
   - 全局规则：`~/.jarvis/rules.yaml`
   - 项目规则：`.jarvis/rules.yaml`
...
1. **中心规则仓库**（最高优先级）
   ...
   - 存储位置：`~/.jarvis/central_rules_repo`
   - 支持自动克隆和每日更新
...
您可以将本地开发的优秀规则分享到中心规则仓库，供团队其他成员使用。规则分享功能通过 `RuleShareManager` 实现，可以通过编程方式调用，或等待未来版本添加 CLI 命令支持。

**注意**：只能分享默认规则目录（`~/.jarvis/rules/`）中的规则，不能分享配置的规则目录和项目规则目录中的规则。

规则分享流程：

1. 更新中心仓库到最新版本
2. 检测并显示可分享的规则列表（排除已存在的）
3. 支持选择单个或多个规则（格式：`1,2,3` 或 `4-9` 或 `all`）
4. **注意**：规则文件会被**移动**到中心库，而不是复制，原文件将被删除
5. 自动提交并推送到中心规则仓库
...
项目 .jarvis/rules.yaml: tdd, security
全局 ~/.jarvis/rules.yaml: python_style, clean_code
```

当使用 `--rule-names tdd,clean_code` 时：

- `tdd` 规则：从中心规则仓库加载（最高优先级）
- `clean_code` 规则：从中心规则仓库加载（如果存在），否则从全局 `rules.yaml` 加载
