...
- 强制任务输入: 由于无法在运行时交互式地输入任务，因此必须在启动命令中通过参数（如 `jvs -T "..."`, `jca -r "..."`）提供初始任务。如果未提供，程序将报错退出。
...
| `-r`, `--restore-session`              | 从 `.jarvis/saved_session.json` 恢复会话。                                                                             |
...
| `-r`, `--restore-session`        | 从 `.jarvis/saved_session.json` 恢复会话状态。                                                                                            |
...
### 4.4.2 记忆整理工具 (`jarvis-memory-organizer` / `jmo`)
