...
我们提供了交互式配置命令 `jqc` (jarvis-quick-config)，它可以引导您快速完成配置：
...
1. 选择 AI 平台（OpenAI 或 Claude）
2. 输入 API 基础 URL 和 API 密钥
3. 选择可用的模型
4. 配置 Normal/Smart/Cheap 模型
5. 测试 API 连接性
6. 自动保存配置到 `~/.jarvis/config.yaml`
...
# 支持数组格式或字符串格式（逗号分隔）
# 默认值为 Ctrl+J 和 Ctrl+]
# 常用快捷键：c-j (Ctrl+J), c-] (Ctrl+]), c-enter (Ctrl+Enter)
submit_keys:
  - c-j
  - c-]
...
API Key 现在支持配置多个 Key，请求时自动轮询使用，适用于需要分散请求压力或避免单个 Key 限流的场景。支持以下格式：

- **单个 Key（字符串）**：`openai_api_key: "sk-xxx"` — 与之前行为一致
- **逗号分隔（字符串）**：`openai_api_key: "sk-xxx1,sk-xxx2,sk-xxx3"` — 自动拆分并轮询
- **列表格式**：`openai_api_key: ["sk-xxx1", "sk-xxx2", "sk-xxx3"]` — 直接使用并轮询

环境变量同样支持逗号分隔格式：`OPENAI_API_KEY=sk-xxx1,sk-xxx2,sk-xxx3`
...
# 代码代理启用提示词优化
jca -o -T "实现一个完整的微服务架构，包括服务注册、配置中心和API网关"

# 组合使用多个参数
jca -o --worktree -T "重构整个用户管理模块，使用新的设计模式"
```
...
- 强制任务输入: 由于无法在运行时交互式地输入任务，因此必须在启动命令中通过参数（如 `jvs -T "..."`, `jca -T "..."`）提供初始任务。如果未提供，程序将报错退出。
...
| `-r`, `--restore`                      | 交互式选择恢复会话。                                                                                                   |
| `-R`, `--restore-session`              | 从指定会话文件恢复。                                                                                                   |
| `-e`, `--edit`                         | 快速编辑默认的 `~/.jarvis/config.yaml` 配置文件。                                                                      |
| `--share-methodology`                  | 分享本地方法论到中心方法论仓库。                                                                                       |
| `--share-tool`                         | 分享本地工具到中心工具仓库。                                                                                           |
| `--share-rule`                         | 分享本地规则到中心规则仓库。                                                                                           |
| `-I`, `--interactive-config`           | 启动交互式配置向导：基于当前配置补充设置。                                                                             |
| `-D`, `--disable-methodology-analysis` | 禁用方法论和任务分析（覆盖配置文件设置）。                                                                             |
| `-o`, `--optimize-system-prompt`       | 启用系统提示词自动优化。系统会根据用户需求使用大模型优化系统提示词，使其更贴合具体任务场景。详见"提示词自动优化"章节。 |
| `--backup-data [目录]`                 | 备份 Jarvis 数据目录。可选传入备份目录；默认使用 `~/jarvis_backups`。                                                  |
| `--restore-data <路径>`                | 从指定的压缩包恢复 Jarvis 数据。此操作会覆盖现有数据，请谨慎使用。                                                     |
| `-n`, `--non-interactive`              | 启用非交互模式：用户无法与命令交互，脚本执行超时限制为 5 分钟。                                                        |
| `-d`, `--dispatch`                     | 将任务派发到新的 tmux 窗口中执行（仅在 tmux 环境中有效），当前进程退出。                                               |
| `--rule-names <名称列表>`              | 指定规则名称列表（逗号分隔），用于加载指定规则。                                                                       |
| `-q`, `--quick-mode`                   | 极速模式：跳过方法论加载、规则筛选等，直接执行任务。                                                                   |
| `-k`, `--keep-jvs`                     | 禁止自动切换到代码开发模式（jca），保持使用通用代理（jvs）。                                                           |
| `-S`, `--skip-config-selector`         | 跳过内置配置选择器，直接使用默认通用代理。                                                                             |
| `--quick-config`                       | 启动快速配置向导：快速配置 LLM 平台信息（Claude/OpenAI）。详见"快速配置"章节。                                         |
| `--print-prompt`                       | 打印提示信息（覆盖配置文件设置）。                                                                                     |
| `--check`                              | 检查所有工具的安装情况（原 jck 命令）。                                                                                |
| `--check-lint` / `--check-build`       | 分别检查 lint 工具 / 构建工具。                                                                                        |
| `--check-json`                         | 以 JSON 格式输出检查结果（配合其他 check 选项使用）。                                                                  |
| `--check-tool <工具名>`                | 检查指定工具。                                                                                                         |
| `--web-gateway`                        | 启用 Web Gateway 服务（WebSocket 输入输出）。                                                                          |
| `--web-gateway-port <端口>`            | Web Gateway 监听端口（默认 8000）。                                                                                    |
| `--proxy-node <节点ID>`                | HTTP 代理请求转发到的目标节点 ID。                                                                                     |
| `--master-url <URL>`                   | Master 节点 URL，用于 HTTP 代理请求转发。                                                                              |
| `--agent-id <ID>`                      | Agent ID，由 Web Gateway 的 AgentManager 分配的唯一标识符。                                                            |
| `--install-plugin <路径>`              | 安装插件：指定插件目录或压缩文件路径（支持 .tar、.tar.gz、.tgz、.zip）。                                               |
| `--uninstall-plugin <名称>`            | 卸载插件：指定要卸载的插件名称。                                                                                       |
...
1. 交互式配置
   在启动前基于当前配置补充设置；保存后需重新运行命令以使用新配置：
...
1. 快速配置 LLM 平台
   使用 `--quick-config` 参数快速配置 Claude 或 OpenAI 平台：
...
- 输入 LLM 平台类型（claude/openai）
- 输入 API 基础 URL 和密钥
- 自动获取可用模型；获取失败时支持手动输入模型名称
- 选择要配置的模型
- 分别选择 normal / smart / cheap 三类默认模型
- 测试所选 normal 模型的 API 连通性
- 输入模型组名称（提供默认值建议）
- 设置默认最大 token 数
- 自动生成并保存配置到 `~/.jarvis/config.yaml`
...
- **任务列表管理**: 支持复杂任务的模块化执行，自动创建子任务、管理依赖关系、跟踪任务状态。
...
| `--task-file`                    | 从文件读取任务描述（支持纯文本或 JSON 格式）。                                                                                            |
| `-g`, `--llm-group`              | 临时指定模型组，覆盖默认配置。                                                                                                            |
| `-G`, `--tool-group`             | 临时指定工具组，覆盖默认配置。                                                                                                            |
| `-f`, `--config`                 | 指定自定义配置文件的路径。                                                                                                                |
...
| `-r`, `--restore`                | 交互式选择恢复会话。                                                                                                                      |
| `-R`, `--restore-session`        | 从指定会话文件恢复。                                                                                                                      |
| `--prefix <文本>`                | 为生成的提交信息主题添加前缀（用空格分隔）。                                                                                              |
| `--suffix <文本>`                | 为生成的提交信息末尾添加后缀（用换行分隔）。                                                                                              |
| `--rule-names <名称列表>`        | 指定规则名称列表，用逗号分隔，用于加载指定规则。详见"项目规则"章节。                                                                     |
| `--disable-review`               | 禁用代码审查：在代码修改完成后不进行自动代码审查。                                                                                        |
| `--review-max-iterations <次数>` | 代码审查最大迭代次数，达到上限后停止审查（默认 0，表示无限）。                                                                            |
| `-o`, `--optimize-system-prompt` | 启用系统提示词自动优化。系统会根据用户需求使用大模型优化系统提示词。详见"提示词自动优化"章节。                                           |
| `-n`, `--non-interactive`        | 启用非交互模式：用户无法与命令交互，脚本执行超时限制为 5 分钟。                                                                            |
| `-w`, `--worktree`               | 启用 git worktree 模式，在独立分支上开发。                                                                                                |
| `-d`, `--dispatch`               | 将任务派发到新的 tmux 窗口中执行（仅在 tmux 环境中有效），当前进程退出。                                                                  |
| `-q`, `--quick`                  | 极速模式：取消任务分类、规则自动加载、上下文推荐、方法论加载。                                                                            |
| `--print-prompt`                 | 打印提示信息（覆盖配置文件设置）。                                                                                                        |
| `--web-gateway`                  | 启用 Web Gateway 服务（WebSocket 输入输出）。                                                                                             |
| `--web-gateway-port <端口>`      | Web Gateway 监听端口（默认 8000）。                                                                                                       |
| `--proxy-node <节点ID>`          | HTTP 代理请求转发到的目标节点 ID。                                                                                                        |
...
```bash
jca -T "在用户认证模块中，增加一个'忘记密码'的功能。需要创建一个新的API端点 /api/forgot-password，并实现发送重置邮件的逻辑。"
```
...
```bash
jca -T "修复 src/utils/calculator.py 中的除零错误。当除数为零时，应该返回一个错误提示而不是抛出异常。"
```
...
```bash
jca -T "重构 src/services/user_service.py 文件，将其中过长的 'create_user' 方法拆分为多个更小的私有方法，提高代码可读性。"
```
...
```bash
# 使用 TDD 和代码清洁规则
jca --rule-names tdd,clean_code -T "实现用户登录功能"

# 使用项目特定的规则
jca --rule-names project_style -T "添加新的 API 端点"
```
...
```bash
jca --disable-review -T "快速修复一个简单的 bug"
```
...
```bash
# 最多进行 2 次审查迭代
jca --review-max-iterations 2 -T "实现复杂功能"
```
...
```bash
# 添加文件分析工具
jca --append-tools file_analyzer -T "分析项目依赖"
```
...
```bash
# 启用提示词优化，针对代码重构任务
jca -o -T "重构用户认证模块，使用新的加密算法，确保向后兼容"

# 启用提示词优化，针对新功能开发任务
jca -o -T "实现一个完整的支付系统，包括订单处理、支付网关集成和退款功能"
```
...
6. **全局规则目录**：全局数据目录下的 `rules/` 目录
7. **内置规则**（优先级最低）：Jarvis 系统内置的规则
...
3. 运行 `jca --rule-names <规则名称列表> -T "<任务描述>"` 来使用指定的规则
...
1. 与特定模型聊天
   首先，使用 `jpm info` 查找您想使用的平台和模型名称。然后通过 `-g` 参数指定模型组来开始对话。
...
```bash
# 示例：监听 0.0.0.0:8000，使用当前默认平台与模型
jpm service --host 0.0.0.0 --port 8000
```
...
# 你也可以通过参数指定角色配置文件与模型组
jpm role -c ./my_roles.yaml -g <llm_group_name>
```
...
- `install [--shell <类型>]`: 为指定 shell 安装“命令未找到”处理器（支持 `fish`、`bash`、`zsh`，默认 `fish`）。
- `uninstall [--shell <类型>]`: 卸载指定 shell 的处理器（支持 `fish`、`bash`、`zsh`，默认 `fish`）。
...
1. 安装“命令未找到”处理器 (支持 Fish / Bash / Zsh)
   这是 `jss` 的一个强大功能，它能让您像使用自然语言一样操作终端。

```bash
# 1. 安装处理器（默认 fish，可用 --shell 指定 bash/zsh）
jss install
...
代码审查功能已集成到 `jca`（代码代理）中。在代码修改完成后，`jca` 会自动进行代码审查，发现问题则自动修复。您可通过以下参数控制：

- `--disable-review`：禁用代码审查（快速模式）
- `--review-max-iterations <次数>`：限制代码审查最大迭代次数

详见 [4.5 代码代理](#45-代码代理-jarvis-code-agent--jca) 章节。
...
1. 查看工具详情
   `show` 命令可以查看指定工具的详细信息（包括参数定义），便于调用前确认参数格式。

```bash
# 查看指定工具的详细信息
jt show read_code
# 以 JSON 格式输出完整工具信息
jt show read_code --json
```
...
### 4.12.2 记忆整理工具 (`jarvis-memory-organizer` / `jmo`)
...
## 4.14 安全分析套件 (`jarvis-sec` / `jsec`)

`jsec` 是 Jarvis 的安全分析套件，专门用于对代码工程进行安全扫描和分析。它采用**配置驱动**的设计理念，通过配置文件管理扫描参数，支持启发式扫描和 AI 深度验证，能够识别潜在的安全漏洞和风险。

### 核心特性

- **配置驱动**：通过配置文件（`.jarvis/jsec/config.json`）管理扫描目标、语言过滤、输出格式等参数，便于复用和版本控制
- **命令行参数覆盖**：支持在 `scan` 命令中通过命令行参数临时覆盖配置，灵活便捷
...
- **外部工具集成**：支持从外部工具的 JSON 输出进行安全分析，自动学习非标准格式

### 工作流程

```
配置管理（jsec config）→ 执行扫描（jsec scan）→
直扫（direct_scan）→ 聚类（Cluster Agent）→ 分析（Analysis Agent）→
验证（Verification Agent）→ 复核（Review Agent）→ 报告聚合
```

1. **配置管理**：使用 `jsec config` 命令设置扫描目标、语言过滤、排除目录、输出格式等参数
2. **直扫（Direct Scan）**：使用预定义的正则表达式模式快速识别潜在安全问题
3. **聚类（Clustering）**：使用 LLM 将相似的安全问题聚类，减少重复分析
4. **分析（Analysis）**：为每个候选问题创建独立的 Agent 任务进行深度分析
5. **验证（Verification）**：使用独立的验证 Agent 对分析结果进行二次验证
6. **复核（Review）**：最终复核和报告聚合
...
- `config`：管理扫描配置文件
- `scan`：执行安全扫描（从配置文件读取）
- `analyze`：从外部 JSON 文件分析安全问题
- `heuristic`：启发式扫描（快速静态分析，无需 AI 深度验证）

#### `config` - 管理配置文件

配置文件位于 `.jarvis/jsec/config.json`，包含以下主要配置项：

- `target`：扫描目标（文件路径或目录路径）
- `languages`：语言过滤列表（如 `c`, `cpp`, `rust`）
- `exclude_dirs`：排除目录列表
- `output`：输出配置（format 和 file）
- `analysis`：分析参数（cluster_limit、enable_verification、force_save_memory）
- `runtime`：运行时配置（llm_group）

**参数**：

| 参数                                | 描述                                                                 |
| ----------------------------------- | -------------------------------------------------------------------- |
| `--target <路径>`                   | 设置扫描目标（文件路径或目录路径）                                    |
| `--languages <语言列表>`            | 设置语言过滤（逗号分隔，如 c,cpp,rust）                               |
| `--exclude-dirs <目录列表>`         | 设置排除目录（逗号分隔）                                             |
| `--output-format <格式>`            | 设置输出格式（csv/markdown）                                         |
| `--output-file <路径>`              | 设置输出文件路径                                                     |
| `--cluster-limit <数量>`            | 设置聚类限制（每批最多处理的告警数）                                  |
| `--enable-verification/--no-verification` | 是否启用二次验证                                                 |
| `--force-save-memory/--no-force-save-memory` | 是否强制使用记忆                                             |
| `--llm-group <组名>`                | 设置模型组                                                           |
| `--show`                            | 显示当前配置                                                         |
| `--clear`                           | 清空配置（重置为默认值）                                             |
...
# 设置模型组
jsec config --llm-group deep_seek_v3
...
从配置文件读取所有配置并执行扫描。如果配置文件不存在，会给出友好提示引导用户使用 `config` 命令。

**支持扫描模式**：
- **目录扫描**：扫描指定目录下的所有文件
- **文件扫描**：扫描指定文件（会扫描文件所在目录，然后过滤只保留该文件的问题）
...
# 2. 设置输出格式
jsec config --output-format markdown --output-file report.md
...
从外部扫描工具的 JSON 输出进行安全分析，支持自动学习非标准格式。

**参数**：

| 参数                                | 描述                                                                 |
| ----------------------------------- | -------------------------------------------------------------------- |
| `input`                             | 外部 JSON 文件路径（必需参数）                                        |
| `--output-format <格式>`            | 输出格式（csv/markdown）                                             |
| `--output-file <路径>`              | 输出文件路径                                                         |
| `--cluster-limit <数量>`            | 聚类限制（每批最多处理的告警数，默认50）                              |
| `--enable-verification/--no-verification` | 是否启用二次验证（默认启用）                                    |
| `--force-save-memory/--no-force-save-memory` | 是否强制使用记忆（默认关闭）                                   |
...
# 禁用二次验证（快速模式）
jsec analyze issues.json --no-verification

# 调整聚类批次大小
jsec analyze issues.json --cluster-limit 100
```
...
- **CSV 报告**（当配置为 csv 格式时）：结构化的 CSV 格式报告，便于导入 Excel 等工具分析
- **控制台输出**：分析过程的实时输出和最终报告摘要

### 注意事项

- 首次使用需要先运行 `jsec config` 配置扫描参数
- 首次运行可能需要较长时间，特别是对于大型代码库
- 建议在 CI/CD 流程中集成，定期进行安全扫描
- 对于误报，可以通过复核流程进行过滤
- 支持 C/C++ 和 Rust 语言的安全分析
- 如果 Agent 分析过程出错或无输出，会自动回退到直扫基线（fast 模式）
- 使用 `analyze` 命令分析外部工具输出时，如果格式不符合标准，系统会自动创建 Agent 学习格式并转换

## 4.15 C→Rust 迁移套件 (`jarvis-c2rust` / `jc2r`)
...
验证转译后的 Rust 代码与原 C 代码的功能对齐性，并支持迭代优化。需先完成 `run` 的 transpile 与 optimize 阶段。

**参数**：

| 参数                          | 描述                                     |
| ----------------------------- | ---------------------------------------- |
| `-g/--llm-group <组名>`       | 用于 LLM 分析和优化的模型组              |
| `-i/--max-iterations <次数>`  | 最大迭代次数（默认 10）                  |
| `--interactive`               | 启用交互模式（默认非交互模式）           |
| `--quick-mode`                | 快速模式，关闭方法论、规则加载等         |
...
# 指定模型组与最大迭代次数
jc2r verify -g deep_seek_v3 -i 5
```
...
Jarvis 提供了 tmux 任务派发功能，可将任务派发到新的 tmux 窗格中执行。**需在配置中设置 `enable_tmux: true` 才能使用**。适用于需要将任务分离到独立窗格或监控长时间运行任务的场景。

### 4.16.1 核心功能

Tmux 任务派发系统提供了以下核心功能：

1. **任务派发**：将任务派发到新的 tmux 窗格中执行，主会话可继续其他操作
...
jca --worktree -T "重构用户认证模块，使用新的加密算法"
```
...
4. **资源管理**：派发任务会消耗额外系统资源，请根据系统能力合理安排

5. **会话管理**：派发的任务会在独立的窗格中运行，可以使用 tmux 命令 (如 `tmux attach`) 管理会话

## 4.17 配置管理工具 (`jarvis-config` / `jcfg`)
