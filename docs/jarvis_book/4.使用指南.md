...
2. 对于腾讯元宝，您需要：
   ...
   - 找到任意一个发往 `yuanbao.tencent.com` 的请求，在请求头(Request Headers)中找到 `Cookie` 字段，并复制其完整的值
   - 将 Cookie 字符串填入 `yuanbao_cookies` 字段（如果使用新配置方式）或 `ENV.YUANBAO_COOKIES`（如果使用旧配置方式）
...
- 多实例：不同端口的实例互不影响；每个实例会在 ~/.jarvis 下维护独立的 PID 文件（jarvis*web*{port}.pid）。
...
| `--review-max-iterations <次数>` | 代码审查最大迭代次数，达到上限后停止审查（默认0，表示使用配置文件中的值，通常为3次；0表示不限制）。 |
| `-n`, `--non-interactive`        | 启用非交互模式，用于自动化脚本。详见"通用交互技巧"章节。                                            |
...
```bash
# 添加网页搜索和文件分析工具
jca --append-tools search_web,file_analyzer -r "分析项目依赖并搜索最佳实践"
```
...
与模型配置类似，RAG 也支持通过 `rag_groups` 和 `rag_group` 来定义和切换不同的 RAG 配置组合。这对于在不同类型的知识库（例如，代码库 vs. 文档库）之间切换，或者测试不同嵌入/重排模型的效果非常有用。
...
## 4.18 快速配置工具 (`jarvis-quick-config` / `jqc`)
...
2. **自动分支管理**：自动生成和清理 worktree 分支
3. **软链接支持**：自动将原仓库的 `.jarvis` 目录软链接到 worktree 中，确保配置和数据的一致性
4. **智能合并**：使用 rebase 策略将 worktree 分支合并回原分支，保持线性历史
...
1. **创建 worktree**：系统会自动生成一个唯一的分支名（格式：`jarvis-{project_name}-{timestamp}-{random_suffix}`），并在该分支上创建 git worktree
2. **链接配置**：将原仓库的 `.jarvis` 目录软链接到 worktree 中
3. **执行任务**：在 worktree 目录中执行代码修改任务
4. **合并回主分支**：任务完成后，使用 rebase 策略将 worktree 分支变基到原分支，然后通过 fast-forward 合并
5. **清理资源**：自动删除 worktree 目录和对应的分支
...
在 tmux 环境中，`execute_batch_tasks` 功能会被启用，允许在多个窗格中并行执行任务。

### 4.19.6 注意事项

1. **tmux 安装**：确保系统已安装 tmux
...
2. **自动启动**：当不在 tmux 环境中但使用 `--dispatch` 参数时，系统会自动创建新的 tmux 会话并执行任务

3. **资源管理**：并行执行多个任务会消耗额外系统资源，请根据系统能力合理安排

4. **会话管理**：派发的任务会在独立的窗格中运行，可以使用 tmux 命令 (如 `tmux attach`) 管理会话

## 4.20 配置管理工具 (`jarvis-config` / `jcfg`)
