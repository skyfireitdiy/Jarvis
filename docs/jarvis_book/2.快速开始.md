...
1. 检测并自动安装 `uv` 工具。
2. 优先获取最新 tag 对应的 Jarvis 源码，并使用浅克隆减少下载体积。
3. 使用 `uv tool install -e .` 安装 Jarvis。
...
PyPI、`uv` 直接从 Git 安装、Docker 等方式仍可用于特定场景，但主安装入口已统一为“源码下载 + `uv tool` 安装”。如需容器方式，请参考相关部署文档。
