...
首次运行 Jarvis 时，系统还会就以下可选功能询问您是否开启（默认均为关闭），您可以按需开启或保持关闭：

- 检测 Git 仓库并建议切换到代码开发模式（jca）：当当前目录是一个 Git 仓库时，启动 `jvs` 会提示是否切换到 `jca`（jarvis-code-agent）以获得更强的代码开发体验。
- 启动前列出内置配置供选择：在进入默认通用代理前，列出内置的 agent/multi_agent/roles 配置清单，便于直接选择对应场景启动（默认启用，可通过 `jvs -S` 或 `jvs --skip-config-selector` 跳过）。
