...
1. 检测并自动安装 `uv` 工具（Python 包管理器）。
2. 从 Gitee 镜像（国内优先）或 GitHub 克隆最新的 Jarvis 项目代码到 `~/Jarvis` 目录。
3. 使用 `uv tool install -e .` 安装项目及其依赖。
4. 配置相关的环境变量和路径。

> **提示**：脚本会优先尝试 Gitee 镜像，国内访问速度更快。如果克隆失败会自动回退到 GitHub。
...
> **注意**: PyPI 版本可能不是最新的，推荐使用源码安装获取最新功能。
