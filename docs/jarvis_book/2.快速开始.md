...
2. 自动安装 `uv`（如系统未安装）。
3. 使用 `uv tool install -e . --python 3.12` 安装 Jarvis。
4. 可选安装 `rg`、`fd`、`fzf`、`tmux`、`tree` 等提升体验的工具。

> **提示**：脚本会优先尝试 GitHub 上的最新 tag；如果无法获取，会回退到 Gitee 或默认分支。
>
> 脚本会自动安装 `uv`，无需手动准备。
...
我们提供了便捷的启动命令 `jarvis-service run`，它会同时启动 Web 网关和前端服务：
...
1. 启动 Web 网关服务（默认端口 8000）
2. 安装前端依赖（首次运行时会自动执行 `npm install`）
3. 构建前端发布产物（`npm run build`）
4. 启动前端发布服务（默认端口 5173）

> 如需以开发模式启动前端（热加载、跳过构建），可使用 `jarvis-service run --dev`。
