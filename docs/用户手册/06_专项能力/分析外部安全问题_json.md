...
1. 准备一份安全问题 JSON 文件。
2. 在终端中输入类似命令：`jsec analyze 你的问题.json`
3. 根据需要补充输出格式、输出文件、聚类限制、二次验证开关和强制记忆开关，例如：`jsec analyze 你的问题.json --output-format markdown --output-file report.md --cluster-limit 30 --enable-verification`
