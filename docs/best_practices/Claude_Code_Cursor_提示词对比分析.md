...
| **项目级文档** | CLAUDE.md | AGENTS.md / instructions.md | ✅ 已有（`.jarvis/rule` 项目规则文件） |
...
#### ✅ 完善的Token管理机制
- **上下文压缩**：多层次渐进式压缩策略，自动触发
  - Token阈值触发：剩余token < 25%时自动压缩
  - 滑动窗口压缩：保留最近的对话历史
  - 摘要压缩：使用LLM生成对话摘要
- **文件读取限制**：
  - 文件大小限制：10MB
  - Token限制检查：`read_code` 工具自动检查token限制
  - 行号范围支持：支持 `start_line` 和 `end_line` 参数
  - 自动截断：超过限制时自动截断并提示用户
- **工具输出截断**：根据剩余token数量动态截断工具输出
- **Git Diff截断**：diff内容最多占用输入窗口的10%

### 4.2 可以借鉴的改进点

#### ✅ 项目级说明文档（已实现）
- **现状**：已有 `.jarvis/rule` 项目规则文件，自动加载到系统提示词
- **功能**：完全等同于 Claude Code 的 `CLAUDE.md` 和 Cursor 的 `AGENTS.md`
- **建议**：
  - 在文档中明确说明 `.jarvis/rule` 可以作为项目级说明文档使用
  - 提供最佳实践指南，指导用户如何编写项目级说明文档
  - 可以包含：项目概述、技术栈、架构设计、编码规范、目录结构、特殊约定等
...
#### 🔄 上下文引用机制（@file / @folder）- 需考虑token限制
- **现状**：用户需要手动指定文件路径，通过 `read_code` 工具读取
- **设计考虑**：大文件会占用太多上下文token，需要智能处理
- **建议**：
  - 支持 `@filename` 或 `@folder` 语法，自动将文件/目录内容注入到上下文中
  - **必须考虑文件大小和token限制**：
    - 支持行号范围引用：`@file.py:100-200`（只引用指定行范围）
    - 支持智能摘要模式：`@file.py:summary`（只引用文件摘要，不包含完整内容）
    - 自动检测文件大小，超过阈值时提示用户指定行号范围
    - 借鉴 `read_code` 工具的token限制机制（自动截断并提示）
  - 支持目录引用时，只引用文件列表和摘要，不包含完整内容
- **优势**：更直观、更便捷，同时避免大文件占用过多token
- **参考**：Jarvis 的 `read_code` 工具已有完善的token限制和行号范围支持
...
#### 1. 项目级说明文档最佳实践（已实现，需完善文档）
**现状**：
- ✅ 已有 `.jarvis/rule` 项目规则文件，自动加载到系统提示词
- ✅ 支持 Jinja2 模板变量，可动态渲染
- ✅ 优先级：项目规则 > 全局规则 > 内置规则
...
## 目录结构
- `{{ git_root_dir }}/src/`: 源代码
- `{{ git_root_dir }}/tests/`: 测试代码
- `{{ git_root_dir }}/docs/`: 文档
...
## 开发规范
- 所有 API 必须包含 Swagger 文档
- 错误处理使用统一的异常类
- 日志使用结构化日志格式
```

**注意**：`.jarvis/rule` 文件会自动加载，无需额外配置。支持 Jinja2 模板变量，可以使用 `{{ git_root_dir }}`、`{{ current_dir }}` 等变量。
...
#### 4. 上下文引用机制（@file / @folder）- 智能token管理
**实现方案**：
- 解析用户输入中的 `@filename` 或 `@folder` 标记
- **智能处理大文件**：
  - 检查文件大小，超过阈值（如10MB）时拒绝读取
  - 检查token限制，超过剩余token的80%时自动截断或提示
  - 支持行号范围：`@file.py:100-200`（只引用指定行范围）
  - 支持摘要模式：`@file.py:summary`（只引用文件摘要）
- 注入到用户输入的上下文中
...
系统处理：读取 user_service.py 完整内容，注入到上下文中

# 行号范围引用（大文件）
用户输入：查看 @user_service.py:100-150 中的登录函数
系统处理：只读取第100-150行，注入到上下文中

# 摘要模式（超大文件）
用户输入：了解 @large_file.py:summary 的结构
系统处理：生成文件摘要（函数列表、类结构等），注入到上下文中

# 目录引用（只引用文件列表）
用户输入：查看 @src/ 目录下的所有Python文件
系统处理：列出文件列表和简要描述，不包含完整内容
```

**Token限制策略**（借鉴 `read_code` 工具）：
- 文件大小限制：10MB
- Token限制：不超过剩余token的80%
- 自动截断：超过限制时自动截断并提示用户
- 行号范围：支持 `start_line` 和 `end_line` 参数

**参考实现**：
- `read_code` 工具已有完善的token限制和行号范围支持
- `is_context_overflow()` 函数可用于检查token限制
- `truncate_git_diff_with_context_limit()` 展示了如何限制内容长度
...
### 优先级 1（立即实施）
1. ✅ **项目级说明文档最佳实践**：完善文档说明，指导用户更好地使用 `.jarvis/rule` 作为项目级说明文档
...
### 可以借鉴的改进
- ✅ 项目级说明文档（已实现：`.jarvis/rule`，需完善文档说明）
- 🔄 Scoped Rules（作用域规则）
- 🔄 上下文引用机制（@file / @folder）
- 🔄 确认理解阶段增强
...
1. **上下文理解能力**：✅ 项目级文档（`.jarvis/rule` 已实现）+ 🔄 智能上下文引用（需考虑token限制）
2. **规则系统灵活性**：🔄 Scoped Rules + 更细粒度约束
3. **用户体验**：🔄 确认机制 + Prompt 模板库
4. **可维护性**：✅ 标准化文档（已实现）+ 🔄 模板复用

**重要设计原则**：
- **Token管理优先**：上下文引用机制必须考虑token限制，避免大文件占用过多上下文
- **智能截断**：借鉴 `read_code` 工具的token限制机制，自动截断并提示用户
- **灵活引用**：支持行号范围、摘要模式等多种引用方式，适应不同场景

**重要发现**：
- Jarvis 的项目规则文件（`.jarvis/rule`）已经实现了 Claude Code 的 `CLAUDE.md` 和 Cursor 的 `AGENTS.md` 的功能
- 项目规则文件支持 Jinja2 模板变量，功能更强大
- 需要完善文档说明，让用户更好地理解和使用这个功能
