...
| **L1 剥离破坏服务器端前缀缓存命中**  | 成本/延迟上升 | 见 §6.3；剥离点尽量靠后、低频化；L0 优先；阶段 C 实测命中率 |

### 6.3 对服务器端 prompt caching 的影响（评审补充）

> 本节为方案评审时新增的分析，回答"L1 剥离旧工具结果是否影响模型服务器端缓存命中"。

**结论**：L1 剥离**会**破坏服务器端前缀缓存命中，但影响是局部、可预期的；在 Jarvis 所连的 OpenAI 兼容后端上，其"性价比"低于 Claude Code 原生环境，需在落地时权衡。

#### 6.3.1 服务器端缓存机制（事实）

Jarvis 实际连接的是 OpenAI 兼容 API（vLLM / DeepSeek / ModelScope 等第三方托管），其 prompt caching 为**纯自动前缀匹配**，无显式断点：

- **vLLM Automatic Prefix Caching**：按 token block（默认 16 tokens）哈希缓存 KV；新请求从第一个 block 起逐 token 匹配，命中"最长公共前缀"；前缀一旦在某处不一致，该处之后全部失效。
- **DeepSeek 上下文硬盘缓存**：默认开启；按"缓存前缀单元"完整匹配（请求结束位置落盘 + 公共前缀检测 + 固定 token 间隔落盘）；`usage` 返回 `prompt_cache_hit_tokens` / `prompt_cache_miss_tokens` 可观测命中。

**理想状态（无压缩）**：Jarvis 每轮请求只追加新消息（`self.messages.append`），system 在首位稳定，故第 N+1 轮请求是第 N 轮的**前缀超集** → 服务器端几乎完全命中，只需 prefill 新增消息。这是前缀缓存最理想的工作模式。

#### 6.3.2 L1 剥离如何破坏命中（推理）

L1 从历史**最旧**处剥离 `[assistant(tool_calls), role=tool]` 配对。设历史为 `[S, U1, A1(tc), T1, A2, U2, A3(tc), T3, A4, U3, ...]`，剥离最旧的 `[A1(tc), T1]` 后变为 `[S, U1, A2, U2, A3(tc), T3, A4, U3, ...]`。服务器端缓存的前缀是 `[S, U1, A1, T1, A2, ...]`，新请求在 U1 之后即分叉 → **公共前缀只到 `[S, U1]`，剥离点之后的历史 KV 全部失效**，需重新 prefill。剥离越靠前、剥离越多，失效范围越大。

#### 6.3.3 与 Claude Code 环境的本质差异（为何不能照搬）

Claude Code 连 Anthropic，缓存是**显式 `cache_control` 断点 + 20 block 回溯窗口**，可多断点分层把高价值静态前缀（system / tools / CLAUDE.md）"钉住"。microcompact 剥离发生在断点**之后**的动态对话区，**不触碰静态断点缓存**——只牺牲变化最快、命中价值最低的对话层，保住 token 大头（静态前缀）。Claude Code 把缓存命中率列为 SEV 级监控指标，其剥离是"牺牲低价值缓存、保高价值缓存"的理性取舍。

Jarvis 所连后端**无显式断点**，命中=整个请求最长公共前缀，无法分层保护任何一层；L1 剥离是"一刀切"伤剥离点后全部历史。故同样的剥离动作，在 Claude Code 上"只伤低价值层"，在 Jarvis 上"伤剥离点后整段历史"，**性价比显著更低**。

#### 6.3.4 对方案的含义（观点）

1. **L1 剥离在 Jarvis 环境性价比低于 Claude Code**，是方案评审的减分项，但非否决项。
2. **L1 并非不可用**：它省的是客户端 token + prefill，代价是破坏部分缓存。是否划算取决于剥离频率、剥离点位置、后端缓存计费（DeepSeek 命中约 0.1×）。
3. **若保留 L1，应让剥离尽量发生在对话尾部、缩小失效范围**，或提高触发阈值、低频化，避免每轮小步剥离导致缓存长期低命中。
4. **L0 落盘是缓存友好的**（回填时截断，不改变已发送历史），应优先于 L1。
5. **阶段 C 需实测**：对比"开 L1 / 关 L1"的 `prompt_cache_hit_tokens` 命中率与成本，作为 L1 是否保留的决策依据。
