...
package "知识库 (Knowledge Base)" {
  [本地方法论] as LocalMethodologies
  [中心方法论库 (Git)] as CentralRepo
}
...
MethodologyEngine --> LocalMethodologies : "读/写"
LocalMethodologies <--> CentralRepo : "同步/分享"
...
### 3.3. 方法论引擎：知识的沉淀、复用与共享

Jarvis 不仅仅是一个任务执行器，它还是一个能够通过“实战”积累经验并不断进化的系统。其核心的学习机制就是**方法论引擎**，它通过一种简单而强大的“完全情境学习（Full-Context Learning）”模式，结合 Git 实现知识的沉淀、复用与团队共享。
...
- **工作流程**:
    1.  **知识注入 (任务开始时)**:
        -   当一个新任务启动时 (`load_methodology` 函数)，方法论引擎会整合并加载来自多个源的方法论：
            - **本地方法论**: `~/.jarvis/methodologies/` 目录下的个人方法论。
            - **项目方法论**: 当前项目 `.jarvis/methodologies/` 目录下的方法论。
            - **额外目录**: 配置文件中 `JARVIS_METHODOLOGY_DIRS` 指定的任意目录。
            - **中心方法论库**: 如果在 `config.yaml` 中配置了 `JARVIS_CENTRAL_METHODOLOGY_REPO`，系统会**自动克隆或拉取**该 Git 仓库中的所有方法论，并每日检查更新。
        -   **智能筛选**: 为了优化上下文效率，引擎首先向 LLM 提供所有方法论的**标题列表**，让其根据当前任务需求，选择出相关性最高的几个。
        -   **精确注入**: 只有被选中的方法论的**完整内容**，连同所有可用工具的描述和用户任务，才会被打包成最终的提示（Prompt）。
        -   这个提示明确地指示大语言模型：“**请参考以下我为你筛选出的历史经验（方法论），为当前的新任务规划详细的执行步骤。**”
        -   这种“先筛选、后注入”的策略，取代了原有的“填鸭式”方法，确保了上下文的相关性和高效性，显著降低了 Token 消耗。
    2.  **知识生成 (任务结束时)**:
    ...
    3.  **知识共享 (手动触发)**:
        -   用户可以通过 `jarvis --share-methodology` 命令，将本地积累的、尚未在中心库中存在的方法论进行筛选，并通过 `git commit` 和 `git push` 分享到中心方法论仓库。这极大地促进了团队间的知识传递和最佳实践同步。

- **优势**:
    - **越用越聪明**: 每次成功的任务复盘，都可能为知识库增加一条新的方法论，使得 Jarvis 在处理未来相似任务时，规划更精准、效率更高。
    - **团队赋能**: 通过中心方法论库，团队的最佳实践得以沉淀和共享，新成员可以快速学习和应用团队的经验，老成员的知识也能被系统性地保留和传承。
