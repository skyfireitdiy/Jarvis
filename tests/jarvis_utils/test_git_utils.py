...
class TestDetectLargeCodeDeletion:
    ...
    def test_no_large_deletion(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout=" 5 files changed, 10 insertions(+), 5 deletions(-)"
        )
    ...
    def test_large_deletion_detected(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout=" 10 files changed, 50 insertions(+), 300 deletions(-)"
        )
    ...
    def test_custom_threshold(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout=" 5 files changed, 10 insertions(+), 150 deletions(-)"
        )
...
class TestIsFileInGitRepo:
    ...
    def test_file_in_repo(self, mock_run):
        ...
        with patch("jarvis.jarvis_utils.git_utils.os.path.abspath", return_value="/path/to/repo/file.py"):
