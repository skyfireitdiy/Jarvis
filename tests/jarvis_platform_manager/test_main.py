...
class TestValidatePlatformModel:
    ...
    @patch("jarvis.jarvis_platform_manager.main.PrettyOutput")
    def test_valid_platform_and_model(self, mock_pretty_output):
        """测试有效的平台和模型"""
        result = validate_platform_model("openai", "gpt-4")
        assert result is True

    @patch("jarvis.jarvis_platform_manager.main.PrettyOutput")
    def test_missing_platform(self, mock_pretty_output):
        """测试缺少平台"""
        result = validate_platform_model(None, "gpt-4")
        assert result is False
        mock_pretty_output.print.assert_called_once()

    @patch("jarvis.jarvis_platform_manager.main.PrettyOutput")
    def test_missing_model(self, mock_pretty_output):
        """测试缺少模型"""
    ...
    @patch("jarvis.jarvis_platform_manager.main.PrettyOutput")
    def test_both_missing(self, mock_pretty_output):
        """测试平台和模型都缺少"""
    ...
    @patch("jarvis.jarvis_platform_manager.main.PrettyOutput")
    def test_empty_string_platform(self, mock_pretty_output):
        """测试空字符串平台"""
        result = validate_platform_model("", "gpt-4")
        assert result is False

    @patch("jarvis.jarvis_platform_manager.main.PrettyOutput")
    def test_empty_string_model(self, mock_pretty_output):
        """测试空字符串模型"""
...
class TestLoadRoleConfig:
    ...
    def test_nonexistent_file(self, temp_dir):
        ...
        with patch("jarvis.jarvis_platform_manager.main.PrettyOutput") as mock_pretty:
            result = load_role_config(str(nonexistent))
            assert result == {}
            mock_pretty.print.assert_called_once()
