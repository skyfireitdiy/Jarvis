...
class TestJarvisStatsIntegration:
    ...
    def test_full_workflow(self, setup_integration):
        ...
        StatsManager.increment(
            "api_calls", tags={"endpoint": "/posts", "method": "POST"}
        )
        ...
        api_stats = StatsManager.get_stats(
            "api_calls", last_hours=1, tags={"endpoint": "/users"}
        )
        assert api_stats["count"] == 1

        # 3. 获取聚合数据
        aggregated = StatsManager.get_stats(
            "page_views", last_hours=1, aggregation="hourly"
        )
        # 聚合数据返回时间戳为key的字典结构
            ...
            try:
                ...
                for i in range(10):
                    ...
                    stats = StatsManager.get_stats(
                        f"concurrent_{worker_id}", last_hours=1
                    )
