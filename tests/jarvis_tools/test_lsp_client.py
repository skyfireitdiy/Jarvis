...
def test_lsp_client_tool_execute_missing_params(tool):
    ...
    assert "action" in result.get("stderr", "").lower() or "缺少" in result.get("stderr", "")
    
    result = tool.execute({"action": "get_symbol_info"})
    assert result["success"] is False
    assert "file_path" in result.get("stderr", "").lower() or "缺少" in result.get("stderr", "")
...
def test_lsp_client_tool_unsupported_language(tool, temp_project_dir):
    ...
    if not result.get("success"):
        assert "不支持" in result.get("stderr", "") or "not supported" in result.get("stderr", "").lower()
