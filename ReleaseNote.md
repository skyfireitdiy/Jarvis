### Release Note - v0.3.0 2025-07-27

#### **新功能 (Features)**
-   **全新文档系统 (Jarvis Book)**: 新增了完整的 Jarvis Book 文档，提供系统化的项目介绍、快速入门、使用指南和开发手册，帮助用户更好地理解和使用 Jarvis。

#### **优化与重构 (Refactors & Improvements)**
-   **CLI框架升级**: 将项目所有命令的底层框架从 `argparse` 全面迁移至 `typer`，提供了更现代化、可维护性更强的命令行交互体验。
-   **HTTP库切换**: 将底层HTTP请求库从 `httpx` 切换为 `requests`，统一并简化了项目的网络请求依赖。
-   **组件移除**: 为了精简项目，移除了已废弃的 `jarvis-git-details` (`jgd`) 工具和不再维护的 `oyi` 平台。
-   **依赖清理**: 从项目依赖中彻底移除了 `httpx`。

#### **文档更新 (Documentation)**
-   **README重构**: 大幅简化了 `README.md` 的内容，使其更专注于快速入门和核心功能展示，并将详细信息引导至新的 Jarvis Book。
-   **技术文档同步**: 更新了 `technical_documentation.md` 中的命令列表，移除了已删除的命令。

本次更新是一次重大升级，我们重构了命令行界面，引入了全新的系统化文档，并对核心依赖和平台支持进行了优化和精简，旨在为用户提供更稳定、更易用、文档更完善的 AI 助手体验。
