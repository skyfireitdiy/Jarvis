...
#### **新功能 (Features)**
- **模型组 (Model Groups)**: 新增了模型组配置功能，允许用户在 `config.yaml` 中预定义多套模型组合（包括常规模型、思考模型及Token限制等），并通过 `JARVIS_MODEL_GROUP` 或命令行参数 `--model_group` 快速切换，极大地提升了在不同平台或任务间切换的灵活性。
...
- **配置加载逻辑重构**: 重构了底层的配置加载系统，以完全支持模型组的优先级覆盖规则（独立配置 > 模型组配置 > 默认值）。
...
#### **文档更新 (Documentation)**
- **README更新**: 详细说明了新的模型组配置方法，并更新了配置项说明表格。
- **产品报告更新**: 更新了产品报告中的多个应用案例，使其更具深度和技术代表性（例如，从Web开发转向嵌入式开发，从常规错误修复转向Coredump分析）。
- **技术文档更新**: 在技术文档中增加了模型组设计的详细说明、工作机制和配置示例，并更新了所有相关的配置项描述。
- **工具描述增强**: 为 `generate_new_tool` 工具增加了提示，建议在工具代码中加入打印信息以方便追踪。

#### **其他 (Miscellaneous)**
- **类型提示与健壮性**: 为多个模块（如MCP客户端、git工具类）增加了更精确的类型提示和空值检查，提升了代码的健壮性。

本次更新的核心是引入了强大的**模型组**功能，为专业用户提供了前所未有的灵活性和便利性。同时，通过新增的**安装脚本**和**文档优化**，我们显著改善了新用户的入门体验。内部的重构和优化也为系统未来的扩展奠定了更坚实的基础。
