...
- **🎯 规则加载工具**
  - **动态规则加载**：为 code_agent 添加 load_rule 工具，支持在运行时动态加载指定的规则文件。工具使用 jinja2 模板引擎渲染规则内容，支持内置变量（current_dir、git_root_dir、jarvis_src_dir 等），提升规则使用的灵活性和复用性
  - **工具使用指南**：在 prompt_manager 中添加 load_rule 工具的详细使用指南，包括工具说明、参数解释、变量列表和使用示例，帮助开发者快速上手

- **🎯 技能开发规范**
  - **技能规范文档**：新增 `builtin/rules/development_tools/add_skills.md`，定义技能开发的完整规范，包括技能结构、注册流程、参数定义等。提供技能模板和最佳实践，指导开发者正确开发和注册新技能

#### **📌 修复 (Fixes)**

- **任务分析器逻辑修复**
  - **非交互模式限制**：修复 task_analyzer 逻辑，确保仅在非交互模式下执行任务分析，避免在交互模式下产生不必要的干扰

- **安装脚本修复**
  - **语法错误修复**：修复安装脚本中的语法错误和变量引用问题，确保安装脚本在各种 Shell 环境下都能正确执行

#### **🔧 优化与重构 (Refactors & Improvements)**

- **规则目录结构优化**
  - **测试规则调整**：重构测试规则目录结构，将各语言的测试规则（如 go_test.md、rust_test.md 等）组织到更清晰的位置，提升规则管理的可维护性
