### Release Note - v0.7.3 (2025-11-28)

#### **🚀 新功能 (Features)**

- **代码代理能力增强**
  - 新增自动格式化修改文件功能，代码修改后自动执行格式化
  - 支持通过配置初始化智能模型平台，提升代码生成质量
  - 增强代码审查逻辑，支持破坏性变更和文件结构检查
  - 为代码块输出添加行号支持，提升代码可读性
  - 支持一次调用同时修改多个文件，提升批量编辑效率
  - 增强工具调用解析的鲁棒性，支持大模型自动修复工具调用格式错误
- **事件总线与上下文管理优化**
  - 为事件总线增加优先级机制，支持事件处理的优先级控制
  - 支持通过 `<!!!SUMMARY!!!>` 标记触发历史总结与清空
  - 支持模型主动触发摘要与多平台场景化选择
  - 增加对话轮次计数与阈值配置支持，默认阈值从 30 调整为 50
  - 在总结提示中增加无效方案总结字段，提升上下文保留精度
- **Token 管理与 UI 增强**
  - 在对话界面实时显示 Token 使用百分比和进度条
  - 新增消息自动截断功能以避免超出 Token 限制
  - 基于剩余 Token 的上下文管理策略，智能管理对话历史
  - 为对话状态栏添加当前时间戳，提升用户体验
  - 支持因长度限制中断时自动续生成回复
- **jarvis-c2rust 转译工具链重大升级**
  - **测试代码保护机制**：实现细粒度测试代码删除检测与自动回退机制，通过订阅 BEFORE_TOOL_CALL 和 AFTER_TOOL_CALL 事件，在每次工具调用前后记录 commit 并检测是否误删 `#[test]` 或 `#[cfg(test)]` 标记，若检测到不合理删除则通过 LLM 判断并自动回退，覆盖 transpile、todo-fix、build-fix 及 review-fix 四个关键环节
  - **优化器重构**：重构优化器为 CodeAgent 驱动，所有优化步骤（Clippy 告警消除、unsafe 清理、可见性优化、文档补充）统一通过 CodeAgent 完成，提升智能分析与修复能力
  - **Clippy 告警处理增强**：改用 JSON 格式解析 Clippy 告警以提升准确性，支持按文件批量修复（每次修复前 10 个告警），新增 missing_safety_doc 告警检测与按文件迭代修复，完善 clippy 自动修复失败时的回退逻辑
  - **断点续跑增强**：支持断点续跑时自动 reset 到对应 commit 确保代码状态一致，支持按步骤保存优化进度（steps_completed），引入独立配置文件（config.json）并重构根符号与禁用库管理，实现流水线断点续跑与前置依赖检查机制
  - **代码生成与审查优化**：统一代码生成与修复逻辑为单一 Agent，启用方法论和分析功能，明确 TDD 流程（先写测试再写实现），优化函数审查流程以优先使用 commit 差异信息，增强代码审查逻辑以支持破坏性变更和文件结构检查
  - **转译流程优化**：每轮转译循环前自动执行 `cargo fmt`，支持跳过资源释放类函数的实现（通过 RAII 机制自动管理资源），强化 Rust 生成要求并强制包含测试用例，增强 LLM 输出解析容错性并支持 Markdown 代码块
  - **配置管理重构**：引入独立配置文件（config.json）并重构根符号与禁用库管理，新增 `jarvis-c2rust config` 命令用于管理配置，支持通过命令行指定禁用库列表，为根符号自动确保模块在 lib.rs 中导出
  - **CLI 架构简化**：移除 scan、lib-replace、prepare、transpile、optimize 等独立子命令，统一由 `run` 命令执行完整流水线，简化 transpile 命令参数并统一配置管理
  - **其他增强**：支持 CSV 格式报告输出，在日志中添加处理进度信息，为各 CodeAgent 实例添加名称标识，强制要求修复后必须执行验证命令，增强测试失败诊断与修复验证机制，为 cargo test 命令添加超时限制
- **代码分析与构建验证增强**
  - **上下文推荐优化**：对上下文推荐的符号和文件进行去重（基于 name + file_path + line_start），改进符号使用分析以区分定义与调用并支持定义位置追踪（优先通过 LSP 获取符号定义位置），将上下文推荐默认使用 cheap 平台以降低成本
  - **构建验证器增强**：为各语言构建验证器（CMake、Go、Java、Makefile、Node.js、Python、Rust）添加实时控制台输出，包括构建成功/失败的实时打印信息、耗时和截断的错误输出
  - **构建流程优化**：将 Go、Python 和 Rust 构建验证器升级为同时执行编译与测试（go test / pytest / cargo test），简化构建流程并移除 cargo check 阶段，统一使用 cargo test 验证（因其已包含编译检查）
  - **Rust 构建验证器增强**：为 Rust 构建验证器添加环境变量支持（RUST_BACKTRACE=1、RUSTFLAGS="-A warnings"）和错误处理，改用 subprocess.run 显式传递环境变量，增加对超时、命令未找到及其他异常情况的处理
- **LSP 客户端增强**
  - 支持按语言查找 LSP 工作目录：新增 `_find_lsp_work_dir` 函数，根据语言类型自动查找包含对应配置文件（如 Cargo.toml、package.json 等）的合适工作目录
  - 完善 LSP 通信协议：严格遵循 LSP 协议的 Content-Length 格式，增强对进程异常退出、初始化失败和请求超时的处理能力，优化错误信息输出
  - 修复 LSP 客户端资源泄露问题：在主流程结束后增加统一清理，防止文件句柄和子进程残留，优化关闭逻辑支持优雅/强制终止，引入最大缓存限制避免无限增长
- **代码编辑工具增强**
  - 引入编辑错误类型枚举（EditErrorType），明确区分各类编辑错误（块ID不存在、缓存无效、多处匹配等），提升错误诊断能力
  - 增强编辑失败时的错误提示和上下文展示：当 edit 操作匹配到多处文本时，显示最多 5 个匹配位置的上下文（前后各 2 行），根据不同的错误类型提供针对性的修复建议
  - 限制 edit 操作仅在唯一匹配时执行，提升操作安全性
  - 确保编辑后文件内容末尾保留换行符，修复从缓存恢复文件时块之间缺少换行符的问题
  - 简化块间换行符处理逻辑并支持文件末尾换行符标志
  - 在编辑失败时提供 read_code 工具使用提示，建议在编辑失败后使用 read_code 工具重新读取文件再尝试编辑
  - 为工具调用错误提示添加长响应检测，当响应内容超过 2048 字符时提示用户可能是上下文溢出导致解析失败
- **JSONNet 兼容性增强**
  - 支持使用 ``` 作为多行字符串标识
  - 支持保留多行字符串原始缩进并恢复
  - 支持自动修复 `|||` 多行字符串缩进问题
  - 增强对输入中反引号和代码块标记的处理与错误提示
  - 增强 markdown 代码块剥离逻辑以支持更多边界场景
- **配置与规则系统增强**
  - 支持为廉价和智能操作配置独立的平台与模型
  - 统一 LLM 平台选择策略以匹配任务复杂度
  - 支持配置附加说明并应用于 LLM 提示词
  - 支持 YAML 命名规则文件与 `--rule-name` 参数
  - 支持多个规则名称参数化
  - 将默认 AI 平台切换为 openai 并默认模型设为 gpt-5
- **其他功能增强**
  - **技术文档**：新增 Jarvis Agent 技术总结文档（15 项核心优化技术），新增 Jarvis C2Rust 与 SEC 核心技术总结文档（12 项核心技术），更新技术报告内容并重构模块分类
  - **Web 模式增强**：支持自定义 Web 模式下交互式终端的启动命令（`--web-launch-cmd` 参数），允许用户指定用于重启交互式终端的完整命令
  - **调试与可观测性**：为关键操作新增调试输出（终端生命周期、工具调用流程等），优化工具调用日志可读性并添加带 emoji 的调试日志
  - **代码审查优化**：优化函数审查流程以优先使用 commit 差异信息，增强代码审查要求并移除记忆相关工具，确保审查基于最新代码而非历史记忆
  - **工具调用增强**：支持大模型自动修复工具调用格式错误，增强工具调用解析的鲁棒性（支持从任意位置提取完整 JSON 对象、清理额外标记、自动补全缺失结束标签）
  - **代码读取优化**：优化大文件超出 token 限制时的部分读取逻辑，自动计算安全读取范围并返回部分内容，包含详细提示信息指导用户如何继续读取剩余内容

#### **📌 修复 (Fixes)**

- **代码读取与编辑修复**
  - 修复代码块显示相对行号而非真实行号的问题：在 `_get_blocks_from_cache` 返回的块数据中添加 start_line 字段，确保格式化输出时显示文件中的真实行号
  - 修复同一文件多个范围请求时块重复的问题：新增 `_handle_merged_ranges` 方法合并同一文件的多个范围请求并去重，按文件路径分组处理请求避免重叠范围导致的块重复输出
  - 修复从缓存恢复文件时块之间缺少换行符的问题：在拼接多个文本块时自动插入换行符，避免内容粘连
  - 修复多行字符串缩进处理逻辑：统一结束标记处理为 `\n|||`，记录所有行的原始缩进信息（包括 0 缩进），统一预处理为 1 空格基础缩进
  - 修复 jsonnet 多行字符串结束标记缩进问题：检测并移除结束标记前的空白，确保生成的字符串符合 jsonnet 语法规范
- **工具与平台修复**
  - 修复工具筛选逻辑确保内置工具始终保留：在 ToolRegistry 中添加 `_builtin_tool_names` 集合追踪内置工具，修改筛选逻辑仅对用户自定义工具进行筛选，筛选结果与内置工具合并
  - 修复 LSP 客户端资源泄露问题：在主流程结束后增加统一清理，防止文件句柄和子进程残留，优化关闭逻辑支持优雅/强制终止
  - 修复脚本执行及终端会话进程关闭逻辑：改进进程终止流程，确保在超时或异常情况下进程能被正确关闭，并关闭所有相关文件描述符
  - 修复 Kimi 平台图片处理逻辑和上传文件返回值缩进错误：修正因缩进不正确导致的 else 分支逻辑错位问题
- **jarvis-c2rust 修复**
  - 修复当无新文件时遗漏未完成优化步骤的问题：当所有文件都已处理但仍有未完成优化步骤时，强制使用全部 Rust 文件作为目标以确保步骤得以执行
  - 修复审查通过时未正确记录问题数量的问题：当代码审查通过时，改为始终记录实际发现的问题数量，确保审查结果和指标数据准确反映实际情况
  - 修复 clippy 自动修复失败时的回退逻辑：先检查工作区是否有实际变更，仅在有修改时才执行回退，避免不必要的 git 操作
  - 修复 cargo 测试调用超时和异常处理：通过 try-except 捕获 TimeoutExpired 和其他异常，统一视为测试失败并继续修复流程
  - 修复格式错误处理逻辑并增加重试提示：在 JSON 解析失败时明确设置解析错误信息并立即进入重试循环，在每次重试前输出日志提示
  - 修复保留现有根符号配置并去除 main 过滤：允许 jarvis_c2rust config 追加而非覆盖现有 root_symbols，同步 run 子命令移除自动排除 main 的行为
  - 修复过滤 "mod" 关键字避免错误模块处理：在多个关键位置增加对 "mod" 字符串的显式过滤，确保转译器仅处理合法模块名称
  - 修复修正 append_tools 参数类型错误：将 append_tools 从列表修改为字符串，以符合下游接口对参数类型的预期
- **Agent 与上下文修复**
  - 修复子 Agent 递归调用与代码规则加载提示：在子 Agent 工具中禁用 sub_agent/sub_code_agent，防止无限递归，代码 Agent 增加已加载规则的实时打印
  - 修复 token 使用量统计逻辑以包含历史对话：通过 `get_used_token_count()` 获取完整的历史 token 并加入当前响应 token，确保剩余 token 显示准确
  - 修复修正 token 计数比例以匹配 OpenAI 官方估算：将 token 计算结果统一放大 10/7 倍，使估算值更接近 OpenAI 官方统计的实际 token 消耗
  - 修复降低 token 阈值以解决超长消息处理失败：将拆分和重试时的阈值因子从 0.8 进一步降至 0.6，避免部分提交和长上下文处理时因 token 估算误差再次超出限制
- **其他修复**
  - 修复优化批次处理逻辑并补全无风险项目记录：解决分析 Agent 返回空数组或全为无风险时未持久化的问题，统一打印各阶段风险 gid 列表
  - 修复终端输出滚动逻辑异常问题：在 BasePlatform 的响应处理末尾补充对 text_content 的滚动截断逻辑，确保当内容超出可视区域时只保留最新若干行
  - 修复 Live 输出后多余的空行问题：移除了在 Live 上下文内调用 console.print()，避免重复输出
  - 修复记忆库整理确认默认值：将记忆库整理提示的默认确认值从 True 改为 False，避免在无人干预时自动执行可能影响数据的操作
  - 修复确保所有 Agent 在 crate 根目录下创建：统一在创建每个 Agent 前切换至 crate 根目录，并在创建后恢复原目录，以保证工具行为一致性

#### **🔧 优化与重构 (Refactors & Improvements)**

- **输出系统重构**
  - 移除 PrettyOutput 依赖，统一使用标准 print 输出
  - 统一使用 print 替代 PrettyOutput 输出日志和带图标提示
  - 移除 PrettyOutput 依赖并简化输出逻辑
  - 重构聊天输出逻辑并新增 Token 用量实时展示
  - 优化输出格式并清理冗余代码
- **jarvis-c2rust 模块重构**
  - **模块拆分**：拆分 transpiler 模块并提取常量与工具函数，新增 constants.py（配置与路径常量）、models.py（FnRecord 数据类）、loaders.py（符号数据库与映射管理）、utils.py（通用工具函数）
  - **CLI 架构重构**：重构 CLI 架构并优化项目根目录推断逻辑，移除 scan、lib-replace、prepare、transpile、optimize 等独立子命令，统一由 `run` 命令执行完整流水线，简化 transpile 命令参数并统一配置管理
  - **优化流程重构**：重构优化流程为独立步骤函数（`_run_clippy_elimination_step()` 与 `_run_optimization_step()`），将按告警单次修复改为按文件批量修复（每次修复前 10 个告警），移除结构优化功能并强化迭代修复与回退机制
  - **代码生成与审查优化**：统一代码生成与修复逻辑为单一 Agent，启用方法论和分析功能，简化代码生成提示并明确 TDD 流程，优化代码审查迭代逻辑，调整破坏性变更检测逻辑并简化审查代理创建流程
  - **构建流程优化**：简化构建流程，移除 cargo check 阶段，统一使用 cargo test 验证，优化格式错误处理逻辑，调整 cargo fmt 执行时机（从每轮循环开始前移至每个函数转译前）
  - **其他优化**：优化库替换评估进度显示逻辑（区分初始根函数与递归子节点的进度），避免重复处理相同符号并优化 cargo test 调用，简化编译参数提取逻辑并返回原始字符串，提取编译参数解析与修复提示构建逻辑，关闭所有 Agent 的 force_save_memory 选项
- **代码读取与编辑优化**
  - 简化文件分割逻辑，用分割点集合替代范围合并
  - 将过大的语法单元按 20 行分割
  - 优化大文件超出 token 限制时的部分读取逻辑
  - 重构 JSONNet 兼容层中的代码块剥离逻辑
- **上下文推荐优化**
  - 重构上下文推荐以提升性能和准确性
  - 优化上下文推荐逻辑，降低推荐成本
- **Agent 系统优化**
  - **摘要生成优化**：区分 token 压缩与任务完成两种摘要生成场景，为 `generate_summary` 添加可选参数 `for_token_limit`，在 token 不足时统一使用 SUMMARY_REQUEST_PROMPT 确保压缩效果
  - **事件总线优化**：将事件总线流程改为关键流程直接调用，非关键流程才广播事件，关键业务（memory_manager、task_analyzer）改为直接调用对应实例方法，确保主链路稳定性
  - **对话总结优化**：重写对话总结提示词以提升上下文保留精度，将摘要请求的目标从"简要"调整为"无关键信息缺失"，明确列出必须保留的六大类关键信息（目标、事实、决策、技术细节、进展、偏好）
  - **上下文管理优化**：移除基于轮次的自动总结配置，改为基于 token 阈值（当前模型输入窗口 80% token 使用量），优化 is_context_overflow 的 token 计算性能（通过引入基于字符长度的快速预估，减少约 50% 以上的精确 token 计算调用）
  - **其他优化**：移除冗余提示并增加调试日志，移除冗余的 hasattr 安全检查，为 Agent 子类添加类型标识（`_agent_type` 字段），在总结提示中增加无效方案总结字段
- **构建验证优化**
  - 统一 CodeAgent 并简化构建验证流程
  - 将构建验证默认超时时间从 600 秒缩短为 30 秒，后调整为 600 秒
  - 将代码执行超时时间增加至 600 秒
- **平台与工具优化**
  - **UI 性能优化**：优化 subtitle 更新频率和滚动逻辑（通过计数器和时间间隔双重策略控制更新），增大内容更新最小间隔从 0.5 秒调整为 1 秒以减少界面闪烁，增大最小缓冲区大小从 5 调整为 20 以减少频繁刷新
  - **长上下文处理优化**：优化长上下文处理，支持递归拆分与动态阈值调整，新增 `_submit_part_with_split` 方法实现超长文本递归二分拆分与阈值因子动态降低，提取长上下文处理逻辑并增强重试机制（首次失败后自动降低 20% 阈值再次拆分）
  - **平台清理**：重构 jarvis_platform 输出处理，移除废弃的 AI8 平台实现，统一使用标准 print 输出日志
- **其他优化**
  - **重试机制优化**：移除冗余 sleep_time 参数并统一重试逻辑（采用指数退避策略替代固定等待时间），将重试次数从 5 次增加到 6 次（第 6 次等待时间对应更新为 32 秒）
  - **工具调用优化**：优化工具调用检测提示文案（将"有工具调用"改为"检测到工具调用"），仅在真正检测到工具调用时才输出日志，仅在 long_hint 为空时尝试使用大模型修复工具调用格式
  - **代码质量优化**：优化代码格式和注释，统一多行打印格式并清理冗余代码，优化函数跳过实现的评估逻辑与提示信息，过滤 "mod" 关键字避免错误模块处理
  - **工具清理**：优化代码审查要求并移除记忆相关工具（retrieve_memory、save_memory、clear_memory），移除 ask_user 工具及相关引用，移除未启用的工具列表

#### **📚 文档更新 (Documentation)**

- **技术总结文档**
  - 新增 Jarvis Agent 技术总结文档：完整梳理并记录 Jarvis Agent 的 15 项核心优化技术，涵盖事件驱动架构、记忆管理、工具筛选、上下文管理、任务分析、子 Agent、LSP/Tree-sitter 支持、构建验证、静态检查及大代码删除门禁等关键能力
  - 新增 Jarvis C2Rust 与 SEC 核心技术总结文档：梳理并归档 C2Rust 转译与 SEC 安全分析的 12 项核心技术，涵盖 AST 解析、依赖拓扑、LLM 模块规划、安全检查器、断点续跑等关键机制
- **技术报告更新**
  - 更新技术报告内容，重构模块分类并更新组件图与职责说明（详细描述通用 Agent 的模块划分，新增核心、管理器、工具执行、输入/输出处理、Web 支持、分享管理、辅助、平台层及入口等十大类别）
  - 更新章节编号以保持结构一致性（如 AgentRunLoop 从 3.2 改为 3.5）
  - 重构 jarvis-sec 与 jarvis-c2rust 的 Agent 交互时序图说明，优化文档结构使两个模块的 Agent 流程描述更加一致、准确
- **代码文档优化**
  - 补充代码代理调试指引：说明如何通过日志、打印和临时文件快速定位问题
  - 补充各类优化场景下修复已有问题的指导说明：在多个优化任务的说明中统一增加"修复已有实现的问题"部分，明确要求在执行优化的同时若发现逻辑错误、潜在 bug、内存安全等问题应一并修复
  - 在破坏性变更检测指南中补充测试用例删除检查，并给出例外场景说明
  - 更新代码工程师系统提示词：将简洁版提示词替换为详细流程规范，明确工具使用顺序、代码质量要求及调试指引
  - 明确 Clippy 告警处理规范：强调只有在确认误报时才允许使用 `#[allow(...)]`，并要求提供明确理由
- **代码注释中文化**
  - 要求所有代码注释必须使用中文（包括文档注释、行内注释、块注释等）
  - 将 BasePlatform 类及方法注释翻译为中文
  - 将 Symbol 类和 SymbolTable 类的文档字符串翻译为中文
  - 翻译函数文档字符串为中文（input.py 模块等）
- **格式与兼容性**
  - 支持 ``` 作为多行字符串分隔符：在多个模块的提示信息和错误引导中，统一将多行字符串分隔符从仅支持 `|||` 扩展为同时支持 `|||` 或 ```
  - 更新使用指南和附录文档

#### **🔧 其他 (Miscellaneous)**

- **配置与功能调整**
  - 增加 PDF 连续流页面高度至 6000cm：为支持更长内容的连续打印，将 CSS 中 @page 的高度从 5000cm 提升至 6000cm
  - 添加自动记忆整理功能开关：新增 `JARVIS_ENABLE_MEMORY_ORGANIZER` 配置项，用于控制是否在任务完成后自动检测并提示用户整理记忆库，默认关闭
  - 统一移除 CLI 欢迎语：将所有模块 CLI 入口从 init_env 带欢迎语调用改为无参调用，消除冗余输出
  - 移除补丁格式配置项 JARVIS_PATCH_FORMAT：统一使用默认的 all 模式，简化配置流程
  - 命令行直接指定任务时跳过预处理和配置选择：避免在通过 `-T/--task` 直接传入任务时，仍然提示选择预定义任务、切换 jca 模式或选择内置配置
- **测试增强**
  - 添加多个测试用例，包括外部修改文件后缓存失效的集成测试（分别在 test_edit_file.py 和 test_read_code.py 中验证当文件被外部编辑器实际修改后，系统缓存能够正确失效并提示用户）
  - 添加多行字符串中反引号处理的测试用例：覆盖 `|||` 多行字符串中包含多个反引号、行首反引号、``` 代码块标记、嵌套多行字符串以及特殊字符组合等场景
  - 新增针对 jsonnet_compat 模块和 LSP 客户端的完整单元测试，覆盖多行字符串处理、markdown 代码块剥离、LSP 操作及多语言支持等场景
- **其他**
  - 优化代码格式和类型提示
  - 移除 ask_user 工具及相关引用，移除未启用的工具列表

本次更新是一次全面的功能增强和系统优化版本。核心亮点是**jarvis-c2rust 转译工具链的重大升级**，通过引入测试代码删除检测、CodeAgent 驱动的优化器、断点续跑机制等，显著提升了转译的可靠性和代码质量。同时，**Token 管理与 UI 增强**让用户能够实时了解资源使用情况，**事件总线优先级机制**和**上下文管理优化**让系统更加智能和高效。**输出系统重构**统一了日志输出方式，提升了代码的可维护性。

**技术亮点**：

- **测试代码保护机制**：通过订阅 BEFORE_TOOL_CALL 和 AFTER_TOOL_CALL 事件，在每次工具调用前后记录 commit 并检测是否误删 `#[test]` 或 `#[cfg(test)]` 标记。若检测到不合理删除，通过 LLM 判断并自动回退到工具调用前的 commit，覆盖 transpile、todo-fix、build-fix 及 review-fix 四个关键环节，确保测试完整性
- **CodeAgent 驱动优化器**：将优化器重构为 CodeAgent 驱动，所有优化步骤（Clippy 告警消除、unsafe 清理、可见性优化、文档补充）统一通过 CodeAgent 完成。改用 JSON 格式解析 Clippy 告警以提升准确性，支持按文件批量修复（每次修复前 10 个告警），提升智能分析与修复能力
- **智能 Token 管理**：基于剩余 Token 的上下文管理策略（剩余 token 低于 20% 触发自动摘要），在对话界面实时显示 Token 使用百分比和进度条，新增消息自动截断功能，支持因长度限制中断时自动续生成回复，确保对话连续性
- **多平台场景化选择**：扩展 PlatformRegistry 支持 cheap/smart/normal 三类平台，根据任务复杂度自动选择合适 LLM 平台。代码生成等复杂场景使用 smart 平台，上下文推荐、网页读取等一般任务使用 normal 或 cheap 平台，优化资源使用并提升任务执行效率
- **结构化编辑增强**：引入编辑错误类型枚举（EditErrorType），明确区分各类编辑错误。当 edit 操作匹配到多处文本时，显示最多 5 个匹配位置的上下文，根据不同的错误类型提供针对性的修复建议，提升错误诊断能力和用户体验
- **输出系统统一**：移除 PrettyOutput 依赖，统一使用标准 print 输出，并在消息前添加 Unicode 图标（如 ⚠️、❌、ℹ️、✅）以保留视觉语义。仅在需要语法高亮的场景保留 PrettyOutput，简化输出逻辑并减少模块耦合
