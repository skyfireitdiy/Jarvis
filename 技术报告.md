...
- 上下文与复用：
  - 构建当前函数的上下文头部（全量/精简），在复用代码生成与修复Agent时拼接（_reset_function_context/_compose_prompt_with_context/_refresh_compact_context）
  - **Review Agent 每次迭代重新创建**：在审查循环中，每次迭代都重新创建 Review Agent 实例，清除之前的对话历史和记忆，确保基于最新代码状态进行审查，不依赖历史对话
- 代码生成与修复（CodeAgent，统一复用）：
  - 在单个函数生命周期内创建并复用同一个CodeAgent实例，统一用于代码生成和修复任务（_get_code_agent）
  - CodeAgent配置：启用方法论和分析功能（use_methodology=True, use_analysis=True），附加工具包括 read_symbols 和 methodology，用于提供更好的代码生成和修复能力
  ...
  - 记忆功能：CodeAgent默认不强制保存记忆（force_save_memory=False），但提示词中会建议Agent在需要时使用 save_memory 工具记录关键信息
...
- 构建校验与最小修复（核心闭环）：
  ...
  - 测试失败时：错误分类（missing_import/type_mismatch/visibility/borrow_checker/dependency_missing/module_not_found），使用CodeAgent进行最小修复并继续循环（_classify_rust_error/_cargo_build_loop）
  ...
  - 测试失败信息反馈：测试失败时使用 `cargo test -- --nocapture` 获取完整的测试失败信息（包括测试用例名称、断言失败位置、期望值与实际值、堆栈跟踪等），并通过专门的 `<TEST_FAILURE>` 标签传递给CodeAgent，明确区分测试失败和编译错误
...
- 审查与复核：
  - Review Agent 合并审查功能一致性和严重问题（_review_and_optimize）：
    - **每次迭代重新创建 Agent**：在审查循环的每次迭代中，都重新创建 Review Agent 实例，清除之前的对话历史和记忆，确保基于最新代码状态进行审查，不依赖历史对话或缓存信息
    ...
    - **允许签名不一致**：允许函数签名、参数数量、参数类型、返回类型等与C实现不一致，只要功能实现了即可。只有当签名变更确实导致调用方代码无法编译或运行时，才报告为破坏性变更
  - 审查采用循环验证机制：发现问题 → 修复 → 构建验证 → 重新审查（每次重新创建 Agent），最多迭代 review_max_iterations 次（0 表示无限重试）
