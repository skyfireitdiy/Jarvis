...
- 初始化步骤（按顺序）：
  ...
  5) 读取规则文件（按优先级顺序加载，动态加载机制）：
     - **规则文件组织**：规则以 Markdown 文件形式组织，分为两类：
       - 通用规则：位于 `src/jarvis/jarvis_code_agent/rules/` 目录，包含开发实践规则（TDD、代码审查、重构、整洁代码、错误处理、安全、性能、文档、Git工作流等）
       - 测试规则：位于 `src/jarvis/jarvis_code_agent/test_rules/` 目录，包含各语言的测试规则（Python、JavaScript、Java、Go、Rust、C/C++、Ruby、PHP等）
