...
- 初始化步骤（按顺序）：
  ...
  4) 创建 Agent 实例：
     ...
     - 启用规划（plan，默认从配置读取）
  5) 订阅 AFTER_TOOL_CALL 事件：注册 `_on_after_tool_call` 回调
...
- 核心流程：
  ...
  2) 构建按文件补丁预览（_build_per_file_patch_preview）：
  ...
  5) 会话上下文更新：
     ...
     - 静态扫描引导（可选）：若启用静态分析且修改了文件，生成 lint 建议并追加到 `agent.session.addon_prompt`
  6) 用户确认机制（提交被拒绝时）：
     - 输出预览与拒绝提示
...
alt 提交成功
  ...
  alt 启用静态分析且修改了文件
    ...
    CA -> CA: 生成静态扫描引导并追加到 addon_prompt
  end
else 被拒绝或未提交
  CA -> CA: 输出预览并提示拒绝
  alt 用户确认或无需确认
...
- 静态检查提示：
  - 根据文件扩展名或文件名（通过 `get_lint_tools`）选择 lint 工具
  - 支持从 `{get_data_dir()}/lint_tools.yaml` 加载自定义配置
  - 当启用静态分析（is_enable_static_analysis）时，追加"集中扫描与一次性修复"的附加提示
- 用户确认机制：
...
- 统计集成：扩展指标（语言分布、文件修改热度）以增强可观测性；在 `_on_after_tool_call` 中追加统计记录点。
...
- 执行流程（run 方法）：
  ...
  5) 运行 Agent：`self.agent.run(enhanced_input)`
  6) 任务完成后处理：
