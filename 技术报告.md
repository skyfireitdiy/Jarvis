...
- 审查与复核：
  - Review 仅判断功能一致性（核心输入输出、主要功能逻辑是否与 C 实现一致）；允许 Rust 实现修复 C 代码中的安全漏洞或使用不同的类型设计、错误处理方式、资源管理方式等，只要功能一致即可；不检查类型匹配、边界检查、资源释放细节等技术细节
  - TypeBoundary 仅检查可能导致功能错误的严重问题（如明显的空指针解引用、明显的越界访问等），不检查类型匹配、指针可变性、边界检查细节等技术细节
...
- **模块与签名规划**
  ...
  - **约束**：
    - 目标文件位于 `crate/src/` 下，禁止指向 `src/main.rs`
    - 文件后缀为 `.rs` 或 `mod.rs`
    - 签名需包含 `fn` 名称，函数接口设计应遵循 Rust 最佳实践，不需要兼容 C 的数据类型
    - 禁止使用 `extern "C"`；函数应使用标准的 Rust 调用约定，不需要 C ABI
    - 优先使用 Rust 原生类型（如 i32/u32/usize、&[T]/&mut [T]、String、Result<T, E> 等），而不是 C 风格类型（如 core::ffi::c_*、libc::c_*）
    - 参数个数与顺序可以保持与 C 一致，但类型设计应优先考虑 Rust 的惯用法和安全性
  - **重试机制**：对规划结果进行严格校验，至多 `plan_max_retries` 次重试（默认 5），失败回退至兜底模块与启发式签名
...
- **代码生成与最小修复**
  - **生成策略**：
    ...
    - 生成最小单元测试：为目标函数生成 `#[cfg(test)]` 的 smoke 测试（占位参数、unsafe 包裹）
  - **构建验证**：
    - 先执行 `cargo check`（更快），通过后再执行 `cargo test`（确保最小测试通过）
    - 失败时按分类标签进行聚焦修复：`missing_import`/`type_mismatch`/`visibility`/`borrow_checker`/`dependency_missing`/`module_not_found` 等
    - 每轮修复均保持最小改动：修正声明/依赖、补齐未实现被调函数、精确 `use` 导入（避免通配）、必要时更新 `Cargo.toml`
  - **签名一致性检查**：
    - 基于 C 的 `params`/`return_type` 检查 Rust 签名的一致性（参数个数、指针可变性、ptr+len 组合、返回指针可变性）
    - 按问题列表执行"仅签名层面"的最小修复（sig-fix），并记录 `metrics`（`_check_signature_consistency`、`_infer_rust_signature_hint`）

- **审查与复核**
  - **Review Agent**：审查"功能一致性"（核心输入输出、主要功能逻辑是否与 C 实现一致），允许 Rust 实现修复 C 代码中的安全漏洞或使用不同的类型设计、错误处理方式、资源管理方式等，只要功能一致即可。不检查类型匹配、边界检查、资源释放细节等技术细节。OK 则进入二次审查；否则最小优化并回到构建闭环（`_review_and_optimize`）
  - **TypeBoundary Review Agent**：仅检查可能导致功能错误的严重问题（如明显的空指针解引用、明显的越界访问等），不检查类型匹配、指针可变性、边界检查细节等技术细节。若不通过则最小修复（`_type_boundary_review_and_fix`）
  - 审查失败不阻塞主流程，通过最小修复回到构建闭环，直到通过或达上限
...
- **验证顺序**：
  ...
  - 每轮记录 `build_attempts`、`impl_verified`、`last_build_error` 等度量到 `progress.json` 的 `current.metrics` 字段
  - 最多迭代 `max_retries` 次（0 表示不限制），超出上限则记录失败并继续下一函数（支持断点续跑）
...
- **transpiler（转译阶段）**
  ...
  - **TypeBoundary Review Agent**（`_type_boundary_review_and_fix`）：
    - 聚焦指针可变性检查（`*const T` ↔ `&T`，`*mut T` ↔ `&mut T`）
    - 空指针检查（确保指针非空）
    - 边界检查（`(ptr,len)` 组合的上界检查）
    - FFI 不变式验证（`extern "C"` + `#[repr(C)]` 的布局/对齐要求）
    - SAFETY 注释完整性检查（若缺失则补充）
  - **签名一致性检查**（`_check_signature_consistency`）：基于 C 的 `params`/`return_type` 检查 Rust 签名的一致性（参数个数、指针可变性、ptr+len 组合、返回指针可变性），必要时进行"仅签名层面"的最小修复。
...
- **注释即契约**：
  ...
  - TypeBoundary Review 强制检查 SAFETY 注释完整性，缺失则补充
...
- **transpiler/TypeBoundary（转译阶段）**：
  - 在生成阶段：CodeAgent 尽量生成安全抽象，避免不必要的 unsafe
  - 在复核阶段：TypeBoundary Review Agent 强制补齐不变式与边界检查
  - 检查重点：指针可变性、空指针检查、边界检查、FFI 不变式与 SAFETY 注释
...
bzip2 是一个基于 Burrows-Wheeler 算法和 Huffman 编码的文件压缩工具，其代码库包含约 15 个 C/C++ 源文件，涵盖核心压缩库（libbzip2）和命令行工具（bzip2、bzip2recover 等）。本文档基于对该项目的完整安全分析过程，展示 jarvis-sec 系统的分析能力。

#### 分析过程概述

采用 jarvis-sec 的四阶段流水线对 bzip2 项目进行了全面安全分析：

1. **启发式扫描阶段**：通过纯 Python 实现的 C/C++ 检查器对项目进行静态扫描，识别出潜在的安全问题模式，生成候选问题列表（heuristic_issues.jsonl），共检出约 964 个候选问题。
...
经过四阶段流水线分析，系统从约 964 个候选问题中筛选出 **7 个真实安全问题**，误报率约为 99.3%，体现了系统在降低误报方面的有效性。

**问题分布统计：**
- **按类别**：
  - `unsafe_api`: 4 个（缓冲区溢出相关）
  - `memory_mgmt`: 2 个（空指针解引用、内存分配未检查）
  - `error_handling`: 1 个（I/O 错误处理缺失）
- **按风险文件**：
  - `bzip2recover.c`: 3 个高风险问题
  - `bzlib.c`: 3 个中高风险问题
  - `bzip2.c`: 1 个高风险问题
- **按严重性**：
  - `high`: 6 个
...
**问题1-3：bzip2recover.c 中的字符串操作溢出**
- **位置**：`bzip2recover.c:473, 484, 486`
- **问题**：在生成恢复文件名时，使用 `strcpy` 和 `strcat` 进行字符串拼接，未检查总长度是否超过 `BZ_MAX_FILENAME`（2000 字节）限制
- **触发路径**：外部输入文件名 → `strcpy(outFileName, inFileName)` → `strcat(inFileName+ofs)` → `strcat(outFileName, ".bz2")`，当总长度超过缓冲区时造成栈溢出
- **影响**：可能导致程序崩溃或远程代码执行
- **置信度**：0.85-0.9，**严重性：high**

**问题7：bzip2.c 中的文件名后缀操作溢出**
- **位置**：`bzip2.c:1136`
- **问题**：`mapSuffix` 函数中使用 `strcat(name, newSuffix)` 时，未检查 `name` 缓冲区剩余空间
- **影响**：栈缓冲区溢出，可导致程序崩溃或任意代码执行
- **置信度**：0.9，**严重性：high**
...
**问题4：malloc 返回值未检查**
- **位置**：`bzlib.c:104`
- **问题**：`default_bzalloc` 函数中 `malloc` 返回值未检查，当内存分配失败返回 NULL 时，后续使用会导致空指针解引用
- **触发路径**：`BZ2_bzCompressInit` / `BZ2_bzDecompressInit` → `default_bzalloc` → `malloc` 返回 NULL → 直接使用指针
- **影响**：段错误导致程序崩溃，可能被远程利用造成拒绝服务
- **置信度**：0.65，**严重性：medium**

**问题6：可能的空指针解引用**
- **位置**：`bzlib.c:1564`
- **问题**：`BZ2_bzerror` 函数中直接解引用 `errnum` 参数，未检查是否为 NULL
- **触发路径**：外部通过 `BZ2_bzerror(b, NULL)` 调用，导致 `*errnum = err` 解引用 NULL 指针
- **影响**：立即触发段错误，导致拒绝服务或程序异常终止
- **置信度**：0.6，**严重性：high**
...
**问题5：fclose 返回值未检查**
- **位置**：`bzlib.c:1530`
- **问题**：`BZ2_bzclose` 函数中调用 `fclose(fp)` 后未检查返回值，当文件系统出现 I/O 错误时，错误被静默丢弃
- **触发路径**：磁盘已满、文件系统只读、网络文件系统断开等场景下，`fclose` 返回 EOF 表示写入缓存失败，但代码未处理
- **影响**：数据可能未真正落盘，压缩文件尾部不完整，后续读取时出现 CRC 错误或解压失败
- **置信度**：0.65，**严重性：medium**

#### 系统分析能力体现

1. **精准的启发式规则**：系统能够识别 C/C++ 代码中的不安全 API 使用（如 `strcpy`、`strcat`）、内存管理问题（如 `malloc` 未检查、空指针解引用）和错误处理缺失（如 I/O 调用未检查返回值）等常见安全模式。
...
通过 bzip2 项目的分析实践，验证了 jarvis-sec 系统在以下方面的能力：

- **高精度**：从 965 个候选问题中筛选出 7 个真实问题，误报率约 99.3%
- **全面性**：覆盖缓冲区溢出、内存管理、错误处理等多个安全类别
- **实用性**：提供详细的触发路径分析和修复建议，便于开发者快速定位和修复问题
- **可扩展性**：基于启发式规则和 AI 验证的混合架构，能够适应不同规模和复杂度的代码库
