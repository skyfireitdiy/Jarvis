...
- **Agent 用于规划**：
  - 模块规划（`llm_module_agent.py`）：使用 Agent 生成 crate 模块结构（JSON）
...
4. **工具共享**：所有系统共享 `ToolRegistry`（工具注册表），使用统一的工具接口（`read_code`、`edit_file`/`rewrite_file`、`execute_script` 等）
...
- **events**（事件定义）
  ...
  - 源码位置：`src/jarvis/jarvis_agent/events.py`
...
- **TaskManager**（任务管理）
  ...
  - 聚焦：任务加载、选择界面、补充信息收集
  - 源码位置：`src/jarvis/jarvis_agent/task_manager.py::TaskManager`
...
- **builtin_input_handler**（内置输入处理器）
  - 职责：处理内置命令（如 `prompt`、`clear` 等）
  - 源码位置：`src/jarvis/jarvis_agent/builtin_input_handler.py`
- **shell_input_handler**（Shell 输入处理器）
  - 职责：处理 Shell 命令（以 `!` 开头）
  - 源码位置：`src/jarvis/jarvis_agent/shell_input_handler.py`
- **file_context_handler**（文件上下文处理器）
  - 职责：处理文件引用（如 `@file.txt`），自动加载文件内容到上下文
  - 源码位置：`src/jarvis/jarvis_agent/file_context_handler.py`
...
- **OutputHandlerProtocol**（输出处理器协议）
  - 职责：定义输出处理器的接口协议
  - 源码位置：`src/jarvis/jarvis_agent/protocols.py::OutputHandlerProtocol`
- **WebOutputSink**（Web 输出接收器）
  - 职责：将输出事件广播到 Web 前端
  - 源码位置：`src/jarvis/jarvis_agent/web_output_sink.py::WebOutputSink`
...
- **WebServer**（Web 服务器）
  ...
  - 源码位置：`src/jarvis/jarvis_agent/web_server.py`
- **WebBridge**（WebSocket 桥接）
  - 职责：提供线程安全的 WebSocket 交互桥，处理多行输入和确认请求
  - 聚焦：阻塞式输入/确认请求、事件约定、客户端管理
  - 源码位置：`src/jarvis/jarvis_agent/web_bridge.py::WebBridge`
- **StdioRedirect**（标准流重定向）
  - 职责：将标准输出/错误重定向到 WebSocket 通道
  - 聚焦：工具直接打印输出的捕获与转发
  - 源码位置：`src/jarvis/jarvis_agent/stdio_redirect.py`

**3.1.7 分享管理模块（Share Management）**

- **ShareManager**（分享管理器基类）
  - 职责：提供工具和方法论分享的通用逻辑（Git 仓库管理、资源选择等）
  - 源码位置：`src/jarvis/jarvis_agent/share_manager.py::ShareManager`
- **MethodologyShareManager**（方法论分享管理器）
  - 职责：管理方法论的分享流程
  - 源码位置：`src/jarvis/jarvis_agent/methodology_share_manager.py::MethodologyShareManager`
- **ToolShareManager**（工具分享管理器）
  - 职责：管理工具的分享流程
  - 源码位置：`src/jarvis/jarvis_agent/tool_share_manager.py::ToolShareManager`

**3.1.8 辅助模块（Utilities）**

- **prompts**（提示词定义）
  - 职责：定义默认提示词模板（DEFAULT_SUMMARY_PROMPT、SUMMARY_REQUEST_PROMPT 等）
  - 源码位置：`src/jarvis/jarvis_agent/prompts.py`
- **prompt_builder**（提示词构建器）
  - 职责：构建工具使用提示（action_prompt）
  - 源码位置：`src/jarvis/jarvis_agent/prompt_builder.py`
- **protocols**（协议定义）
  - 职责：定义接口协议（OutputHandlerProtocol 等）
  - 源码位置：`src/jarvis/jarvis_agent/protocols.py`
- **utils**（工具函数）
  - 职责：提供通用工具函数（如 `join_prompts`）
  - 源码位置：`src/jarvis/jarvis_agent/utils.py`
- **config_editor**（配置编辑器）
  - 职责：提供配置文件编辑功能
  - 源码位置：`src/jarvis/jarvis_agent/config_editor.py::ConfigEditor`
- **language_support_info**（语言支持信息）
  - 职责：提供编程语言支持信息（文件扩展名映射、语言特性等）
  - 源码位置：`src/jarvis/jarvis_agent/language_support_info.py`
...
- **jarvis**（主入口）
  - 职责：`jarvis` 命令的入口点，提供完整的 CLI 接口和 Web 模式支持
  - 聚焦：命令行参数解析、配置管理、AgentManager 初始化、Web 服务启动
  - 源码位置：`src/jarvis/jarvis_agent/jarvis.py`
- **main**（代理入口）
  - 职责：`jarvis-agent` 命令的入口点，提供简化的 Agent 启动接口
  - 聚焦：Agent 定义文件加载、Agent 实例化、任务执行
  - 源码位置：`src/jarvis/jarvis_agent/main.py`
...
- **输入处理器链**（builtin_input_handler/shell_input_handler/file_context_handler）
  ...
  - builtin_input_handler：处理内置命令（如 `prompt`、`clear` 等）
...
- **jarvis**（主入口）
  - `jarvis` 命令的入口点，提供完整的 CLI 接口（配置管理、数据备份/恢复、Web 模式等）
- **main**（代理入口）
...
##### 3.7 文件编辑工具（edit_file/rewrite_file）设计

读者要点

- 用途：通过工具调用进行最小必要变更（edit_file）或整文件替换（rewrite_file）
- 编辑模式：edit_file 仅支持结构化编辑模式，基于 read_code 的缓存机制进行精确编辑
- 安全约束：
  - 结构化编辑：基于缓存验证，缓存无效时提示重新读取；支持原子写入和失败回滚
  - rewrite_file 采用原子写并可回滚
- 选择策略：优先使用结构化编辑（通过 block_id），仅在大范围重构或整文件生成时使用 rewrite_file
- 结果呈现：返回成功与差异摘要；失败时明确说明原因（缓存无效、块不存在等）
...
- 初始化步骤（按顺序）：
  ...
  4) 构建工具白名单：
     - 基础工具：execute_script、search_web、read_code、save_memory、retrieve_memory、clear_memory、sub_code_agent
     - 追加工具：通过 `append_tools` 参数（逗号分隔）追加，自动去重
  5) 读取规则文件：
     - 全局规则：`{get_data_dir()}/rules`（若存在）
     - 项目规则：`.jarvis/rules`（若存在）
     - 按顺序拼接为单一 `<rules>` 区块，追加到系统提示
  6) 调用父类 Agent 初始化（super().**init**）：
...
- 代码读取：read_code
  - 用途：结构化读取源代码文件，支持按语法单元（函数、类等）提取代码块，为每个块生成唯一 ID，便于后续结构化编辑。
  - 功能特性：
    - 结构化输出：支持的语言按语法单元提取，不支持的语言按空白行分组或行号分组
    - 块 ID 生成：为每个代码块生成唯一标识符（如 `function_name`、`ClassName.method_name`），缓存中使用顺序ID（`block-1`、`block-2`等）
    - 边界处理：返回与请求范围有重叠的完整语法单元，确保语义完整性
    - Token 限制：自动估算输出 token 数，超过限制（2/3最大窗口）时提示分批读取
    - 缓存机制：基于文件修改时间的缓存系统，支持从缓存快速恢复文件内容，提高编辑效率
    - 原始模式（raw_mode）：当 `raw_mode=true` 时，按每20行分组读取，适用于不支持语法分析的语言
    - 空白行分组优化：对超过20行的空白行分组块自动按每20行再次分割，避免单个块过大
    - 导入语句提取：自动提取并单独显示导入/包含语句，便于理解依赖关系
    - 上下文感知：集成 ContextManager，自动提供代码上下文信息（当前作用域、使用的符号、相关文件等）
  - 缓存机制：
    - 缓存格式：使用 `id_list`（有序块ID列表）和 `blocks`（块ID到内容的映射）存储文件结构
    - 缓存验证：通过文件修改时间（mtime）验证缓存有效性，允许0.1秒误差
    - 缓存恢复：支持从缓存完美恢复原始文件内容（包括换行符处理）
    - 缓存更新：文件编辑后自动更新缓存，保持一致性
  - 最佳实践：遵循"先读后写"，优先按范围读取（start_line/end_line）以避免上下文膨胀；使用结构化块 ID 进行编辑，避免手动计算行号；利用缓存机制提高重复读取效率。

- 文件编辑工具：edit_file / rewrite_file
  - edit_file：进行最小必要变更，仅支持结构化编辑
    - 结构化编辑：通过 `block_id` 进行编辑，基于 read_code 缓存机制，支持 delete、insert_before、insert_after、replace、edit 五种操作
    - 原子写入与失败回滚；块不存在或缓存无效需明确失败原因。
  - rewrite_file：整文件重写，适用于大范围生成或重构，支持原子写与回滚。
  - 建议：优先使用 edit_file 进行结构化编辑（通过 block_id），仅在确需整文件重写时使用 rewrite_file。

- 结构化编辑（Structured Editing）
  - 概述：CodeAgent 提供基于代码结构块的精确编辑能力，通过 `read_code` 工具获取结构化块 ID，使用 `edit_file` 工具进行编辑。
  - 核心优势：
    - 精确定位：基于语法单元（函数、类等）的块 ID，避免行号变化导致的定位错误
    - 语义理解：支持的语言（Python、Rust、Go、C/C++、Java 等）按语法单元提取，不支持的语言按空白行分组
    - 操作灵活：支持删除块、在块前插入、在块后插入、替换块四种操作
    - 自动适配：自动处理嵌套作用域，返回完整的语法单元（即使部分在请求范围外）
  - 工作流程：
    1. 使用 `read_code` 工具读取文件，获取结构化输出（包含块 ID）
    2. 从输出中提取目标代码块的 `block_id`（格式如 `function_name` 或 `ClassName.method_name`）
    3. 使用 `edit_file` 工具的 `structured` 模式，通过 `block_id` 进行编辑操作
  - 结构化读取（read_code）：
    - 支持的语言：按语法单元提取（函数、类、结构体、枚举等）
      - Python：使用 AST 解析，提取函数、类、方法等
      - Rust/Go/C/C++/Java：使用 tree-sitter 解析，提取函数、结构体、类等
      - 其他语言：按空白行分组或行号分组（每20行一组，raw_mode=true 时）
    - 输出格式：每个代码块包含
      - `id`：块标识符（如 `main`、`Point.__init__`、`import_1`）
      - `start_line` / `end_line`：行号范围
      - `content`：代码内容（带行号标注）
    - 边界处理：返回与请求范围有重叠的所有语法单元（包括边界上的），确保完整语法单元
    - 嵌套处理：自动处理嵌套作用域，父符号排除被子符号覆盖的行，避免重复显示
  - 结构化编辑（edit_file structured 模式）：
    - 操作类型：
      - `delete`：删除指定的代码块（清空块内容）
      - `insert_before`：在代码块前插入新内容
      - `insert_after`：在代码块后插入新内容
      - `replace`：替换整个代码块
      - `edit`：在代码块内进行 search/replace 操作（需要提供 `search` 和 `replace` 参数）
    - 参数格式：
    ...
    - 基于缓存的编辑机制：
      - 缓存验证：编辑前检查缓存有效性，无效时提示先使用 `read_code` 重新读取
      - 缓存操作：所有编辑操作在缓存副本中进行，避免直接修改原缓存
      - 内容恢复：从修改后的缓存恢复完整文件内容，确保换行符和格式一致性
      - 缓存更新：编辑成功后自动更新缓存，保持缓存与文件同步
    - 定位机制：通过缓存中的 `block_id` 直接定位代码块
      - 使用 `read_code` 工具获取的 `block_id`（格式如 `block-1`、`block-2` 等）
      - 支持从缓存中快速查找块内容
    - 原子写入与回滚：
      - 编辑前创建备份文件（`.bak`）
      - 写入失败时自动回滚到备份
    ...
    - 错误处理：
      - 缓存无效：提示重新读取文件
      - 块不存在：明确提示未找到块ID，建议使用 `read_code` 查看可用块ID
      - 部分失败：支持部分补丁成功，返回详细错误摘要
  - 使用示例：
    - 读取文件获取块 ID：

      ```python
      read_code({"files": [{"path": "src/main.py", "start_line": 1, "end_line": 100}]})
      ```
  ...
  - 最佳实践：
    - 优先使用结构化编辑，避免手动计算行号
    - 先使用 `read_code` 获取结构化块 ID，再进行编辑
    - 对于不支持的语言，使用 `raw_mode=true` 按行号分组读取
    - 嵌套作用域时，优先使用最内层的块 ID（如方法而非类）
  - 实现位置：
    - 结构化提取：`jarvis_code_agent/code_analyzer/structured_code.py::StructuredCodeExtractor`
    - 读取工具：`jarvis_tools/read_code.py::ReadCodeTool`
    - 编辑工具：`jarvis_tools/edit_file.py::EditFileTool._fast_edit()` 和 `_apply_structured_edit_to_cache()`
  - 技术细节：
    - 缓存格式：使用 `id_list`（有序列表）和 `blocks`（字典）存储，支持完美恢复文件内容
    - 换行符处理：缓存中存储时去掉块末尾换行符，恢复时根据 `file_ends_with_newline` 标志正确处理
    - 块分割策略：按行号范围连续分割文件，确保每行只被提取一次，完美恢复原始文件
    - Token 估算：基于结构化输出格式估算 token 数，支持语法单元、空白行分组、行号分组三种模式
...
- **源码位置**：`src/jarvis/jarvis_c2rust/`
  ...
  - `cli.py`：Typer 命令行入口，提供分组式子命令（config/scan/lib-replace/prepare/transpile/run/optimize）
...
- **核心数据目录与产物**（默认路径 `<project_root>/.jarvis/c2rust/`）：
  ...
  - **配置与状态**：
    - `config.json`：配置文件（root_symbols、disabled_libraries），由 config 命令管理
...
- **明确产物与断点续跑**：每阶段产物写入 `.jarvis/c2rust`，形成可复现与可继续的中间结果；各阶段完成后自动记录状态到 `run_state.json`，支持流水线断点续跑；转译与优化阶段支持细粒度 `resume`（通过 `progress.json` 和 `optimize_progress.json` 记录进度）。
...
' 数据产物流向
Config --> LibReplacer : config.json (root_symbols, disabled_libraries)
...
- 配置
  - config.json：根函数名列表（root_symbols）和禁用库列表（disabled_libraries），由 config 命令管理
- 库替代映射
...
- **config**：管理配置文件（`.jarvis/c2rust/config.json`），设置根符号列表和禁用库列表
  - 参数：`--files <paths...>`（头文件或函数名列表文件，头文件自动提取函数名），`--root-list-syms <str>`（根符号列表内联，逗号分隔），`--disabled-libs <str>`（禁用库列表，逗号分隔），`--show`（显示当前配置），`--clear`（清空配置）
...
- **scan**：执行 C/C++ 扫描，输出 `symbols.jsonl` / DOT 图（可生成 PNG）、子图等
  ...
  - 前置依赖：无
  - 状态记录：完成后自动记录到 `run_state.json`
- **lib-replace**：对指定根列表进行库替代评估与剪枝（LLM-only 子树评估）
  - 参数：`-g/--llm-group <name>`（可选模型组），`--interactive`（启用交互模式）
  - 前置依赖：scan
  - 配置来源：从 `config.json` 读取 `root_symbols` 和 `disabled_libraries`
  - 状态记录：完成后自动记录到 `run_state.json`
...
- **transpile**：按 `translation_order.jsonl` 逐函数转译并进行构建修复
  - 参数：`-g/--llm-group <name>`（可选模型组），`-m/--max-retries <N>`（构建/修复最大重试次数，0 表示不限制），`--interactive`（启用交互模式）
  - 前置依赖：scan、prepare
  - 配置来源：从 `config.json` 读取 `root_symbols` 和 `disabled_libraries`
  - 状态记录：完成后自动记录到 `run_state.json`
  - 断点续跑：默认始终启用（通过 `progress.json` 记录细粒度进度）
- **optimize**：对生成的 crate 执行保守优化（unsafe 清理、结构与可见性优化、文档补充、cargo test）
  - 参数：`--crate-dir <path>`（可选，未提供时自动检测），`--unsafe/--no-unsafe`（启用 unsafe 清理，默认启用），`--structure/--no-structure`（启用代码结构优化，默认启用），`--visibility/--no-visibility`（启用可见性优化，默认启用），`--doc/--no-doc`（启用文档补充，默认启用），`-m/--max-checks <N>`（cargo test 最大次数限制，0 表示不限），`--dry-run`（仅统计潜在修改），`--include/--exclude <patterns>`（文件过滤，逗号分隔 glob），`-n/--max-files <N>`（本次最多处理的文件数，0 表示不限），`--resume/--no-resume`（断点续跑，默认启用），`--reset-progress`（重置优化进度），`-r/--build-fix-retries <N>`（构建失败后的最小修复重试次数，默认 3），`--git-guard/--no-git-guard`（启用 Git 保护，默认启用），`--interactive`（启用交互模式）
  - 前置依赖：transpile
  - 状态记录：完成后自动记录到 `run_state.json`
- **run**：一键流水线（scan → lib-replace → prepare → transpile → optimize）
  - 参数：`-g/--llm-group <name>`（LLM 相关阶段使用的模型组），`-m/--max-retries <N>`（transpile 构建/修复最大重试次数），`--interactive`（启用交互模式），`--reset`（重置状态，从头开始执行所有阶段）
  - 配置来源：从 `config.json` 读取 `root_symbols` 和 `disabled_libraries`（使用 `config` 命令预先设置）
  - 断点续跑：根据 `run_state.json` 自动跳过已完成的阶段，支持从任意阶段继续
  - 约束：各阶段执行前自动检查前置依赖，确保流程正确性
...
CLI -> Optimizer : 保守优化（如未完成，unsafe/结构/可见性/文档 + cargo checks）
...
== 阶段1: 扫描 ==
用户 -> CLI: jarvis-c2rust scan
CLI -> Scanner: run_scan()
Scanner -> Scanner: 扫描C/C++代码\n生成symbols.jsonl

== 阶段2: 库替代评估 ==
用户 -> CLI: jarvis-c2rust lib-replace
CLI -> LibReplacer: apply_library_replacement()
...
== 阶段3: 模块规划 ==
用户 -> CLI: jarvis-c2rust prepare
CLI -> ModulePlanner: execute_llm_plan()
...
== 阶段4: 代码转译 ==
用户 -> CLI: jarvis-c2rust transpile
...
loop 对每个函数
    ...
    alt 跳过实现
        ...
        Transpiler -> CodeGenAgent: 创建CodeAgent (生成)\nforce_save_memory=True
...
== 阶段5: 代码优化 ==
用户 -> CLI: jarvis-c2rust optimize
CLI -> Optimizer: optimize_project()
loop 对每个Rust文件
    Optimizer -> Optimizer: unsafe清理\n可见性优化\n文档补充
    Optimizer -> Optimizer: cargo test验证
    alt 优化失败
        Optimizer -> Optimizer: git reset回滚
    end
end
Optimizer -> OptCodeAgent: _codeagent_optimize_crate()
OptCodeAgent -> OptCodeAgent: 创建CodeAgent实例
OptCodeAgent -> OptCodeAgent: CodeAgent.run()\n整体优化
OptCodeAgent -> Optimizer: 返回优化结果
...
4. **阶段4 - 代码转译**（核心阶段）：
   ...
   - **CodeAgent (生成)**：生成 Rust 实现，检查并补齐依赖函数，生成测试用例
   - **CodeAgent (修复)**：修复构建错误和测试失败（启用方法论和分析）
   - **Review Agent**：审查代码功能一致性，发现问题时调用 CodeAgent 优化

5. **阶段5 - 代码优化**：
   - Optimizer 进行保守优化（unsafe 清理、可见性优化等）
   - CodeAgent 进行整体优化

**Agent 复用策略**：

- **单个函数生命周期内复用**：在转译单个函数时，CodeAgent (生成) 和 CodeAgent (修复) 在同一函数处理过程中复用，共享上下文
- **上下文传递**：通过 `_compose_prompt_with_context()` 在复用 Agent 时传递函数上下文
...
- 管理配置文件（`.jarvis/c2rust/config.json`），设置根符号列表（`root_symbols`）和禁用库列表（`disabled_libraries`）。
...
- 代码生成与修复（CodeAgent，分离复用）：
  - 在单个函数生命周期内创建两个CodeAgent实例：代码生成Agent和修复Agent，分别用于代码生成和修复任务
  - 代码生成Agent（_get_generate_agent）：禁用方法论和分析（use_methodology=False, use_analysis=False），仅启用强制记忆功能，用于快速生成初始实现
  - 修复Agent（_get_repair_agent）：启用方法论和分析（use_methodology=True, use_analysis=True），用于深入分析和修复问题
  - 代码生成：在目标模块生成或更新实现，遵循"最小变更、禁止 todo!/unimplemented!、必要时补齐依赖实现"的约束（_codeagent_generate_impl）
  - 强制使用记忆功能：所有Agent启用 force_save_memory=True，要求在完成函数实现后使用 save_memory 工具记录关键信息（函数名称、源文件位置、目标模块、Rust函数签名、核心功能与语义、关键实现细节、依赖关系、类型转换要点、错误处理策略、实际实现的Rust代码要点等）
...
- 构建校验与最小修复（核心闭环）：
  - cargo check → 若失败：错误分类（missing_import/type_mismatch/visibility/borrow_checker/dependency_missing/module_not_found），使用修复Agent进行最小修复并继续循环（_classify_rust_error/_cargo_build_loop）
  - 通过 check 后执行 cargo test（运行所有测试，不区分项目结构）→ 若失败：同样分类与最小修复，直到通过或达上限（check_max_retries 和 test_max_retries，0 表示无限重试）
...
repeat
  ...
  if (skip_implementation==true?) then (是)
    ...
    :初始化上下文与分离的代码生成Agent和修复Agent;
    :代码生成Agent生成/更新实现 (禁用分析和方法论、检查依赖、禁止 todo!/unimplemented!、保存记忆、召回记忆);
    :确保顶层 pub mod + 补齐 mod.rs 声明链;
    :构建闭环: cargo check → (失败? 修复Agent最小修复，启用分析和方法论) → cargo test(所有测试) → (失败? 修复Agent最小修复，包含完整测试失败信息);
    if (构建通过?) then (是)
...
- 构建闭环：
  - 先 cargo check（更快），通过后再 cargo test（运行所有测试，不区分项目结构）；失败时使用修复Agent（启用分析和方法论）按分类标签进行聚焦修复
...
- 使用 Typer 暴露子命令：config / scan / lib-replace / prepare / transpile / run / optimize。
...
- config
  - --files（头文件或函数名列表文件）、--root-list-syms（根符号列表内联）、--disabled-libs（禁用库列表）、--show（显示配置）、--clear（清空配置）
- lib-replace
  - -g/--llm-group、--interactive（从 config.json 读取 root_symbols 和 disabled_libraries）
- run
  - -g/--llm-group、-m/--max-retries、--interactive、--reset（从 config.json 读取配置，根据 run_state.json 断点续跑）
- optimize
...
- cli 子命令：config / scan / lib-replace / prepare / transpile / run / optimize
...
1. **循环控制测试**
   ...
   - `test_run_loop_conversation_rounds`：测试对话轮次计数
   - `test_run_loop_tool_reminder_rounds`：测试工具提醒轮次机制
   - `test_run_loop_auto_summary_rounds`：测试自动摘要轮次触发
...
6. **优化测试**
   ...
   - `test_optimizer_structure_optimization`：测试结构优化
...
**测试文件**：`tests/jarvis_tools/test_file_editing.py`
...
1. **edit_file 测试**
   - `test_edit_file_structured`：测试结构化编辑（delete、insert_before、insert_after、replace、edit 操作）
   - `test_edit_file_unique_match`：测试唯一匹配验证
   - `test_edit_file_atomic_write`：测试原子写入
   - `test_edit_file_rollback`：测试回滚机制

2. **rewrite_file 测试**
   - `test_rewrite_file_full_rewrite`：测试整文件重写
   - `test_rewrite_file_atomic_write`：测试原子写入
   - `test_rewrite_file_rollback`：测试回滚机制
