...
package "功能增强层" #LightYellow {
  ...
  note right of CodeAgent
    基于 Agent，叠加代码相关能力：
    - 代码结构分析
    - 文件编辑工具（edit_file_normal：search/replace 编辑；edit_file_free：基于新代码片段的自由编辑）
...
##### 3.9 文件编辑工具（edit_file_normal / edit_file_free / rewrite_file）设计

读者要点

- 用途：通过工具调用进行最小必要变更（edit_file_normal / edit_file_free）或整文件替换（rewrite_file）
- 编辑模式：
  - edit_file_normal：基于 search/replace 的普通文本编辑（使用相似度匹配，阈值 0.85）
  - edit_file_free：仅提供新代码片段，由工具自动进行模糊匹配和插入/替换
- 安全约束：
  - 编辑工具：支持原子写入和失败回滚
  - rewrite_file 采用原子写并可回滚
- 选择策略：优先使用 edit_file_normal 或 edit_file_free，仅在大范围重构或整文件生成时使用 rewrite_file
- 结果呈现：返回成功与差异摘要；失败时明确说明原因
...
start
:解析响应中的写入请求;
if (edit_file_normal/edit_file_free 调用?) then (是)
  :使用相似度匹配定位代码;
  if (匹配成功?) then (是)
    :应用编辑;
    :原子写入;
...
- 代码读取：read_code
  ...
  - 功能特性：
    ...
    - 单元 ID 生成：为每个代码单元生成唯一标识符（如 `function_name`、`ClassName.method_name`），输出中使用顺序ID（`unit-1`、`unit-2`等）
...
- 文件编辑工具：edit_file_normal / edit_file_free / rewrite_file
  - edit_file_normal：基于 search/replace 的普通文本编辑，使用相似度匹配（阈值 0.85），适合已知原始代码片段、需精确替换的场景
...
- 结构化读取（read_code）：
  ...
  - 文件编辑工具（edit_file_normal / edit_file_free）：
    - edit_file_normal：基于 search/replace 的普通文本编辑，使用相似度匹配（阈值 0.85）
  ...
  - 最佳实践：
    - 优先使用 edit_file_normal 进行简单的 search/replace
    - 使用 edit_file_free 进行基于上下文的模糊匹配编辑
  ...
  - 实现位置：
    ...
    - 编辑工具：`jarvis_tools/edit_file_normal.py` 和 `jarvis_tools/edit_file_free.py`
  - 技术细节：
...
- 文件编辑工具：edit_file_normal / edit_file_free / rewrite_file
  ...
  - 建议：优先使用 edit_file_normal 或 edit_file_free，仅在确需整文件重写时使用 rewrite_file
