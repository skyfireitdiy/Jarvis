...
- 初始化步骤（按顺序）：
  1) Git 配置检查（_check_git_config）：检查 `user.name` 和 `user.email` 是否已设置
  ...
  5) 创建 Agent 实例：
     ...
     - 设置工具白名单（use_tools）
     - 禁用方法论引导（use_methodology=False）
     - 禁用任务分析（use_analysis=False）
     - 关闭自动完成（auto_complete=False）
     - 启用规划（plan，默认从配置读取）
  6) 建立 CodeAgent 与 Agent 的关联：
     - 设置 `self.agent._code_agent = self`，便于工具获取上下文管理器
  7) 初始化上下文推荐器（ContextRecommender，可选）：
     - 若 Agent 已初始化模型且模型可用：
       * 创建 ContextRecommender 实例，传入 ContextManager 和 LLM 模型
       * 用于任务开始时的智能上下文推荐
     - 若初始化失败：仅记录警告，不影响主流程（跳过上下文推荐功能）
  8) 订阅 AFTER_TOOL_CALL 事件：注册 `_on_after_tool_call` 回调
...
- 核心流程：
  ...
  5) CodeAgent 构造与初始化：
     ...
     - CodeAgent 内部执行：git 配置检查、工具白名单构建、规则加载、Agent 创建、事件订阅
  6) 会话恢复（可选）：
...
- 职责：使用 LLM 进行语义理解，根据编辑意图推荐相关的上下文信息
- 核心功能：
  - 使用 LLM 提取用户意图和实体（_extract_intent_with_llm）
  - 基于目标文件推荐依赖文件和被依赖文件
  - 基于目标符号推荐定义和引用位置
  - 使用 LLM 进行语义搜索（_semantic_search_symbols、_semantic_search_files）
  - 使用 LLM 对推荐结果进行相关性评分和排序
  - 查找相关测试文件
- 推荐策略：
  - 基于依赖关系：推荐依赖的文件和被依赖的文件
  - 基于符号引用：推荐符号的定义和引用位置
  - 基于语义理解：使用 LLM 进行语义搜索和评分
...
- 智能上下文推荐容错：
  ...
  - 意图提取失败时使用默认值（unknown），继续推荐流程
  - 语义搜索失败时跳过语义推荐，仅使用基于依赖关系的推荐
- 差异容错：
...
- 上下文推荐扩展：
  ...
  - 语义搜索：优化 LLM 提示词，提高语义搜索准确性
  - 推荐排序：添加更多排序因子（如文件修改时间、符号使用频率等）
- 构建验证扩展：
...
- 执行流程（run 方法）：
  ...
  4) 智能上下文推荐（可选）：
     - 若启用意图识别（is_enable_intent_recognition）且上下文推荐器已初始化：
       * 从用户输入中提取文件路径和符号名称（_extract_file_paths_from_input、_extract_symbols_from_input）
       * 调用 ContextRecommender.recommend_context() 生成上下文推荐
       * 推荐策略：
         - 基于目标文件：推荐依赖文件和被依赖文件
         - 基于目标符号：推荐符号定义和引用位置
         - 基于语义理解：使用 LLM 进行语义搜索和评分
