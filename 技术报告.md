...
- **模块与签名规划**
  ...
  - **约束**：
    ...
    - 文件后缀为 `.rs` 或 `mod.rs`
    - 签名需包含 `fn` 名称，参数个数与顺序对齐 C
    - 指针可变性按 `const/*const`、非 `const/*mut` 区分（启发式映射）
...
- **边界优先**：优先将 unsafe 收敛在 FFI 边界层（`extern "C"` + `#[repr(C)]`），对外提供安全包装 API。在边界层进行参数校验（非空/长度/对齐/别名/生命周期），内部转为安全抽象（`&`/`&mut`/`&[T]`/`&CStr` 等）。
