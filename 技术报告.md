...
- **lib-replace**：对指定根列表进行库替代评估与剪枝（LLM-only 子树评估）
  ...
  - 前置依赖：scan
  - 配置来源：从 `config.json` 读取 `root_symbols`、`disabled_libraries` 和 `additional_notes`
  - 状态记录：完成后自动记录到 `run_state.json`
...
- 5) CodeAgent 整体优化（可选）：
  ...
  - **附加说明支持**：从 `config.json` 读取 `additional_notes`，自动追加到 CodeAgent 整体优化和构建修复提示词末尾，确保用户自定义要求在整个优化流程中生效
