...
- **CodeAgent 用于生成与修复**：
  - 代码生成（`transpiler.py._codeagent_generate_impl`）：使用 CodeAgent 生成转译后的 Rust 函数
  - 构建修复（`transpiler.py._cargo_build_loop`）：使用 CodeAgent 修复构建错误
...
2. **聚类阶段**：将候选问题按文件分组，使用单Agent对每个文件内的告警进行"验证条件一致性"聚类，生成聚类批次（cluster_report.jsonl）。聚类Agent可以在聚类阶段判断告警是否无效（is_invalid字段），无效的告警将不会进入后续验证阶段。
3. **分析阶段**：将每个聚类批次交由分析Agent执行"只读验证"，确认是否存在真实安全风险。如果分析Agent确认有告警，会启动验证Agent进行二次验证，只有验证通过的告警才会写入文件（agent_issues.jsonl）。
...
- 二次验证机制：分析Agent确认告警后，验证Agent会进行二次验证，只有验证通过的告警才会写入文件，减少误报。
- 记忆系统集成：聚类Agent、分析Agent和验证Agent都支持记忆工具（save_memory/retrieve_memory），可以保存和复用分析经验，提高分析效率和准确性。
- 早期无效过滤：聚类Agent可以在聚类阶段判断告警是否无效（is_invalid字段），无效的告警不会进入后续验证阶段，提高整体效率。
...
职责：将候选问题按文件分组，使用单Agent对每个文件内的告警进行"验证条件一致性"聚类，并可以在聚类阶段判断告警是否无效。
...
- 构造聚类 Agent：
  ...
  - 摘要输出格式：仅在 <CLUSTERS>...</CLUSTERS> 内输出 YAML 数组：
    ...
    - gids: 整数数组（属于该簇的全局编号）
    - is_invalid: 布尔值（必填，true 或 false）。如果为 true，表示该聚类中的所有候选已被确认为无效/误报，将不会进入后续验证阶段。
- 无效告警过滤：
  - 如果聚类结果中 is_invalid 为 true，不将该聚类写入 cluster_batches 和 cluster_records，跳过后续验证。
- 断点恢复：
  - 支持断点恢复：若 cluster_report.jsonl 存在，优先复用已有聚类结果。如果断点记录中 is_invalid 为 true，跳过该聚类。
- 写入聚类快照：
...
- 无效聚类（is_invalid=true）不会写入 cluster_batches，直接跳过后续验证。


##### 5.3 阶段3：分析（__init__ 中的验证逻辑）

职责：将每个聚类批次交由分析Agent执行"只读验证"，确认是否存在真实安全风险。如果分析Agent确认有告警，会启动验证Agent进行二次验证。
...
- 构造分析 Agent：
  ...
  - 记忆使用提示：提示Agent使用函数名作为tag，记忆每个函数的要点（如指针判空、输入校验等），分析时充分利用记忆。
...
- 二次验证机制：
  ...
  - 验证Agent摘要输出格式：仅在 <REPORT>...</REPORT> 内输出 YAML 数组，每个元素：
    - gid: int（全局编号）
    - is_valid: bool（分析Agent的结论是否正确）
    - verification_notes: string（验证说明）
...
- 二次验证机制：只有分析Agent和验证Agent都确认的告警才会写入文件，减少误报。
...
- 构建闭环：
  - 先 cargo check（更快），通过后再 cargo test；失败时按分类标签进行聚焦修复
