...
**应用场景**：
  ...
  - 被 jarvis-c2rust 用于代码生成与修复（`_codeagent_generate_impl`）
...
- **CodeAgent 用于生成与修复（统一复用）**：
  ...
  - 测试代码删除检测：基于事件订阅机制（`BEFORE_TOOL_CALL` 和 `AFTER_TOOL_CALL`），在每次工具调用后立即检测测试代码是否被错误删除，若检测到问题则立即回退，确保测试代码不会被意外删除（`transpiler.py._on_before_tool_call`、`transpiler.py._on_after_tool_call`，`optimizer.py` 中同样实现）
...
- **PlatformRegistry**（平台注册表）
  ...
  - 多平台支持：
    - `get_normal_platform()`：获取普通平台，用于一般任务
    - `get_cheap_platform()`：获取廉价平台，用于简单任务（查询重写、关键词提取、相关性评估等）
    - `get_smart_platform()`：获取智能平台，用于复杂任务（代码生成、代码转换等）
- **BasePlatform**（平台基类）
...
##### 3.9 文件编辑工具（edit_file/rewrite_file）设计
