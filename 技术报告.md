...
package "功能增强层" #LightYellow {
  ...
  note right of CodeAgent
    基于 Agent，叠加代码相关能力：
    - 代码结构分析
    - 文件编辑工具（edit_file_structed：结构化编辑；edit_file_normal：search/replace 编辑；edit_file_free：基于新代码片段的自由编辑）
...
##### 3.9 文件编辑工具（edit_file_structed / edit_file_normal / edit_file_free / rewrite_file）设计

读者要点

- 用途：通过工具调用进行最小必要变更（edit_file_structed / edit_file_normal / edit_file_free）或整文件替换（rewrite_file）
- 编辑模式：
  - edit_file_structed：结构化编辑模式，基于 read_code 的缓存机制与 block_id 进行精确编辑
  - edit_file_normal：基于 search/replace 的普通文本编辑
...
- 文件编辑工具：edit_file_structed / edit_file_normal / edit_file_free / rewrite_file
  - edit_file_structed：进行最小必要变更，结构化编辑
  ...
  - edit_file_normal：基于 search/replace 的普通文本编辑，适合已知原始代码片段、需精确替换的场景
  - edit_file_free：仅依赖新代码片段和上下文特征进行模糊匹配定位，适合接近 Cursor 风格的“给出目标代码片段即可”的编辑体验
  - rewrite_file：整文件重写，适用于大范围生成或重构，支持原子写与回滚。
  - 建议：
    - 优先使用 edit_file_structed 进行结构化编辑（通过 block_id）
    - 简单 search/replace 使用 edit_file_normal
    - 仅提供新代码片段、需工具自动定位时使用 edit_file_free
    - 仅在确需整文件重写时使用 rewrite_file。

- 结构化编辑（Structured Editing，edit_file_structed）
  - 概述：CodeAgent 提供基于代码结构块的精确编辑能力，通过 `read_code` 工具获取结构化块 ID，使用 `edit_file_structed` 工具进行编辑。
  - 核心优势：
...
- 结构化编辑与普通编辑：`tests/jarvis_tools/test_edit_file.py`

**测试类**：

- `TestEditFileTool`：测试结构化编辑（edit_file_structed）
- `TestEditFileNormalTool`：测试普通 search/replace 编辑（edit_file_normal）
- `TestEditFileFreeTool`：测试基于新代码片段自动定位的编辑（edit_file_free）

**测试用例示例**：

1. **edit_file_structed 测试**
   - 结构化编辑（delete、insert_before、insert_after、replace、edit 操作）
   - 唯一匹配验证、多处匹配报错、缓存失效处理
   - 原子写入与回滚机制、缓存恢复一致性验证

2. **edit_file_normal 测试**
