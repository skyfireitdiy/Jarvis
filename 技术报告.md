...
- 严格只读分析（不修改仓库）
- 支持断点续扫（通过3个清晰的配置文件：candidates.jsonl、clusters.jsonl、analysis.jsonl）
...
**测试目标**：验证完整六阶段流水线（collect → scan → lib-replace → prepare → transpile → optimize）的正确执行和产物质量。

**测试场景 2.1.1：一键流水线执行**

- **Given**：标准 C/C++ 测试项目，已配置的 LLM 平台，libclang 环境
- **When**：执行 `jarvis-c2rust run --files <headers> -o roots.txt -g <llm-group>`
- **Then**：
  - 所有阶段按顺序执行完成
  - 生成 `roots.txt`（collect 产物）
...
- **Given**：标准 C/C++ 测试项目
- **When**：分阶段执行各子命令（collect → scan → lib-replace → prepare → transpile → optimize）
- **Then**：
  - 每个阶段独立执行成功
  - 阶段间产物正确传递
  - 可以从中断点恢复执行
...
- **Given**：已执行部分阶段的测试项目（存在 `progress.json` 或 `optimize_progress.json`）
- **When**：使用 `--resume` 选项重新执行转译或优化
- **Then**：
  - 系统从断点恢复，跳过已处理的函数或文件
  - 已转换的函数不被重复处理
  - 新转换的函数被正确追加
...
**验证方法 2.2.1：collect 到 scan 的数据传递**

- **Given**：collect 阶段生成的 `roots.txt`
- **When**：执行 scan 阶段
- **Then**：
  - `roots.txt` 中的函数名在 `symbols.jsonl` 中能找到对应记录
  - 根函数识别正确（无入边或策略定义的入口）
  - 引用关系图正确构建

**验证方法 2.2.2：scan 到 lib-replace 的符号表传递**
