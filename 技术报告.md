...
- 事件总线（EventBus）、任务规划（TaskPlanner）、输入/输出处理链
- 支持非交互模式、子 Agent 递归规划执行等场景
...
**增强能力**：
- 代码结构分析（文件/函数/类层次结构）
- 精确补丁生成（PATCH：SEARCH/REPLACE、区间替换）
- 整文件重写（REWRITE）
- 变更影响分析（依赖关系、调用链分析）
- 自动迭代修复（基于构建错误）

**应用场景**：
- 被 jarvis-c2rust 用于代码生成与修复（`_codeagent_generate_impl`、`_codeagent_optimize_crate`）
...
- **Agent 验证**：使用 Agent 进行逐条验证（只读工具：`read_code`/`execute_script`）
...
**使用方式**：
- **Agent 用于规划**：
  - 模块规划（`llm_module_agent.py`）：使用 Agent 生成 crate 模块结构（YAML）
  - 签名规划（`transpiler.py._plan_module_and_signature`）：使用 Agent 选择目标模块与 Rust 函数签名
  - 代码审查（`transpiler.py._review_and_optimize`）：使用 Agent 审查逻辑一致性
- **CodeAgent 用于生成与修复（统一复用）**：
  - 代码生成与修复：在单个函数生命周期内复用同一个 CodeAgent 实例，同时用于代码生成和修复任务（`transpiler.py._codeagent_generate_impl`、`transpiler.py._cargo_build_loop`）
...
- 核心方法：
  ...
  - `_evaluate_plan_adjustment(...)`：评估计划是否需要调整
    ...
    - 输出 <PLAN_ADJUSTMENT> 块，包含 need_adjust、reason、adjusted_plan
    - 仅调整剩余计划，不修改已完成的子任务
...
@startuml
!theme plain
title 任务规划与子任务调度（拆分判定 / 深度限制 / 状态可视化 / 动态调整 / 结果写回）
...
if (是否需要拆分? plan=True 且任务可分) then (是)
  if (已达深度上限? plan_depth >= plan_max_depth) then (是)
    ...
    :将 <PLAN>/<SUB_TASK_RESULTS>/<RESULT_SUMMARY> 写回父会话;
  endif
else (否)
  :跳过规划，常规执行;
endif
...
- 计划状态可视化：在全局视图和执行过程中实时显示任务进度（待执行/执行中/已完成），使用状态标记提升可读性
- 计划动态调整：每完成一个子任务后，基于已完成结果评估剩余计划是否需要调整，支持根据实际情况优化后续步骤
- 调整约束：仅调整剩余计划，不修改已完成的子任务；调整后总数仍受 4 个子任务上限限制
- 子 Agent：默认非交互自动完成，继承父 Agent 能力与配置，传递原始任务与前置结果避免背景缺失
- 写回约定：将 <PLAN>/<SUB_TASK_RESULTS>/<RESULT_SUMMARY> 写回父会话，便于后续模型使用
- 容错：单个子任务失败不阻断整体流程，按需记录并继续其他子任务；计划评估失败不影响主流程
...
关键点
- CodeAgent 直接继承 Agent，通过 `super().__init__()` 初始化父类，获得 Agent 的所有核心能力（模型交互、工具调用、会话管理等）。
...
- CodeAgent 继承 Agent 的所有核心能力：
  - 模型交互：通过 `self.model` 访问 LLM 模型
  - 工具调用：通过 `self._call_tools()` 调用工具
  - 会话管理：通过 `self.session` 管理对话历史
  - 事件系统：通过 `self.event_bus` 订阅和发布事件
...
- 用户确认机制：
  - 提交被拒绝时，根据 `is_confirm_before_apply_patch` 配置决定是否要求用户确认
...
说明
- 该防护机制仅在非交互模式下生效，交互模式下仍使用原有的用户确认机制（`confirm_large_code_deletion()`）。
...
loop 对每个文件
  loop 对每个批次
    ...
    Cluster --> Entry : <CLUSTERS> YAML (含is_invalid字段)
    alt is_invalid=true
      Entry -> Entry : 跳过无效聚类，不加入cluster_batches
    else is_invalid=false
      Entry -> Entry : 加入cluster_batches，写 cluster_report.jsonl
    end
  end
end
...
loop 对每个cluster_batch
  ...
  Analyze --> Entry : <REPORT> YAML (has_risk判断)
  alt has_risk=true
    Entry -> Verify : 构造验证任务(二次验证)
    Verify --> Entry : <REPORT> YAML (is_valid判断)
    alt is_valid=true
      Entry -> Entry : 追加 agent_issues.jsonl
    else is_valid=false
...
- 断点恢复：
  - 支持断点恢复：若 cluster_report.jsonl 存在，优先复用已有聚类结果。如果断点记录中 is_invalid 为 true，且包含 invalid_reason，可以复用；如果缺少 invalid_reason，需要重新聚类。
- 写入聚类快照：
...
##### 5.2.1 阶段2.5：复核（__init__ 中的复核逻辑）
...
- 二次验证机制：
  ...
  - 验证Agent摘要输出格式：仅在 <REPORT>...</REPORT> 内输出 YAML 数组，每个元素：
    - gid: int（全局编号）
...
- **run**：一键流水线（collect → scan → lib-replace → prepare → transpile → optimize）
  - 参数：`--files <hdrs...>`（可选，提供则先执行 collect），`-o/--out <path>`（collect 输出文件，未提供时默认为 `.jarvis/c2rust/roots.txt`），`--root-list-syms <str>`（未提供 `--files` 时必填），`--disabled-libs <str>`（可选），`-g/--llm-group <name>`（LLM 相关阶段使用的模型组），`-m/--max-retries <N>`（transpile 构建/修复最大重试次数），`--resume/--no-resume`（transpile 断点续跑，默认启用）
...
- 复用 scanner 的工具函数（`_try_import_libclang`、`find_compile_commands`、`load_compile_commands`、`scan_file`），确保与扫描器行为一致。

关键接口
- `collect_function_names(files, out_path, compile_commands_root?)`：主入口，返回输出文件路径
...
start
:过滤头文件（.h/.hh/.hpp/.hxx），跳过非头文件;
:准备 libclang（版本 16-21）;
:为每个文件查找 compile_commands.json（全局或局部）;
:为每个文件准备解析参数：
  - 从 compile_commands 获取 args（若存在）
  - 推断语言标志（-x c-header / -x c++-header）
  - 确保包含 -I <file.parent>;
:尝试 scan_file（收集定义）;
if (scan_file 失败?) then (是)
  :回退为 _collect_decl_function_names（收集声明）;
endif
:提取函数名（优先 qualified_name，回退为 name）;
:去重（保持首次出现顺序）;
:写入 out_path（每行一个，UTF-8 编码）;
stop
@enduml
```
...
- 上下文与复用：
  - 构建当前函数的上下文头部（全量/精简），在复用代码生成与修复Agent/Review等Agent时拼接（_reset_function_context/_compose_prompt_with_context/_refresh_compact_context）
- 代码生成与修复（CodeAgent，统一复用）：
  - 在单个函数生命周期内复用同一个CodeAgent实例，同时用于代码生成和修复任务
  - 代码生成：在目标模块生成或更新实现，遵循"最小变更、禁止 todo!/unimplemented!、必要时补齐依赖实现"的约束（_codeagent_generate_impl）
  - 强制使用记忆功能：代码生成Agent启用 force_save_memory=True，要求在完成函数实现后使用 save_memory 工具记录关键信息（函数名称、源文件位置、目标模块、Rust函数签名、核心功能与语义、关键实现细节、依赖关系、类型转换要点、错误处理策略、实际实现的Rust代码要点等）
...
- 构建校验与最小修复（核心闭环）：
  - cargo check → 若失败：错误分类（missing_import/type_mismatch/visibility/borrow_checker/dependency_missing/module_not_found），使用复用的代码生成Agent进行最小修复并继续循环（_classify_rust_error/_cargo_build_loop）
  - 通过 check 后执行 cargo test → 若失败：同样分类与最小修复，直到通过或达上限（check_max_retries 和 test_max_retries，0 表示无限重试）
  - 测试失败信息反馈：测试失败时使用 `cargo test -- --nocapture` 获取完整的测试失败信息（包括测试用例名称、断言失败位置、期望值与实际值、堆栈跟踪等），并通过专门的 `<TEST_FAILURE>` 标签传递给修复Agent，明确区分测试失败和编译错误
  - 每轮记录 check_attempts、test_attempts、impl_verified、last_build_error 等度量
- 审查与复核：
...
repeat
  ...
  :初始化上下文与复用的代码生成与修复Agent;
  :CodeAgent 生成/更新实现 (检查依赖、禁止 todo!/unimplemented!、保存记忆);
  :确保顶层 pub mod + 补齐 mod.rs 声明链;
  :构建闭环: cargo check → (失败? 代码生成Agent最小修复) → cargo test → (失败? 代码生成Agent最小修复，包含完整测试失败信息);
  if (构建通过?) then (是)
...
- 构建闭环：
  - 先 cargo check（更快），通过后再 cargo test；失败时使用复用的代码生成Agent按分类标签进行聚焦修复
  - check 阶段最多迭代 check_max_retries 次（0 表示无限重试），test 阶段最多迭代 test_max_retries 次（0 表示无限重试）
  - 测试失败时使用 `cargo test -- --nocapture` 获取完整测试失败信息（测试用例名称、断言失败位置、期望值与实际值、堆栈跟踪等），通过 `<TEST_FAILURE>` 标签传递给修复Agent，明确区分测试失败和编译错误
...
- `enable_*`：四类优化步骤开关（unsafe/structure/visibility/doc），均为布尔值，默认全部启用
...
- **断点续跑**：使用 `optimize_progress.json` 记录 `processed` 文件列表（posix 相对路径），支持跳过已处理文件
- **报告输出**：优化结果写入 `optimize_report.json`，包含统计摘要（files_scanned/unsafe_removed/unsafe_annotated/duplicates_tagged/visibility_downgraded/docs_added/cargo_checks/errors）
##### 5.7 CLI（命令行协调者）
...
4. **验证闭环**：每次改动后执行 `cargo check/test`；optimizer 自动尝试移除 unsafe 并回退失败，形成自动化收敛
...
本次扫描聚合报告检出并确认 **11 个安全问题**，覆盖多个典型类别与文件，体现了系统的覆盖度与有效性。

**问题分布统计：**
- **按类别**：
  - `unsafe_api`: 5 个（主要为字符串拼接/拷贝导致的溢出风险）
  - `memory_mgmt`: 3 个（use-after-free 可疑、空指针解引用）
  - `error_handling`: 1 个（I/O 写入未检查返回值）
  - `unsafe_usage`: 2 个（格式化字符串可控导致的风险）
- **按风险文件**：
  - `bzip2.c`
  - `bzlib.c`
  - `bzip2recover.c`
...
##### 1. 不安全 API 使用与缓冲区溢出（5 个）

- `bzip2recover.c:482`（`sprintf`）
  - 使用 `sprintf` 写入至 `split`，在剩余空间不足时可能溢出；建议改为 `snprintf` 并严格校验写入长度。
- `bzip2.c:1136`（`strcat`）
  - `mapSuffix` 直接拼接新后缀，未检查剩余空间；建议改为 `strncat` 并做长度边界检查。
- `bzip2.c:1163`（`strcat`）
  - `compress` 流程在文件名近上限时追加固定后缀，存在越界风险；建议在追加前验证可用空间或使用安全拼接策略。
- `bzip2.c:1351`（`strcat`）
  - `uncompress` 在不可识别后缀时追加固定后缀，可能越界；建议在多次映射后重新评估剩余空间。
- `bzip2.c:1744`（`strcpy`）
  - `snocString` 对动态分配的目标执行不受限 `strcpy`，存在整数溢出与堆溢出风险；建议使用 `strncpy` 并校验分配大小。

（上述 5 项均评为 high 严重性，置信度多为 0.9 或接近。）

##### 2. 内存管理问题（3 个）

- `bzlib.c:954`、`bzlib.c:1132`（use-after-free 可疑）
  - 在 `BZ2_bzWriteOpen`/`BZ2_bzReadOpen` 初始化失败路径中，`BZ_SETERR(ret)` 与 `free(bzf)` 顺序不当可能访问已释放内存；建议调整顺序为先设置错误后释放，或在宏中判空。
- `bzlib.c:1564`（可能的空指针解引用）
  - `BZ2_bzerror` 中对 `b`/`errnum` 的访问在外部传入 NULL 时可能触发段错误；建议在函数入口添加判空并返回参数错误码。

（两项 use-after-free 可疑与一项空指针问题评为 high 严重性，置信度约 0.6-0.65。）

##### 3. 错误处理缺失（1 个）

- `dlltest.c:138`（I/O 写入未校验）
  - `fwrite` 返回值未检查，写入失败可能被误判为成功；建议检查返回字节数并在不一致时进行错误处理。

##### 4. 不安全用法（2 个）

- `bzip2.c:1638`、`bzip2.c:1682`（格式化字符串可控）
  - 在 `usage`/`redundant` 相关路径中，用户可控内容进入 `fprintf` 的格式串，存在格式化字符串攻击风险；建议改为固定格式或使用 `fputs` 并避免将外部输入作为格式字符串。

#### 系统分析能力体现

1. **精准的启发式规则**：系统能够识别 C/C++ 代码中的不安全 API 使用（如 `strcpy`、`strcat`、`sprintf`）、内存管理问题（use-after-free 可疑、空指针解引用）以及错误处理缺失与格式化字符串风险等常见安全模式。
...
4. **上下文感知的分析**：系统能够理解代码逻辑，例如：
   - 识别 `strcpy` + `strcat` 的连续操作可能导致缓冲区溢出
   - 分析函数调用链，追踪从 API 入口到问题点的完整路径
   - 评估前置条件的合理性，判断问题的可触发性

#### 结论

通过 bzip2 项目的最新分析实践，验证了 jarvis-sec 系统在以下方面的能力：

- **高风险识别能力**：本次共确认 11 个问题，其中 high 严重性 10 个，覆盖关键路径与多类缺陷。
- **全面性**：覆盖缓冲区溢出、内存管理、错误处理与格式化字符串等多类安全风险。
...
commonlibrary_c_utils 是 OpenHarmony 的基础工具库，提供了系统级的基础功能支持，包括内存管理、I/O 事件处理、线程管理、引用计数等核心组件。基于安全扫描结果，共检出 **41 个安全问题**（high 严重性 30 个，medium 严重性 11 个），主要分布在 `parcel.cpp`、`refbase.h`、`io_event_reactor.cpp` 等核心文件中。以下针对主要问题类别提出系统化的改进方案。
...
针对检测到的 16 个空指针解引用问题，建立分层的空指针防护机制：

- **参数验证层**：在公共 API 入口处统一添加参数空指针检查，特别是 `IOEventHandler` 的 `Start`、`Stop`、`Update` 方法，以及 `WeakRefCounter` 的构造函数，确保外部传入的指针参数在函数入口即被验证。

- **成员变量保护层**：对于可能被多线程访问或生命周期复杂的成员变量（如 `io_event_reactor.cpp` 中的 `backend_`、`refbase.cpp` 中的 `refCounter_`），采用智能指针或引用计数机制，在访问前统一进行空指针检查，避免在对象析构或重置后访问无效指针。

- **链表遍历安全机制**：在 `DoClean` 等链表操作函数中，建立安全的遍历模式，在访问节点的 `prev_`、`next_` 指针前先验证节点有效性，避免在链表结构异常时解引用空指针。

- **内存分配失败处理**：对于 `MakeSptr` 等使用 `new` 操作符的函数，建立内存分配失败的处理流程，在分配失败时返回错误状态或抛出异常，避免对 `nullptr` 进行后续操作。

**资源泄漏防护：**

- **RAII 模式应用**：在 `CreatePThread` 等涉及资源分配的函数中，采用 RAII（Resource Acquisition Is Initialization）模式，使用智能指针或作用域守卫（scope guard）管理资源生命周期，确保在异常路径或失败路径中资源能够自动释放。

- **容器访问安全化**：对于 `SafeMap::ReadVal` 等容器访问函数，使用 `find` 方法替代 `operator[]`，避免在查询不存在的键时自动插入元素，影响并发安全性和数据一致性。

**多线程安全增强：**

- **迭代器失效防护**：在 `CleanUp` 等涉及容器遍历的函数中，采用更严格的同步机制（如读写锁、互斥锁），或在遍历期间复制容器快照，避免在多线程环境下迭代器失效导致的内存访问违规。

###### 2. 错误处理机制改进方案（8 个问题）

**系统调用返回值检查规范：**

建立统一的系统调用错误处理规范，对所有可能失败的系统调用（如 `close`、`pthread_mutex_lock`、`fdopendir`）进行返回值检查：

- **资源关闭操作**：在析构函数和清理函数中，对 `close` 系统调用的返回值进行检查，失败时记录错误日志，并在可能的情况下进行重试或标记资源状态。

- **同步原语错误处理**：对 `pthread_mutex_lock` 等同步原语的返回值进行检查，失败时返回错误码或抛出异常，避免在锁获取失败时继续执行可能导致竞态条件的代码。

- **文件操作错误传播**：在 `GetDirFiles`、`ForceRemoveDirectory` 等文件操作函数中，建立错误传播机制，将 `fdopendir` 失败和 `close` 失败的错误信息向上层传播，而不是静默忽略。

**API 错误状态设计：**

- **返回值类型改进**：对于 `Parcel::Read` 等可能失败的读取操作，将返回值类型从直接返回数据改为返回 `std::optional<T>` 或 `Result<T, Error>` 类型，明确区分成功和失败状态，避免调用者误判操作结果。

- **Rust FFI 错误处理**：在 Rust 代码的 FFI 边界处，使用 `match` 表达式或 `?` 操作符正确处理 `CString::new` 等可能返回错误的操作，将错误转换为适当的错误码或异常，而不是使用 `expect` 强制解包导致程序 panic。

###### 3. 缓冲区溢出防护方案（3 个问题）

**字符串操作安全化：**

- **前置长度验证**：在使用 `strcpy_s`、`strncpy` 等字符串拷贝函数前，先验证源字符串长度是否超过目标缓冲区容量，对于超过的情况进行截断或返回错误，而不是依赖函数本身的边界保护。

- **动态缓冲区分配**：对于可能接收任意长度文件名的场景（如 `ForceRemoveDirectory`），考虑使用动态分配的缓冲区或 `std::string`，而不是固定大小的栈缓冲区，从根本上避免缓冲区溢出风险。

**容器边界检查机制：**

- **安全访问接口**：对于 `SortedVector` 等容器类，提供两种访问接口：`operator[]` 用于性能关键路径（假设调用者已确保索引有效），`at()` 方法用于需要边界检查的场景（抛出异常）。同时，在文档中明确说明各接口的使用场景和前置条件。

- **边界检查宏/函数**：建立统一的边界检查辅助函数或宏，在访问容器元素前进行范围验证，对于越界访问返回错误状态或抛出异常，避免未定义行为。

###### 4. 类型安全改进方案（7 个问题）

**const 正确性重构：**

- **参数设计优化**：对于需要修改对象状态的函数（如 `WriteRemoteObject`、`WriteParcelable`），将参数类型从 `const` 改为非 `const`，或提供函数重载版本，明确区分只读和可写场景，避免使用 `const_cast` 移除常量限定符。

- **类型声明调整**：对于需要传递给系统调用但被声明为 `const` 的参数（如 `mapped_file.cpp` 中的 `hint_`），根据实际使用场景调整类型声明，或创建非 const 副本用于系统调用，避免违反类型系统的 const 安全保证。

**类型转换安全化：**

- **对齐检查机制**：在 `Parcel::Read` 等涉及内存对齐的场景中，添加完整的对齐检查，确保在 ARM32 等对对齐要求严格的架构上，所有数据访问都满足对齐要求，避免硬件异常。

- **函数指针类型统一**：对于 `CreatePThread` 等涉及函数指针类型转换的场景，统一线程函数签名设计，使用适配器函数或统一的回调接口，避免多次 `reinterpret_cast` 导致参数类型不匹配。

- **模板类型约束**：对于 `wptr<T>::operator=(O* other)` 等模板函数，添加类型安全约束（如使用 `std::is_convertible<O*, T*>::value` 进行编译期检查），对于不兼容的类型使用 `static_cast` 而不是 `reinterpret_cast`，避免未定义行为。

**FFI 边界类型安全：**

- **输入验证机制**：在 Rust FFI 边界处，对传入的字符串参数进行验证，检查是否包含 null 字节等非法字符，在验证失败时返回明确的错误码，而不是使用 `expect` 导致程序崩溃。

##### 实施优先级建议

基于问题严重性和影响范围，建议按以下优先级实施改进：

1. **高优先级**（立即修复）：30 个 high 严重性问题，特别是空指针解引用和类型安全问题，这些可能导致程序崩溃或安全漏洞。

2. **中优先级**（近期修复）：11 个 medium 严重性问题，主要是错误处理缺失和缓冲区溢出风险，虽然不直接导致崩溃，但可能影响系统稳定性和安全性。

3. **长期优化**：建立代码审查规范和静态分析工具集成，在开发阶段预防类似问题的引入，并定期进行安全审计。

#### commonlibrary_rust_ylong_runtime

commonlibrary_rust_ylong_runtime 是 OpenHarmony 的 Rust 异步运行时库，提供了任务调度、IO 驱动、同步原语等核心功能。基于安全扫描结果，共检出 **160 个安全问题**（按类别：error_handling 75 个、unsafe_usage 76 个、concurrency 7 个、ffi 1 个），主要分布在 `ylong_runtime/src/sync/mpsc/unbounded/queue.rs`、`ylong_runtime/src/executor/blocking_pool.rs`、`ylong_runtime/src/io/read_buf.rs` 等核心文件中。以下针对主要问题类别提出系统化的改进方案。
...
针对检测到的 60+ 个 `unwrap()` 和 `expect()` 使用问题，建立分层的错误处理机制：

- **锁获取错误处理**：对于 `Mutex::lock()`、`RwLock::write()` 等可能返回 `PoisonError` 的同步原语，统一使用 `match` 表达式或 `?` 操作符处理错误，而不是直接 `unwrap()`。对于锁中毒的情况，使用 `PoisonError::into_inner()` 恢复锁状态，或记录错误日志后继续执行，避免程序直接 panic。涉及文件包括：
  - `ylong_io/src/sys/windows/selector.rs`（多处 lock().unwrap()）
  - `ylong_runtime/src/util/slab.rs`（slots.lock().unwrap()）
  - `ylong_runtime/src/executor/blocking_pool.rs`（shared.lock().unwrap()）
  - `ylong_runtime/src/executor/block_on.rs`（mutex.lock().unwrap()）

- **系统调用错误处理**：对于 `Poll::new()`、`Waker::new()`、`fs::canonicalize()` 等可能失败的系统调用，使用 `Result` 类型传播错误，而不是使用 `unwrap()` 导致程序崩溃。在关键路径（如初始化阶段）无法处理错误时，使用 `expect()` 提供有意义的错误信息，便于调试和问题定位。涉及文件包括：
  - `ylong_ffrt/build.rs`（fs::canonicalize().unwrap()）
  - `ylong_io/src/waker.rs`（Poll::new().unwrap()、Waker::new().unwrap()）
  - `ylong_signal/src/common.rs`（Signal::deregister_action().unwrap()）

- **容器访问安全化**：对于 `VecDeque::pop_front()`、`Option::unwrap()` 等可能返回 `None` 的操作，使用 `while let Some(item) = queue.pop_front()` 模式或 `if let Some(item) = option` 模式，避免在空容器上调用 `unwrap()`。涉及文件包括：
  - `ylong_runtime/src/executor/queue.rs`（pop_front().unwrap()）
  - `ylong_runtime/src/executor/blocking_pool.rs`（pop_front().unwrap()）

- **Arc 引用计数检查**：对于 `Arc::try_unwrap()` 等可能失败的操作，使用 `match` 或 `if let` 处理多个引用的情况，而不是使用 `expect()` 强制解包。涉及文件包括：
  - `ylong_runtime/src/fs/async_file.rs`（Arc::try_unwrap().expect()）

**panic/unreachable 替换策略：**

针对检测到的 10+ 个 `panic!()` 和 `unreachable!()` 使用问题，建立优雅的错误处理机制：

- **状态机错误处理**：对于任务状态机、文件状态等状态转换场景，使用 `Result<T, StateError>` 类型返回错误，而不是使用 `unreachable!()` 或 `panic!()`。在状态不匹配时返回明确的错误信息，允许调用者进行恢复或重试。涉及文件包括：
  - `ylong_runtime/src/task/raw.rs`（状态机 panic）
  - `ylong_runtime/src/fs/async_file.rs`（FileState unreachable!()）

- **系统资源错误处理**：对于线程创建失败、IO 驱动初始化失败等系统资源问题，实现优雅降级策略，记录错误日志后返回错误状态，而不是直接 panic。涉及文件包括：
  - `ylong_runtime/src/executor/blocking_pool.rs`（thread::spawn 失败 panic）
  - `ylong_runtime/src/net/driver.rs`（Poll::new() 失败 panic）
  - `ylong_runtime/src/executor/driver.rs`（epoll_wait 失败 panic）

- **条件变量等待错误处理**：对于 `Condvar::wait()`、`Condvar::wait_timeout()` 等可能返回 `PoisonError` 的操作，使用错误处理机制，而不是直接 `unwrap()`。涉及文件包括：
  - `ylong_runtime/src/executor/blocking_pool.rs`（wait_timeout().unwrap()）
  - `ylong_runtime/src/executor/block_on.rs`（condvar.wait().unwrap()）

**忽略结果处理规范：**

- **FFI 调用结果检查**：对于 `from_overlapped()` 等 FFI 函数调用，检查返回值并记录错误日志，而不是使用 `let _ = ...` 忽略错误。涉及文件包括：
  - `ylong_io/src/sys/windows/selector.rs`（from_overlapped 结果被忽略）

- **系统调用返回值验证**：对于 `raw_os_error()` 等可能返回 `None` 的方法，使用 `unwrap_or_default()` 或模式匹配处理，而不是直接 `unwrap()`。涉及文件包括：
  - `ylong_io/src/sys/windows/selector.rs`（raw_os_error().unwrap()）

###### 2. 不安全代码使用改进方案（76 个问题）

**原始指针安全化：**

针对检测到的 20+ 个原始指针使用问题，建立安全的指针管理机制：

- **空指针检查机制**：在所有解引用原始指针前，添加空指针检查，使用 `NonNull` 类型包装指针，或使用 `if ptr.is_null() { return error; }` 模式。涉及文件包括：
  - `ylong_runtime/src/task/task_handle.rs`（task.get() 解引用前未检查）
  - `ylong_runtime/src/ffrt/ffrt_timer.rs`（ffrt_timer_start 返回 null 未检查）
  - `ylong_runtime/src/ffrt/spawner.rs`（exec_future 中 data 指针未检查）

- **指针生命周期管理**：对于传递给 FFI 回调的原始指针，使用 `Arc` 或 `Pin` 确保指针在回调期间保持有效，避免 use-after-free。涉及文件包括：
  - `ylong_runtime/src/ffrt/ffrt_timer.rs`（timer_wake_hook 中的 waker 指针）
  - `ylong_runtime/src/ffrt/spawner.rs`（exec_drop 中的 data 指针）

- **栈变量指针传递防护**：对于传递给 FFI 的栈变量指针，确保在 FFI 调用期间变量保持有效，或使用堆分配确保生命周期。涉及文件包括：
  - `ylong_runtime/src/ffrt/spawner.rs`（FfrtTaskAttr 栈变量传递给 FFI）

**MaybeUninit 安全初始化：**

针对检测到的 10+ 个 `MaybeUninit` 使用问题，建立安全的未初始化内存处理机制：

- **显式初始化替代 assume_init**：对于数组和结构体的初始化，使用显式循环初始化或 `array::from_fn`，而不是使用 `MaybeUninit::zeroed().assume_init()`。涉及文件包括：
  - `ylong_runtime/src/sync/mpsc/unbounded/queue.rs`（Block::new 中的数组初始化）
  - `ylong_runtime/src/time/wheel.rs`（Level::new 中的数组初始化）

- **边界检查机制**：在使用 `assume_init()` 前，添加边界检查，确保初始化的长度不超过缓冲区容量。涉及文件包括：
  - `ylong_runtime/src/io/read_task.rs`（assume_init(len) 前未检查边界）
  - `ylong_runtime/src/io/read_buf.rs`（多处 assume_init 使用）

**类型转换安全化：**

针对检测到的 15+ 个不安全的类型转换问题，建立安全的类型转换机制：

- **对齐检查机制**：在进行指针类型转换前，验证指针对齐要求，使用 `align_of` 检查指针与目标类型的对齐是否匹配。涉及文件包括：
  - `ylong_io/src/sys/windows/selector.rs`（overlapped 指针转换未检查对齐）

- **所有权转移文档化**：对于 `from_raw_fd`、`from_raw_socket` 等涉及所有权转移的 unsafe 函数，添加详细的文档说明所有权语义，确保调用者理解所有权转移的契约。涉及文件包括：
  - `ylong_io/src/sys/unix/tcp/socket.rs`（from_raw_fd 所有权转移）
  - `ylong_io/src/sys/unix/uds/socket.rs`（from_raw_fd 所有权转移）

- **双重所有权避免**：在调用 `from_raw_fd` 等转移所有权的函数后，避免继续使用原始文件描述符，或使用 `as_raw_fd()` 方法获取描述符。涉及文件包括：
  - `ylong_io/src/sys/unix/tcp/socket.rs`（from_raw_fd 后继续使用原 fd）
  - `ylong_io/src/sys/unix/tcp/listener.rs`（from_raw_fd 后继续使用原 fd）

**Arc 引用计数管理：**

针对检测到的 5+ 个 `Arc::from_raw` 使用问题，建立正确的引用计数管理机制：

- **配对使用规范**：确保 `Arc::into_raw` 和 `Arc::from_raw` 配对使用，在 `from_raw` 后正确管理引用计数，避免内存泄漏或双重释放。涉及文件包括：
  - `ylong_runtime/src/executor/current_thread.rs`（RawWaker 中的 Arc 管理）
  - `ylong_runtime/src/executor/block_on.rs`（RawWaker 中的 Arc 管理）
  - `ylong_runtime/src/util/slab.rs`（Arc::from_raw 使用）

- **引用计数平衡**：在 `RawWaker` 的 `clone`、`wake`、`wake_by_ref`、`drop` 函数中，确保 Arc 引用计数的正确增减，避免使用 `mem::forget` 导致引用计数失衡。涉及文件包括：
  - `ylong_runtime/src/executor/current_thread.rs`（clone 中使用 mem::forget）
  - `ylong_runtime/src/executor/block_on.rs`（wake 中未平衡引用计数）

**文件描述符和句柄安全：**

- **有效性检查机制**：在调用 `closesocket`、`BorrowedFd::borrow_raw` 等操作前，验证文件描述符或句柄的有效性。涉及文件包括：
  - `ylong_io/src/sys/windows/tcp/socket.rs`（closesocket 前未检查）
  - `ylong_io/src/sys/unix/waker.rs`（eventfd 返回 -1 未检查）
  - `ylong_runtime/src/net/sys/unix/datagram.rs`（BorrowedFd::borrow_raw 前未检查）

- **错误返回值处理**：对于 `eventfd`、`epoll_create1` 等系统调用，检查返回值是否为 -1，在失败时返回错误而不是继续使用无效描述符。涉及文件包括：
  - `ylong_io/src/sys/unix/waker.rs`（eventfd 返回 -1 未检查）
...
针对检测到的 7 个 `unsafe impl Send` 或 `unsafe impl Sync` 问题，建立线程安全保证机制：

- **内部同步原语替换**：对于包含 `RefCell` 等非线程安全类型的结构体，将内部字段替换为 `Mutex` 或 `RwLock`，确保线程安全后再实现 `Send`/`Sync`。涉及文件包括：
  - `ylong_runtime/src/executor/worker.rs`（Worker 包含 RefCell 但实现 Send/Sync）
  - `ylong_runtime/src/sync/mpsc/unbounded/queue.rs`（Queue 包含 RefCell 但实现 Sync）

- **文档化线程安全保证**：对于确实需要 `unsafe impl Send/Sync` 的场景，添加详细的文档说明线程安全保证，包括哪些操作是线程安全的，哪些操作需要外部同步。涉及文件包括：
  - `ylong_runtime/src/executor/current_thread.rs`（CurrentThreadScheduler 实现 Sync）
  - `ylong_runtime/src/time/sleep.rs`（Sleep 实现 Send/Sync）
  - `ylong_io/src/sys/windows/io_status_block.rs`（IoStatusBlock 实现 Send）

- **原始指针同步机制**：对于包含原始指针的结构体，在实现 `Send`/`Sync` 前，确保指针访问有适当的同步机制（如原子操作、内存屏障）。涉及文件包括：
  - `ylong_runtime/src/time/sleep.rs`（Sleep 包含原始指针但实现 Send/Sync）
  - `ylong_runtime/src/net/schedule_io.rs`（Readiness 实现 Sync）

**数据竞争防护：**

- **内存排序保证**：在多线程环境下使用原子操作时，使用适当的内存排序（`Acquire`、`Release`、`AcqRel`、`SeqCst`），确保内存可见性和同步。涉及文件包括：
  - `ylong_runtime/src/sync/mpsc/unbounded/queue.rs`（原子操作的内存排序）

- **临界区保护**：对于需要原子性操作的代码段，使用锁或其他同步原语保护，避免数据竞争。涉及文件包括：
  - `ylong_runtime/src/executor/worker.rs`（get_inner_ptr 绕过 RefCell 检查）

###### 4. FFI 安全问题改进方案（1 个问题）

**C 字符串安全处理：**

- **字符串验证机制**：在使用 `CStr::from_ptr` 前，验证字符串是否以 null 结尾，检查字符串长度，避免读取越界。涉及文件包括：
  - `ylong_runtime/src/process/pty_process/sys.rs`（CStr::from_ptr 使用前未验证）

- **缓冲区生命周期管理**：确保传递给 `CStr::from_ptr` 的缓冲区在 `CStr` 使用期间保持有效，避免在转换后修改缓冲区。涉及文件包括：
  - `ylong_runtime/src/process/pty_process/sys.rs`（ptsname_r 后修改缓冲区）

##### 实施优先级建议

基于问题严重性和影响范围，建议按以下优先级实施改进：

1. **高优先级**（立即修复）：76 个 unsafe_usage 问题中的 high 严重性问题（约 40+ 个），特别是：
   - 空指针解引用风险（task_handle.rs、ffrt_timer.rs、spawner.rs）
   - 类型转换安全问题（read_buf.rs、selector.rs、tcp/socket.rs）
   - Arc 引用计数管理错误（current_thread.rs、block_on.rs）
   - 文件描述符有效性检查缺失（waker.rs、tcp/socket.rs）

2. **中优先级**（近期修复）：75 个 error_handling 问题，主要是：
   - 锁获取错误处理（selector.rs、slab.rs、blocking_pool.rs）
   - 系统调用错误处理（build.rs、waker.rs、driver.rs）
   - panic/unreachable 替换（task/raw.rs、executor/blocking_pool.rs）

3. **并发安全优化**（中期修复）：7 个 concurrency 问题，包括：
   - Send/Sync 实现安全化（worker.rs、queue.rs、current_thread.rs）
   - 数据竞争防护（queue.rs、worker.rs）
