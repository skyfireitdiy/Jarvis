...
- 主要源码模块：
  - cli.py（Typer 命令行入口）
  - __init__.py（主工作流：run_security_analysis，协调各阶段执行）
...
3. **模块规划测试**
   ...
   - `test_module_agent_yaml_output`：测试 YAML 输出
...
- **Then**：
  - 所有阶段按顺序执行完成
  - 生成 `heuristic_issues.jsonl`（阶段1产物）
  - 生成 `cluster_report.jsonl`（阶段2产物）
  - 生成 `agent_issues.jsonl`（阶段3产物）
...
- **Given**：已执行部分阶段的测试项目（存在 `.jarvis/sec/heuristic_issues.jsonl` 和 `progress.jsonl`）
- **When**：重新执行 `jarvis-sec agent --path <test_project>`
- **Then**：
  - 系统从断点恢复，跳过已完成的阶段
  - 已处理的候选问题不被重复处理
  - 新发现的问题被正确追加
