...
- **源码位置**：`src/jarvis/jarvis_c2rust/`
  ...
  - `optimizer.py`：Rust 代码保守优化器（所有优化步骤均使用 CodeAgent 完成，包括 clippy 告警消除、unsafe 清理、结构优化、可见性优化、文档补充）
...
- **核心数据目录与产物**（默认路径 `<project_root>/.jarvis/c2rust/`）：
  ...
  - **进度与映射**：
    ...
    - `optimize_progress.json`：优化阶段的进度记录（processed 文件列表、steps_completed 步骤列表、step_commits 每个步骤的 commit id、last_commit 最后一步的 commit id）
...
- **run**：一键流水线（scan → lib-replace → prepare → transpile → optimize），支持断点续跑
  ...
  - 阶段说明：
    ...
    - **optimize**：对生成的 crate 执行保守优化（所有优化步骤均使用 CodeAgent 完成，包括 clippy 告警消除、unsafe 清理、结构优化、可见性优化、文档补充），采用默认优化配置，自动检测 crate 根目录；优化前执行 `cargo clippy --fix` 自动修复，每个修复前执行 `cargo fmt` 格式化代码
...
CLI -> Optimizer : 保守优化（如未完成，clippy告警消除/unsafe/结构/可见性/文档 + cargo checks）
...
UnsafeCodeAgent -> UnsafeCodeAgent: 创建CodeAgent实例\n执行 cargo fmt
...
Optimizer -> StructureCodeAgent: _codeagent_opt_structure_duplicates()
StructureCodeAgent -> StructureCodeAgent: 创建CodeAgent实例\n执行 cargo fmt
StructureCodeAgent -> StructureCodeAgent: CodeAgent.run()\n结构优化
Optimizer -> Optimizer: cargo test验证
alt 失败
    Optimizer -> FixCodeAgent: _build_fix_loop()
end
...
VisibilityCodeAgent -> VisibilityCodeAgent: 创建CodeAgent实例\n执行 cargo fmt
...
DocCodeAgent -> DocCodeAgent: 创建CodeAgent实例\n执行 cargo fmt
...
5. **阶段5 - 代码优化**：
   - 优化前执行 `cargo clippy --fix` 自动修复
   - 检查 clippy 告警，如果有则使用 CodeAgent 消除告警（如果没有告警则跳过）
   - 所有优化步骤均使用 CodeAgent 完成：
     - CodeAgent 消除 clippy 告警（如有）
     - CodeAgent 进行 unsafe 清理
     - CodeAgent 进行结构优化（重复代码检测与消除）
     - CodeAgent 进行可见性优化
     - CodeAgent 进行文档补充
   - 每个 CodeAgent 调用前执行 `cargo fmt` 格式化代码
   - 每个步骤后执行 `cargo test` 验证，失败时使用 CodeAgent 进行最小修复
...
component "做快照\n(可回退)" as Snapshot
component "Clippy自动修复\n(cargo clippy --fix)" as ClippyFix
component "Clippy告警检查\n(检查是否有告警)" as ClippyCheck
component "CodeAgent消除告警\n(如有告警)" as ClippyCodeAgent
component "CodeAgent unsafe清理" as UnsafeCodeAgent
component "CodeAgent结构优化" as StructureCodeAgent
...
Snapshot --> StructureCodeAgent
StructureCodeAgent --> Fmt
Fmt --> StructureCodeAgent
StructureCodeAgent --> Tester
Tester --> FixCodeAgent
FixCodeAgent --> Tester
...
- `enable_*`：四类优化步骤开关（unsafe/structure/visibility/doc），均为布尔值，默认全部启用；所有优化步骤均使用 CodeAgent 完成
...
- **断点续跑**：使用 `optimize_progress.json` 记录 `processed` 文件列表（posix 相对路径）、`steps_completed` 步骤列表、`step_commits` 每个步骤的 commit id，支持跳过已处理文件；恢复时自动检测代码状态，如果不一致则 reset 到最后一个已完成步骤的 commit id，确保代码状态与进度记录一致
...
- **优化前预处理**：优化前执行 `cargo clippy --fix` 自动修复；检查 clippy 告警，如果有则使用 CodeAgent 消除告警（如果没有告警则跳过）
- **代码格式化**：每个 CodeAgent 修复前执行 `cargo fmt` 格式化代码，确保代码格式一致
- **所有优化使用 CodeAgent**：所有优化步骤（unsafe 清理、结构优化、可见性优化、文档补充）均使用 CodeAgent 完成，确保智能、准确且可回退
...
- **保守优化（optimizer.py.optimize_project）**
  - 目标：在保持可构建前提下"最小"提升质量（clippy 告警消除、unsafe 清理、重复代码检测与消除、可见性缩小、文档补齐），失败可回滚
  - 技术要点：
    - 所有优化步骤均使用 CodeAgent 完成，确保智能、准确且可回退
    - 优化前执行 `cargo clippy --fix` 自动修复，检查 clippy 告警并使用 CodeAgent 消除（如果没有告警则跳过）
    - 每个 CodeAgent 修复前执行 `cargo fmt` 格式化代码
    - 验证统一使用 `cargo test -q`（而非 `cargo check`）
    - 支持 `git_guard`（步骤前快照，失败时 `git reset --hard`）
    - 支持断点续跑（`optimize_progress.json` 记录 `processed` 文件列表、`steps_completed` 步骤列表、`step_commits` 每个步骤的 commit id，恢复时自动 reset 到对应的 commit id）
...
- **optimizer（优化阶段）**
  ...
  - **unsafe 最小化流程（CodeAgent）**：
    ...
    - 失败则使用 CodeAgent 进行最小修复，并在该块或相邻函数前添加 `/// SAFETY: ... 原因摘要` 注释
    - 记录统计：`unsafe_removed`（成功移除数）和 `unsafe_annotated`（失败回滚并补充注释数）
  - **所有优化步骤均使用 CodeAgent**：unsafe 清理、结构优化、可见性优化、文档补充均使用 CodeAgent 完成，每个步骤修复前执行 `cargo fmt` 格式化代码
