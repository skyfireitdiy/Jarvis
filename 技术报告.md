...
- 安全约束：edit_file 强制唯一匹配（SEARCH 仅一次命中）；区间替换需合法起止标记；rewrite_file 采用原子写并可回滚
- 选择策略：优先 edit_file（影响最小），仅在大范围重构或整文件生成时使用 rewrite_file
...
- 代码读取：read_code
  ...
  - 功能特性：
    ...
    - 块 ID 生成：为每个代码块生成唯一标识符（如 `function_name`、`ClassName.method_name`）
    - 边界处理：返回与请求范围有重叠的完整语法单元，确保语义完整性
    - Token 限制：自动估算输出 token 数，超过限制时提示分批读取
  - 最佳实践：遵循"先读后写"，优先按范围读取（start_line/end_line）以避免上下文膨胀；使用结构化块 ID 进行编辑，避免手动计算行号。
...
- 文件修改与重写：PATCH / REWRITE
  - PATCH（推荐）：进行最小必要变更，支持
    ...
    - 结构化编辑：STRUCTURED（通过块id进行编辑，推荐使用）
...
- 结构化编辑（Structured Editing）
  - 概述：CodeAgent 提供基于代码结构块的精确编辑能力，通过 `read_code` 工具获取结构化块 ID，使用 `edit_file` 工具的 `structured` 模式进行编辑。
  - 核心优势：
  ...
  - 结构化编辑（edit_file structured 模式）：
    ...
    - 定位机制：通过 `StructuredCodeExtractor.find_block_by_id()` 定位代码块
      - 优先在语法单元中查找
      - 若未找到，尝试在空白行分组中查找
      - 支持作用域前缀（如 `ClassName.method_name`）
    - 行号处理：自动处理行号变化，确保编辑后文件结构正确
    - 换行符处理：自动保持原有换行符风格，确保代码格式一致
  - 使用示例：
    - 读取文件获取块 ID：
  ...
  - 实现位置：
    ...
    - 编辑工具：`jarvis_tools/edit_file.py::EditFileTool._apply_structured_edit()`
...
4. **代码编辑**
   ...
   - `test_code_agent_patch_search_replace`：测试 SEARCH/REPLACE 补丁
   - `test_code_agent_patch_interval_replace`：测试区间替换补丁
...
1. **edit_file 测试**
   - `test_edit_file_search_replace`：测试 SEARCH/REPLACE 编辑
   - `test_edit_file_interval_replace`：测试区间替换
