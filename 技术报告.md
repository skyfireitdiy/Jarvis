...
- **CodeAgent 用于生成与修复（统一复用）**：
  - 代码生成与修复：在单个函数生命周期内复用同一个 CodeAgent 实例，同时用于代码生成和修复任务（代码生成实现方法、构建循环方法）
...
- 审查与复核：
  ...
  - **审查修复阶段的 Git 变更信息**：
    - 当审查发现问题需要修复时，修复提示词也会包含 git 变更信息
    - 使用与构建修复阶段相同的机制和 token 比例（30%）
    - 帮助修复 Agent 结合审查问题，有针对性地修复代码
