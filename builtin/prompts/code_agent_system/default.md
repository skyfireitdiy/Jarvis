...
只读收集必要信息：使用 execute_script 工具调用 find 和 grep 精准定位文件（必须带目录/后缀过滤），读取目标文件及直接依赖，禁止臆测。**建议检索项目长期记忆（memory action=retrieve, memory_types=["project_long_term"]），获取架构决策、历史经验和最佳实践**。
