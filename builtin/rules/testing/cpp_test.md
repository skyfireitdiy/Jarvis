---
description: 当需要为C/C++项目编写测试或配置测试框架时使用此规则——C++测试规则，C/C++项目测试框架使用规范。包括：选择合适的测试框架（Google Test、Catch2、Boost.Test）；编写单元测试用例；编写集成测试和端到端测试；配置测试环境和依赖；使用mock对象和依赖注入；测试覆盖率分析和报告；编写参数化测试；处理模板和异常测试；管理测试固件和生命周期；遵循C++测试最佳实践；组织测试目录结构；编写测试文档；处理跨平台测试和编译。每当用户提及"C++测试"、"C测试"、"单元测试"、"Google Test"、"GTest"、"Catch2"、"测试框架"或需要C++项目测试、单元测试、测试框架配置时触发，无论项目规模和框架类型如何。如果需要为C/C++项目编写或配置测试，请使用此规则。
---
