---
name: sdd
description: 当需要先编写规范再实现代码的开发流程时使用此规则——Spec驱动开发规则，先编写规范再实现代码的开发流程。包括：编写详细的规范文档；进行需求分析和澄清；设计API接口和数据结构；编写测试规范和验收标准；实现规范的代码；验证实现是否符合规范；进行代码审查和迭代改进；保持规范和代码的一致性。每当用户提及"Spec驱动"、"规范先行"、"需求分析"、"规范编写"、"SDD"或需要规范驱动开发、需求分析、代码实现时触发，无论编程语言和项目类型如何。如果需要按照先规范后代码的流程进行开发，请使用此规则。
---
...
- **必须**：Spec 文件应放置在项目的 `.jarvis/rules/spec/` 目录
...
0. **YAML 头部（必须）**
   - **必须**：Spec 文件顶部包含 YAML front matter header
   - 格式如下：

   ```yaml
   ---
   name: spec名称 # 必需，spec的简短标识
   description: spec功能描述 # 必需，spec的详细说明
   ---
   ```

   - `name`：spec的名称标识，使用小写字母和下划线
   - `description`：spec的功能描述，清晰说明设计目标
...
- [ ] Spec 文件包含 YAML 头部（name 和 description 字段）
- [ ] Spec 包含功能概述和目标
- [ ] Spec 包含清晰的接口定义
- [ ] Spec 包含完整的输入输出说明
- [ ] Spec 包含边界条件和异常处理
