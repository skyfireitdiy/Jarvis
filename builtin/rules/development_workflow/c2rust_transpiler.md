...
description: 当需要将C/C++代码转换为Rust代码时使用此规则——C/C++到Rust转译器规则，指导C/C++代码到Rust代码的转译过程。包括：分析C/C++代码的结构和依赖；映射C/C++类型到Rust类型；转译C/C++语法到Rust语法；处理C/C++指针和内存管理；转换C/C++宏和预处理器；实现C++类到Rust结构体和trait；处理C/C++特有的并发和线程机制；确保内存安全和所有权规则；处理C/C++标准库转换为Rust等价物；验证转译后代码的功能一致性；优化转译后的Rust代码性能；处理FFI和跨语言调用场景。每当用户提及"C到Rust"、"C++到Rust"、"C转Rust"、"C++转Rust"、"代码转译"、"语言迁移"、"C迁移"、"C++迁移"、"跨语言开发"或需要C/C++代码转译、Rust迁移、跨语言开发时触发，无论项目规模和复杂度如何。如果需要将C/C++代码转换为Rust代码，请使用此规则。
---
