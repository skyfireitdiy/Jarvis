...
用于指导 Go 到 Rust 的代码转译过程，保转译质量、功能一致性与类型安全。本规则涵盖从规、现、构、审、优至评的完整转译流程。
...
- **必须**：每个函数的转译过程都要用 `task_list_manager` 来管理任务
- **必须**：在开始转译前建任务列表，规全任务（规、现、构、审、优、评）
- **必须**：用 task_list_manager 的 `add_tasks` 操作加全子任务
- **必须**：按依赖关系设任务依赖（如：实现阶段依赖规划阶段）
- **禁止**：跳过 task_list_manager，直接行任务

**多层级任务列表要求：**

Go 到 Rust 转译是复杂流程，需据转译规模建立不同层级的任务列表：
...
2. **文件级任务列表**（按需）
   ...
   - **子任务**：含该文件中全函数/结构体的转译任务
   - **示例场景**：
     - 转译 `hash.go` 文件中的所有哈希相关函数与结构体
     - 转译 `http_handler.go` 文件中的 HTTP 处理器结构体

3. **函数/结构体级任务列表**（必）
   - **何时需要**：转译单个函数或结构体时（这是最小粒度，必须建立）
   - **任务内容**：规函数/结构体签名、实现策略、测试用例等
   - **子任务**：含该函数/结构体的规、现、构、审、优、评
   - **示例场景**：
...
- 任务描述：每任务应包含约束条件、应要求、禁事项、验标准
- 预期输出：必用结构化格式列出预期输出
- 验方法：任务完成后必供验方法说明
- **Go 代码位置信息**：**必须**在每子任务中含原始 Go 代码的行号位置信息
  - 函数/结构体级任务：应包含 Go 函数/结构体所在的文件路径与行号范围
    - Go：`path/to/file.go:42-67`
  - 文件级任务：应包含 Go 文件的路径（`.go`）
  - 目录级任务：应包含 Go 目录的路径（如：`path/to/dir/`）
...
- **必须**：先写测试（#[cfg(test)] mod tests），基于 Go 函数/结构体行为设测试用例
- **必须**：编写实现使测试通过，保与 Go 语义等价
- **必须**：优代码，保持测试通过（重构阶段）
- **禁止**：在没有测试的情况下直接编现代码
- **禁止**：用 `todo!` 或 `unimplemented!` 作为占位符
...
- **必须**：核心功能定义与 Go 现一致（主要目的和预期行为）
- **允许**：安全改进导致的行为不一致（修 nil 指针解引用、数据竞争等安全漏洞）
- **略**：语言差异导致的行为不一致（panic 恢复、goroutine 调度等）
- **允许**：用不同的类型设、错误处理方式、资源管方式
- **禁止**：改变核心功能逻辑

**说明：** 核心功能指函数/结构体的主要目的（如"计算哈希值"、"解析字符串"、"管 HTTP 连接"），不包括实现细节。

**Go 特有虑：**

- 结构体的封装性：保持公共接口一致（Go 首字母大写导出），内部实现可以不同
- 接口关系：保持接口定义和现方法一致
- 并发模型：保持并发行为一致，但可以用不同的并发原语
- 错误处理：Go 的多返回值（value, error）可以转为 Rust 的 Result<T, E>
- defer 语义：可以用 Rust 的 Drop trait 实现类似的资源清理

### 4. 模块化设原则

**要求说明：**

- **必须**：按功能内聚与依赖方向选模块，避免循环依赖
- **必须**：模块路径必位于 crate 的 src/ 目录下
- **必须**：优先将函数放置到已存在的模块中
- **允许**：必要时建新的子模块文件
- **禁止**：将不相关的功能放在同一模块
...
- Go 的 package（如 `package main`、`package utils`）映到 Rust 的 module
- Go 的 import 路径（如 `github.com/user/repo/pkg`）映到 Rust 的依赖管（Cargo.toml）
- Go 的内部包（internal）映到 Rust 的私有模块

### 5. 类型安全原则

**要求说明：**

- **必须**：优先用 Rust 原生类型（i32/u32、&str/String、&[T]/&mut [T]、Result<T,E>）
- **禁止**：优先用 Go 风格类型（空接口 interface{}、反射 reflect）
- **禁止**：用 extern "C"（除非是 FFI 导出要求）
- **必须**：函数签名应循 Rust 最佳实践，不需要兼容 Go 的数据类型
- **禁止**：用 `use ...::*` 通配导入
...
- **必须**：每个转译阶段都必生文档记
- **必须**：文档记应含阶段行过程、决策依据、遇到的问题和解决方案
- **必须**：文档记应含 Go 代码位置信息，便于追溯
- **必须**：文档记应用结构化格式（Markdown），便于阅读和维护
- **必须**：文档记应在阶段完成后立即生，保信息准确完整
- **禁止**：跳过文档记，直接进入下一阶段
...
- 文档名：用清晰的名规则（如：`function_name_planning.md`、`file_name_implementation.md`）
- 文档内容：应包含阶段概述、行过程、关键决策、问题与解决方案、验结果
- 文档链接：在任务列表的预期输出中含文档路径

## 你必须执行的操作作

**重要提示：** 在开始转译前，必据转译规模选合适的任务列表层级：

- **转译单个函数**：建函数级任务列表（必）
- **转译单个文件或多个相关函数**：建文件级任务列表，含多个函数级子任务（按需）
- **转译整个目录或多个相关目录**：建目录级任务列表，含多个文件级子任务（按需）

以下操作阶段适用于所有层级的任务列表，但具体行粒度取决于任务列表层级。
...
- **必须**：记原始 Go 代码的位置信息
  - 函数/结构体级：记 Go 函数/结构体所在的文件路径和行号范围
    - Go：`path/to/file.go:42-67`
  - 文件级：记 Go 文件的完整路径（`.go`）
  - 目录级：记 Go 目录的路径（如：`path/to/dir/`）
- **必须**：将位置信息含在任务描述、背景信息或预期输出中
- **必须**：保位置信息准确，便于后续追溯和验

#### 操作2：选模块位置

- 析函数的功能特性，确定所属的功能模块
- 查 crate 目录结构，选或建合适的模块文件
- 保模块路径位于 src/ 目录下
- 评是否需要建新的子模块文件
...
- 如果是根符号，必用 `pub` 关键字
- **必须**：在函数/结构体注释中记对应的 Go 代码位置信息（如：`/// 转译自 path/to/file.go:42-67`）

**Go 特有处理：**

- **结构体转结构体**：Go 结构体转为 Rust 结构体，字段保持相似名
- **方法转 impl**：Go 结构体方法转为 Rust impl 块中的方法
...
- **必须**：文档含以下内容：
  ...
  - 实现必要性评估结果
  - 关键设决策和虑因素
- **必须**：文档存到指定位置（如：`docs/transpilation/<function_name>_planning.md`）
- **必须**：在任务预期输出中含文档路径
...
- 测试用例必覆：
  ...
  - 边界情况（空输入、极值、边界值）
  - 错误情况（如果 Go 现有错误处理）
- 测试用例的预期结果必与 Go 现的行为一致
- 用清晰的测试名称和适当的断言

**Go 特有测试要求：**

- **结构体测试**：测试所有公共方法，包括构造函数、字段访问器、修改器
- **接口测试**：测试接口的全方法，包括不同的现类型
- **并发测试**：测试 goroutine 的并发行为和同步原语（WaitGroup、Mutex、Channel）
- **错误处理测试**：Go 的 error 返回值转为 Rust 的 Result，需要相应测试
...
- **方法实现**：Go 结构体方法转为 Rust impl 块中的方法
...
- **必须**：文档含以下内容：
  ...
  - 重构说明和改进点
  - 代码变更摘要
- **必须**：文档存到指定位置（如：`docs/transpilation/<function_name>_implementation.md`）
- **必须**：在任务预期输出中含文档路径
...
- **必须**：文档含以下内容：
  ...
  - 修后的验结果
  - 构建时间统计（如适用）
- **必须**：文档存到指定位置（如：`docs/transpilation/<function_name>_build.md`）
- **必须**：在任务预期输出中含文档路径
...
- **必须**：文档含以下内容：
  ...
  - 修建议和代码示例
  - 审查结论和改进方向
- **必须**：文档存到指定位置（如：`docs/transpilation/<function_name>_review.md`）
- **必须**：在任务预期输出中含文档路径
...
- **必须**：文档含以下内容：
  ...
  - 回归测试结果
  - 优化效果总结
- **必须**：文档存到指定位置（如：`docs/transpilation/<function_name>_optimization.md`）
- **必须**：在任务预期输出中含文档路径
...
- **必须**：用子 agent（`agent_type: "sub"`）对转译结果进行独立评估
- **必须**：评估范围包括：
  - 功能对齐查：对比 Go 现和 Rust 现的核心功能是否一致
  - 测试覆查：测试用例是否充分覆 Go 函数/结构体的行为
  - 边界情况查：边界条件和错误处理是否与 Go 现一致
  - 性能影响评：Rust 现的性能是否满足要求（如适用）
...
- **必须**：生详细的评估报告，含：
  ...
  - 发现的问题列表（如有）
  - 改进建议（如有）
- **必须**：将评估报告存为文档（如：`docs/transpilation/<function_name>_evaluation.md`）
- **必须**：在任务预期输出中含文档路径

#### 操作2：处理功能未对齐问题

- **必须**：若评发现功能未对齐问题，必建优子任务
- **必须**：优子任务应含：
  - 问题描述：详细说明功能未对齐的具体表现
  - 对齐目标：明确需要对齐的功能点
  - 修计划：制定修方案和验方法
