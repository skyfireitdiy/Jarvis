---
description: 当需要生成PowerPoint演示文稿或制作幻灯片时使用此规则——PPT生成规则，生成演示文稿。包括：根据PPT设计文档生成.pptx文件；创建和编辑幻灯片内容；设置幻灯片布局和样式；添加文本、图片、图表等元素；设计配色方案和主题；应用模板和母版；设置动画和过渡效果；生成演讲者备注；导出和保存演示文稿；处理多语言内容；优化幻灯片性能和兼容性。每当用户提及"PPT生成"、"演示文稿制作"、"生成PPT"、"制作幻灯片"、"项目汇报"、"技术分享"或需要演示文稿制作、项目汇报、技术分享时触发，无论演示主题和复杂度如何。如果需要生成或制作PPT演示文稿，请使用此规则。
---
