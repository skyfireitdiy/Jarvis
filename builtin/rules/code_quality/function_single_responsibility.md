...
description: 当需要识别和重构违反单一职责原则的函数时触发。每当用户提及"单一职责"、"函数拆分"、"重构函数"、"函数过长"、"复杂函数"、"代码异味"时触发。不触发：代码审查（用code_review规则）；性能优化；安全审查。
---
...
本规则定义函数单一职责原则的要求与重构方法。单一职责原则要求函数只做一事，且将此做好。违反单一职责原则的函数通常表现为：逻辑复杂、难以理解、难以测试、难以复用。
...
- **必须**：每函数只完成一明确的业务逻辑或功能
- **必须**：函数名应准确描述函数的单一职责
- **禁止**：在同一函数中混合多个逻辑层次（如：数据验证、业务处理、数据持久化）

**判断标准：**

若你无法用一句话清晰描述"此函数做何"，则其可能违反单一职责原则。

### 2. 函数长度控制

**要求说明：**

- **必须**：函数长度（含注释与空行）不超 50 行
- **必须**：函数的复杂度（圈复杂度）不超 10
- **禁止**：出现超 3 层的嵌套结构
...
2. 检查函数长度，若超 50 行，标记为需重构
3. 分析函数逻辑，识别是否存在多个职责
4. 检查参数数量，若超 5 个，标记为需重构

**常见反模式：**

- 函数名含 "and"、"or"、"andThen" 等连接词
- 函数中含过多 if/else 分支，处理多种不同情况
...
2. **降低复杂度**：用早期返回（Early Return）减少嵌套层级
3. **封装参数对象**：用数据类或字典封装相关参数
4. **命名优化**：确保每函数名准确描述其单一职责
