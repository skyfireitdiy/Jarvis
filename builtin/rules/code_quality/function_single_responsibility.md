...
本规则定义函数单一职责原则之要求与重构方法。单一职责原则要求函数只做一事，且将此做好。违反单一职责原则之函数通常表现为：逻辑复杂、难以理解、难以测试、难以复用。
...
- **必**：每函数只完成一明确之业务逻辑或功能
- **必**：函数名应准确描述函数之单一职责
- **禁**：在同一函数中混合多个逻辑层次（如：数据验证、业务处理、数据持久化）

**判断标准：**

若汝无法用一句话清晰描述"此函数做何"，则其可能违反单一职责原则。

### 2. 函数长度控制

**要求说明：**

- **必**：函数长度（含注释与空行）不超 50 行
- **必**：函数之复杂度（圈复杂度）不超 10
- **禁**：出现超 3 层之嵌套结构
