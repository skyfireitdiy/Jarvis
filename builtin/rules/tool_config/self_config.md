---
name: self_config
description: 当需要通过自然语言描述智能修改Jarvis配置文件时使用此规则——指导Agent理解用户的配置需求并自动修改相应的配置文件。包括：理解用户的自然语言配置描述；识别需要修改的配置文件和参数；使用edit_file工具精确修改配置；验证配置文件的格式正确性；处理配置的依赖关系；确保配置修改的安全性；提供配置修改的反馈；支持多种配置文件格式（YAML、JSON、TOML等）；处理复杂的配置变更；保持配置的一致性。每当用户提及"配置"、"设置"、"修改配置"、"Jarvis配置"、"系统设置"、"更改配置"、"配置文件"或需要Jarvis配置、配置文件修改、系统配置时触发，无论配置的复杂程度如何。如果需要通过自然语言修改Jarvis的配置，请使用此规则。
---
