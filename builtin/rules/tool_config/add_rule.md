...
- **description**（必需）：规则的触发条件说明
  - 用于规则索引展示和自动匹配
  - **只写触发条件和不触发条件**，不写功能介绍、技能说明、规则内容
  - **必须**按照以下详细格式编写
...
```yaml
name: clean_code
description: 当需要[明确触发条件]时触发。每当用户提及"[触发关键词1]"、"[触发关键词2]"或[适用场景]时触发。不触发：[明确不触发的条件1]；[不触发的条件2]；[不触发的条件3]。
```
...
2. **触发关键词**：具体的触发关键词，用引号括起
   - 格式：`每当用户提及"[关键词1]"、"[关键词2]"`
   - 示例：`每当用户提及"整洁代码"、"代码规范"、"最佳实践"`

3. **不触发条件**：明确说明什么情况下**不**应触发该规则
   - 格式：`不触发：[条件1]；[条件2]；[条件3]`
   - 示例：`不触发：仅阅读代码不修改；仅运行测试；讨论编程概念但不涉及代码编写`
...
---
name: clean_code
description: 当需要编写或审查整洁代码时触发。每当用户提及"整洁代码"、"代码规范"、"最佳实践"、"代码审查"、"可读性"或需要编写/审查高质量代码时触发。不触发：仅阅读代码不修改；仅运行测试；讨论编程概念但不涉及代码编写。
license: MIT
---
```
...
- **必须**：包含触发条件、触发关键词、不触发条件三个要素
- **必须**：触发关键词至少3-5个，用引号括起
- **必须**：不触发条件至少2-3项，用分号分隔
- **必须**：明确触发和不触发的边界
- **禁止**：在description中写功能介绍、技能说明、规则内容
- **禁止**：过于简短或模糊的描述
- **建议**：总长度在80-200字之间
