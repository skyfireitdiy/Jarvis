...
- **必须**：规则文件应放置在项目的 `builtin/rules/` 目录下的对应分类子目录中
...
- **必须**：创建规则文件后，必须将规则注册到项目规则列表中
- **规则列表位置：** `{{ git_root_dir }}/.jarvis/rule`
...
规则简短描述。（{% raw %}{{ git_root_dir }}{% endraw %}/.jarvis/rules/<rule_name>.md）
```
...
- [ ] 规则文件已创建在 `.jarvis/rules/` 目录
- [ ] 规则已注册到 `.jarvis/rule` 文件
- [ ] 规则注册格式符合规范
...
**练习 1：** 规则关于"如何编写安全的密码存储代码"

- 涉及安全编码规范
- 答案：**security**

**练习 2：** 规则关于"如何使用 Docker 部署应用"

- 涉及部署流程和规范
- 答案：**deployment**

**练习 3：** 规则关于"函数命名应该使用动词开头"

- 涉及代码编写规范和质量
- 答案：**code_quality**

**练习 4：** 规则关于"如何配置 pytest 测试框架"

- 涉及开发工具的使用和配置
- 答案：**development_tools**

**练习 5：** 规则关于"使用测试驱动开发（TDD）方法"

- 涉及开发方法论和工作流程
- 答案：**development_workflow**

### 常见分类误区

**误区 1：** 将"使用某工具的规则"归类到 tool_config

- **正确做法：** 如果规则是关于如何使用工具进行开发任务，应归类到对应
  的功能分类（如 development_tools、code_quality 等）
- **tool_config** 只用于配置工具本身的设置和规则管理

**误区 2：** 将"重构"归类到 code_quality

- **正确做法：** 重构作为开发流程的一部分，应该归类到 development_workflow
- 重构检查和质量评估才属于 code_quality

**误区 3：** 将"安全测试"归类到 development_workflow

- **正确做法：** 安全测试的核心是安全领域，应该归类到 security
- 即使它涉及测试流程，但安全是更核心的主题
