---
name: solid
description: 当需要应用SOLID设计原则时使用此规则——包括单一职责原则（SRP）、开闭原则（OCP）、里氏替换原则（LSP）、接口隔离原则（ISP）、依赖倒置原则（DIP）五大原则。包括：设计只有一个改变理由的类（SRP）；通过抽象支持扩展，对修改关闭（OCP）；确保子类能够完全替换父类（LSP）；将大接口拆分为小而专一的接口（ISP）；让高层模块和低层模块都依赖抽象（DIP）；识别违反SOLID原则的代码；重构现有代码以符合SOLID原则；评估软件设计的可维护性和可扩展性；设计灵活、易扩展的软件架构。每当用户提及"SOLID"、"单一职责"、"开闭原则"、"里氏替换"、"接口隔离"、"依赖倒置"、"面向对象设计"、"可维护性"、"可扩展性"或需要设计/评估符合SOLID原则的软件时触发，无论他们之后打算如何实施。如果需要设计、实现或评估遵循SOLID原则的面向对象软件，请使用此规则。
---
