---
name: solid
description: 当需要应用SOLID设计原则时触发。每当用户提及"SOLID"、"单一职责"、"开闭原则"、"里氏替换"、"接口隔离"、"依赖倒置"时触发。不触发：整洁架构设计（用clean_architecture规则）；仅代码规范不涉及设计原则；函数式编程不涉及OOP。
---
...
子类对象必能替换父类对象，而不致程序之正确性被破坏。

**执行要求：**

- **必**：子类必完全实现父类之抽象方法
- **必**：子类可扩展父类之功能，但不可改父类原有之功能
- **禁**：子类重写父类方法时抛出父类未声明之异常
- **禁**：子类重写方法时削弱父类方法之输入约束或输出约束
...
高层模块不应依赖低层模块，两者皆应依赖抽象。抽象不应依赖细节，细节应依赖抽象。
