...
- **必须**：核心业务逻辑可在不赖外部框架的情形下测试
- **必须**：Use Cases 可在无数据库、Web 框架的情形下测试
- **必须**：用依赖注入便于测试
- **禁止**：测试需启动完整的外部系统（如数据库、Web 服务器）
...
- 数据格式转换
- 将 Use Cases 的数据格式转换为框架或数据库所需的格式
- 将外部输入转换为 Use Cases 所需的格式

**要求：**

- **必须**：不赖框架细节
- **必须**：只赖 Use Cases 与 Entities 层
- **必须**：通过接口与外层交互
...
- **必须**：业务规则代码只现于 Entities 与 Use Cases 层
- **禁止**：于 Interface Adapters 或 Frameworks 层编写业务逻辑
- **禁止**：于业务逻辑中混入数据库操作、HTTP 请求等

### 数据访问分离

- **必须**：Entities 不赖任何数据库实现
- **必须**：通过接口（Repository）访问数据
- **必须**：数据库实现放于 Frameworks 层
