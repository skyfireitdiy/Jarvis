...
description: 当需要生成系统架构图和组件关系图时使用此规则——包括逻辑视图、实现视图、进程视图、部署视图、用例视图等多种架构视图。包括：基于代码分析生成准确的架构图；展示系统功能模块划分和交互关系；展示代码组织结构、包依赖关系；展示系统运行时进程、线程间的并发和通信；展示系统在物理环境中的部署拓扑；展示用户与系统的交互场景；生成专业的Graphviz DOT代码并渲染为PNG图片。每当用户提及"架构图"、"组件图"、"架构设计图"、"生成图表"、"系统架构"、"依赖关系图"、"部署图"或需要可视化展示系统结构时触发，无论他们之后打算如何使用图表。如果需要生成、绘制或展示架构相关图表，请使用此规则。
---
