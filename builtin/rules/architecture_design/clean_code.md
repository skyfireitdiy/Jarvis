---
description: 当需要编写或审查整洁代码时使用此规则——包括可读性优先、单一职责、DRY等核心原则。包括：编写可读性强的代码，使用有意义的变量名和函数名；确保每个函数和类只做一件事；消除重复代码，提取公共逻辑；使用配置而非硬编码；遵循命名规范（变量、函数、类、常量）；控制函数长度（最好不超过20行）；限制函数参数数量（最好不超过3个）；保持代码组织清晰，相关代码放在一起；删除未使用的代码和过时的注释；编写自解释的代码，减少不必要的注释；进行代码审查，识别代码异味。每当用户提及"整洁代码"、"代码规范"、"最佳实践"、"代码审查"、"可读性"、"命名规范"、"代码质量"或需要编写/审查高质量代码时触发，无论他们之后打算如何处理代码。如果需要编写、审查或改进代码的可读性和质量，请使用此规则。
---
