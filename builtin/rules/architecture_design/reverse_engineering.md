---
description: 当需要从现有代码推导设计意图和架构时使用此规则——包括项目反构和代码反向设计两种场景。包括：分析大型项目的整体架构和模块结构，生成完整的项目设计方案文档；理解新团队成员的项目概览；重构前的架构分析；项目文档化；技术债务评估；理解单个文件或代码片段的功能；从现有代码提取设计方案用于新实现；代码迁移和重构；跨语言实现参考。每当用户提及"反向工程"、"代码分析"、"架构理解"、"推导设计"、"项目文档化"、"代码分析"、"技术调研"或需要理解现有代码结构时触发，无论他们之后打算如何使用分析结果。如果需要打开、分析现有代码以推导其设计和架构，请使用此规则。
---
...
当一个模块满足以下任一条件时，**必须**按功能点拆分成多个独立文档：

1. **文档行数超过500行**：单个文档超过500行时必须拆分
2. **功能点超过5个**：模块包含超过5个独立功能点时建议拆分
3. **文件数量超过20个**：模块包含超过20个源文件时建议拆分
4. **复杂度高**：模块包含多个子系统或复杂交互时建议拆分

**拆分原则**：

1. **按功能拆分**：每个主要功能点生成一个独立文档
   - 示例：调度模块可以拆分为：初始化、消息分发、JOB调度、同步机制、定时器管理等
   - 示例：通信模块可以拆分为：TIPC通信、消息队列、协议处理、流量控制等

2. **按层次拆分**：按模块的层次结构拆分
   - 示例：网络模块可以拆分为：传输层、应用层、接口层
   - 示例：存储模块可以拆分为：缓存层、持久化层、索引层

3. **按组件拆分**：按核心组件拆分
   - 示例：GUI模块可以拆分为：渲染引擎、事件系统、UI组件
   - 示例：数据库模块可以拆分为：查询引擎、事务管理、锁机制
...
├── README.md                      # 项目总览文档
├── sche/                          # 调度模块目录
│   ├── README.md                  # 调度模块总览文档（必选）
│   ├── architecture_analysis.md   # 架构分析文档
│   ├── dependency_analysis.md     # 依赖关系分析文档
│   ├── initialization.md          # 初始化功能文档
│   ├── message_dispatch.md        # 消息分发功能文档
│   ├── job_scheduling.md          # JOB调度功能文档
│   ├── sync_mechanism.md          # 同步机制文档
│   └── timer_management.md        # 定时器管理文档
```

**文档命名规范**：

- 使用小写字母和下划线
- 格式：`<feature_name>.md` 或 `<component_name>.md`
- 名称应清晰反映文档内容

**总览文档要求**：

当模块被拆分为多个功能文档时，**必须**生成一个总览文档（README.md）：

1. **文档位置**：`{{ git_root_dir }}/.jarvis/rules/project_info/<module_path>/README.md`
2. **文档内容**：
   - 模块概述和整体架构
   - 功能点索引（包含所有子文档的链接）
   - 依赖关系图
...
- 文件数：5个
- 功能点：2个（日志记录、日志输出）
- 文档：单个文档 `logging.md`（300行）

**场景2：大型模块（拆分）**

- 模块：调度模块
- 文件数：42个
- 功能点：初始化、消息分发、JOB调度、同步机制、定时器管理等
- 文档：
  - `README.md`（总览，200行）
  - `initialization.md`（初始化，150行）
  - `message_dispatch.md`（消息分发，180行）
  - `job_scheduling.md`（JOB调度，200行）
  - `sync_mechanism.md`（同步机制，170行）
  - `timer_management.md`（定时器，120行）
