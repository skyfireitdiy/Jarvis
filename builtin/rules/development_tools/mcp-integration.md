---
description: 当需要集成MCP (Model Context Protocol) 服务或扩展工具功能时使用此规则——MCP集成规则，Model Context Protocol集成规范。包括：配置MCP服务器连接；注册和加载MCP工具；配置工具参数和环境变量；管理MCP服务生命周期（启动、停止、重启）；处理工具调用和数据传输；实现自定义MCP工具和服务器；集成第三方MCP服务；调试MCP连接问题；优化MCP通信性能；处理错误和异常；管理工具权限和安全；编写MCP工具文档；测试MCP工具功能；扩展MCP协议功能；实现工具链和流水线集成。每当用户提及"MCP集成"、"MCP服务"、"工具扩展"、"协议开发"或需要MCP服务集成、工具扩展、协议开发时触发，无论集成规模和复杂度如何。如果需要集成或开发MCP相关功能，请使用此规则。
---
