...
- **必须**：MCP工具配置应置于`~/.jarvis/config.yaml`文件中的`mcp`列表
- **必须**：每个 MCP 客户端配置都是独立的列表项
...
根据你的MCP服务器实现方式，选择合适的客户端类型：

#### 1. stdio类型（标准输入输出）

**适用场景：** 本地运行的命令行程序，通过stdin/stdout通信

**必需参数：**

- `type`: 必为`"stdio"`
- `command`: 启动MCP服务器的完整命令
...
**适用场景：** 通过HTTP SSE协议通信的MCP服务器

**必需参数：**

- `type`: 必为`"sse"`
- `base_url`: MCP服务器的HTTP基础URL
...
- **必须**：未出现`⚠️`配置错误警告
- **必须**：未出现`⚠️`MCP工具加载失败警告
- **必须**：未出现`⚠️`MCP工具注册失败警告

**工具命名规则：**

- **必须**：MCP工具会以`{name}.tool_call.{tool_name}`的形式注册
- **必须**：资源相关工具会以`{name}.resource.get_resource_list`与`{name}.resource.get_resource`的形式注册
...
- **必须**：准备MCP服务器运行所需的依赖与环境

### 配置过程中的最佳实践

- **荐**：用有意义的客户端名称，便于识别与管理
...
- 将`~/.jarvis/mcp/`目录下的YAML配置迁移至`~/.jarvis/config.yaml`的`mcp`列表中
...
- [ ] 配置已添加至`~/.jarvis/config.yaml`的`mcp`列表中
