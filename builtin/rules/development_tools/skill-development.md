...
description: **当用户提及“安装skill”、“安装技能”时自动触发**——Skill开发规则，Jarvis Skill功能开发规范。包括：遵循Skill开发的标准化流程；设计Skill功能和接口；编写Skill实现代码；实现技能注册和配置；定义技能触发规则和参数；编写技能描述和文档；实现错误处理和异常管理；进行功能分析和需求设计；确保注册信息的一致性；测试技能功能和性能；调试技能运行问题；优化技能执行效率；管理技能依赖和版本；集成技能到Jarvis系统；编写单元测试和集成测试；处理技能间的交互和数据共享。**安装技能时会自动激活新技能，无需手动操作**。每当用户提及"Skill开发"、"功能扩展"、"插件开发"、"技能添加"或需要Skill开发、功能扩展、插件开发时也触发，无论技能类型和复杂度如何。如果需要开发或扩展Jarvis Skill功能，请使用此规则。
---
...
本规则定义了技能开发的标准化流程和要求，确保技能添加的规范性、功能分析的准确性。
...
技能可以是本地路径或Git仓库。Git仓库技能将直接克隆到 `{{ jarvis_data_dir }}/rules/` 目录。
