---
name: jarvis_windows_cli
description: 当需要在Windows平台进行桌面程序自动化或GUI测试时触发。每当用户提及"Windows自动化"、"桌面程序操作"、"GUI测试"、"pywinauto"、"窗口操作"时触发。不触发：Linux/Mac平台操作；Web页面自动化（用browser_cli）；命令行脚本不涉及GUI。
---
...
`jw`（或`jarvis-windows`）是基于pywinauto的Windows桌面程序自动化命令行工具，支持启动应用、连接至已有窗口、执行点击、输入、截图、获取控件树、列举窗口，以及修改常用系统配置（主题、电源、代理等）。**仅支持Windows平台**。

> **⚠️ 重要提醒**
>
> 1. **截图功能限制**：除非用户明确要求，否则不要用`screenshot`命令。截图主要用于调试与验证目的，非默认行为。
> 2. **输出完整性**：`jw`的所有命令都返回JSON格式的完整结果（含`success`、`stdout`、`stderr`字段）。**绝对不允许对jw执行命令的结果进行任何形式的过滤、截断或修改**，必完整返回原始JSON输出，以确保用户能准确判断操作是否成功。
...
- [Windows App工具规范]({{ jarvis_src_dir }}/.jarvis/spec/windows_app_tool_spec.md)
