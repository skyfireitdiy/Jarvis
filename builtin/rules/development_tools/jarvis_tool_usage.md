---
description: 当需要使用或学习Jarvis工具系统时使用此规则——Jarvis工具使用规则。包括：使用jarvis-tool命令行工具管理和查看工具信息；查询工具列表和详细信息；搜索特定功能的工具；理解工具的使用方法和参数配置；配置工具参数和环境变量；测试工具功能和可用性；学习工具的最佳实践和使用技巧；理解工具的输入输出格式；调试工具调用问题；管理工具版本和依赖；集成工具到工作流程；编写工具使用文档；分析工具性能和限制；扩展自定义工具功能。每当用户提及"工具使用"、"工具配置"、"工具学习"、"jarvis-tool"或需要工具使用指导、工具配置、工具学习时触发，无论工具类型和使用场景如何。如果需要使用、配置或学习Jarvis工具系统，请使用此规则。
---
