...
description: 当需要创建技术图表或可视化系统/流程时触发。每当用户提及"画图"、"帮我画"、"生成图"、"做个图"、"架构图"、"流程图"、"可视化"、"出图"、"diagram"时触发。不触发：仅查看已有图片；仅描述图表不生成；纯文本输出不涉及图表。
license: MIT
---

# Fireworks Tech Graph

生成高质量SVG技术图表，并以`rsvg-convert`导出为PNG。

## 辅助脚本（推荐）

本规则含辅助脚本与样式参考文件，位于规则文件同级目录：

- `fireworks-tech-graph/scripts/` - 辅助脚本
- `fireworks-tech-graph/references/` - 样式参考文件

### 1. generate-diagram.sh - 验证SVG并导出PNG
...
节点=服务/组件。分组为水平层（自上而下或自左而右）。

- 典型层：客户端 → 网关/负载均衡 → 服务 → 数据/存储
- 以`<rect>`虚线容器将同层相关服务分组
- 箭头方向随数据/请求流
...
- 优先自上而下；宽流程自左而右
- 菱形示决策，圆角矩形示过程，平行四边形示I/O
- 节点标签宜简短（≤3词）
- 于网格上对齐节点：x位置对齐至120px间隔，y对齐至80px

### Agent架构图

展示AI代理如何推理、使用工具与管理记忆。
...
| 控制/触发 | 橙色#ea580c | 1.5px实线 | 无   | 一系统触发另一系统 |
...
- 对齐至8px网格：水平120px间隔，垂直120px间隔

**箭头标签**（关键）：

- 必有背景rect：`<rect fill="canvas_bg" opacity="0.95"/>`，水平4px、垂直2px内边距
- 置于箭头中点，≤3词，多箭头汇聚时错开15-20px
- 与节点保持10px安全距离
...
- 两箭头必交叉时，始终用跳弧防视觉重叠
- 交叉水平箭头：添加小半圆弧（半径5px）跳过另一线
- 多交叉：错开弧半径（5px、7px、9px）

**验证清单**（完成前运行）：

1. **箭头-组件碰撞**：箭头不得穿过组件内部
2. **文本溢出**：所有文本应在8px内边距内
...
- 字体：通过`<style>font-family: ...</style>`嵌入 — 不用外部`@import`
- `<defs>`：箭头标记、渐变、滤镜、剪切路径
- 文本：最小12px，推荐13-14px标签，11px子标签，16-18px标题
- 所有箭头：`<marker>`带`markerEnd`，尺寸`markerWidth="10" markerHeight="7"`
- 投影：`<feDropShadow>`在`<filter>`中，慎用（仅关键节点）
- 曲线路径：用`M x1,y1 C cx1,cy1 cx2,cy2 x2,y2`三次贝塞尔
- 剪切内容：文本可能溢出节点框时用`<clipPath>`
