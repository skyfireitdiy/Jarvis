---
name: lsp_usage
description: 当需要使用LSP（Language Server Protocol）工具进行代码分析或开发环境配置时使用此规则——LSP工具使用规则，Language Server Protocol工具配置和使用。包括：使用jlsp工具进行代码导航和符号查询；获取代码诊断信息和lint警告；获取代码修复建议和重构建议；查询符号定义和引用；理解代码语义和类型信息；配置LSP服务器和开发环境；集成LSP工具到工作流程；优化LSP通信性能；处理LSP连接问题；提升代码理解能力。每当用户提及"LSP"、"Language Server"、"代码导航"、"符号查询"、"代码诊断"、"类型推导"、"代码补全"、"LSP配置"或需要代码分析、LSP工具使用、开发环境配置时触发，无论编程语言和项目类型如何。如果需要使用LSP工具进行代码分析或配置开发环境，请使用此规则。
---
