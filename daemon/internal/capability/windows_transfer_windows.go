...
// 本文件实现 Windows 平台的分块文件传输能力（windows.fs.transfer.stat / read / write / verify）。
//
// 参数 schema 与字段名与 Linux 侧（linux_transfer_linux.go）**逐字对齐**，
// 便于上层用同一套代码驱动两个平台；差异仅在能力名前缀。
//
// 公共常量与纯逻辑（块大小、区间计算、校验和、base64）全部复用 transfer.go，
// 保证与 Linux 侧语义一致。参数读取复用同包 Windows 侧已有的
// requiredString / optionalBool，以及 transfer.go 的 transferParamInt64 / transferParamBool。
//
// 注意：本机开发环境为 Linux，无 Windows 运行环境，因此本文件只能保证
// 「在 GOOS=windows 下编译通过」与纯逻辑单测，成功路径未做端到端验证。
...
// registerWindowsTransfer 注册 Windows 分块文件传输能力。
func registerWindowsTransfer(reg *Registry) {
	_ = reg.Register(Capability{
		Name: "windows.fs.transfer.stat",
		Description: "获取 Windows 上文件（或目录）的传输元信息：是否存在、大小、修改时间与整文件 SHA-256。" +
		...
		Handler: handleWindowsTransferStat,
	})

	_ = reg.Register(Capability{
		Name: "windows.fs.transfer.read",
		Description: "按分块读取 Windows 上的文件，返回 base64 内容与该块的 SHA-256。" +
		...
		Handler: handleWindowsTransferRead,
	})

	_ = reg.Register(Capability{
		Name: "windows.fs.transfer.write",
		Description: "按分块写入 Windows 上的文件，支持断点续传（按 offset 定位）与首块清空。" +
		...
		Handler: handleWindowsTransferWrite,
	})

	_ = reg.Register(Capability{
		Name: "windows.fs.transfer.verify",
		Description: "校验 Windows 上文件的整文件 SHA-256 是否与期望值一致（大小写不敏感）。" +
			"传输完成后调用，确认落盘内容无损。",
		Platform: PlatformWindows,
		Parameters: map[string]any{
			"type": "object",
		...
		Handler: handleWindowsTransferVerify,
	})
}

// handleWindowsTransferStat 是 windows.fs.transfer.stat 的实现。
func handleWindowsTransferStat(params map[string]any) (any, error) {
...
// handleWindowsTransferRead 是 windows.fs.transfer.read 的实现。
func handleWindowsTransferRead(params map[string]any) (any, error) {
	path, err := requiredString(params, "path")
	if err != nil {
		return nil, err
	}
...
// handleWindowsTransferWrite 是 windows.fs.transfer.write 的实现。
func handleWindowsTransferWrite(params map[string]any) (any, error) {
	path, err := requiredString(params, "path")
	if err != nil {
		return nil, err
	}
...
// handleWindowsTransferVerify 是 windows.fs.transfer.verify 的实现。
func handleWindowsTransferVerify(params map[string]any) (any, error) {
	path, err := requiredString(params, "path")
	if err != nil {
		return nil, err
	}
