...
class BasePlatform(ABC):
    ...
    def __init__(
        ...
        self._compression_applied = False  # 压缩标志位，防止重复压缩
        ...
        if self.agent is not None:
            ...
            if self._summarizing:
                try:
                    ...
                    PrettyOutput.auto_print(
                        f"ℹ️ 正在总结中，跳过本次总结检查（剩余token: {remaining_tokens}/{max_input_tokens}）"
                    )
                except Exception:
                    pass
            else:
                try:
                    ...
                    # 总结检查失败不影响对话流程
                    PrettyOutput.auto_print(f"⚠️ 总结检查失败: {str(e)}")
    ...
    def apply_compression_strategy(self, strategy_type: str = "adaptive") -> bool:
        ...
        返回:
            bool: 压缩是否成功
        """
        if not self.agent or not hasattr(self.agent, f"_{strategy_type}_compression"):
            return False

        try:
            compression_method = getattr(self.agent, f"_{strategy_type}_compression")
            ...
            PrettyOutput.auto_print(f"⚠️ {strategy_type}压缩失败: {str(e)}")
            return False
    ...
    def _check_and_apply_compression(self) -> bool:
        ...
        返回:
            bool: 是否成功应用了压缩
        """
        # 防止重复压缩：如果已经在压缩过程中或已应用压缩，跳过
        if self._summarizing or self._compression_applied:
            return False
        ...
        try:
            ...
            conversation_turn_threshold = get_conversation_turn_threshold()
            turn_limit_triggered = self._conversation_turn > conversation_turn_threshold
            ...
            try:
                ...
                for strategy in strategies:
                    PrettyOutput.auto_print(f"🔄 尝试 {strategy} 压缩策略...")
                    compression_success = self.apply_compression_strategy(strategy)
                    if compression_success:
                        PrettyOutput.auto_print(f"✅ {strategy} 压缩成功")
                        self._compression_applied = True  # 标记已应用压缩
                        break
                    else:
                        PrettyOutput.auto_print(f"⚠️ {strategy} 压缩失败，尝试下一策略")

                if not compression_success:
                    PrettyOutput.auto_print(
                        "⚠️ 所有压缩策略均失败，将回退到原始总结方式"
                    )
                    return False

                # 压缩成功后，重置压缩应用标志，以便后续可以再次压缩
                # 但需要一些时间间隔，所以暂时不要重置

                # 更新剩余token计数
                remaining_tokens_after = self.get_remaining_token_count()
                PrettyOutput.auto_print(
                    f"✅ 压缩完成，剩余token从 {remaining_tokens} 增加到 {remaining_tokens_after}"
                )

                return True
            except Exception as e:
                PrettyOutput.auto_print(f"⚠️ 压缩过程失败: {str(e)}")
                return False
            finally:
                # 无论压缩成功失败，都清除标志位
                self._summarizing = False
