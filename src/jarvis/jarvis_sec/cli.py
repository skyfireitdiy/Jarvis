...
def heuristic(
    ...
    rules: Optional[str] = typer.Option(
        None, "--rules", "-r", help="规则过滤（逗号分隔，如 unsafe_api,buffer_overflow）"
    ),
) -> None:
    """
    ...
    from pathlib import Path
    import json
    from jarvis.jarvis_sec.checkers.c_checker import analyze_c_cpp_text, analyze_c_cpp_file
    ...
    if target_path.is_file():
        ...
        # 排除常见目录
        exclude_dirs = {
            ".git", "build", "out", "target", "dist", "bin", "obj",
            "third_party", "vendor", "deps", "node_modules",
        ...
        for i, file_path in enumerate(c_files, 1):
            ...
            # 进度提示（每10个文件）
            if i % 10 == 0:
                PrettyOutput.auto_print(f"⏳ [heuristic] 已扫描 {i}/{len(c_files)} 个文件")

        PrettyOutput.auto_print(f"✅ [heuristic] 扫描完成，共扫描 {len(c_files)} 个文件")
    else:
