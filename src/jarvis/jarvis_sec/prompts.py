...
- **记忆使用**：
  - 在复核过程中，充分利用 memory 工具（action=retrieve）检索已有的记忆，特别是与当前文件或函数相关的记忆。
...
- **记忆使用**：
  - 在聚类过程中，充分利用 memory 工具（action=retrieve）检索已有的记忆，特别是与当前文件或函数相关的记忆。
  - 如果有必要，使用 memory 工具（action=save）保存聚类过程中发现的函数或代码片段的要点，使用函数名或文件名作为 tag。
