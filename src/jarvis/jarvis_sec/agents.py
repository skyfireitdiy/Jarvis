...
- **记忆使用**：
  - 分析过程中，善用 memory 工具（action=retrieve）检索已有的记忆，特别是与当前分析函数相关的记忆。
  - 若有需要，用 memory 工具（action=save）保存每个函数的分析要点，以函数名为 tag（例如：函数名、文件名等）。
  - 记忆内容示例：某函数的指针已判空、某函数已有输入校验、某函数的调用路径分析结果等。
  - 这样可以避免重复分析，提高效率，并保持分析的一致性。
- 完成对本批次候选问题的判断后，主输出仅打印结束符 {ot("!!!COMPLETE!!!")}，不要输出其他任何内容。任务总结将在后续交互中给出。
""".strip()
