...
- 每次仅执行一操；待工具果后再行下步。
- **记忆使用**：
  - 分析过程中，善用 memory 工具（action=retrieve）检索已有之记忆，特别是与当前分析函数相关之记忆。
  - 若有需要，用 memory 工具（action=save）保存每个函数之分析要点，以函数名为 tag（例如：函数名、文件名等）。
  - 记忆内容示例：某函数之指针已判空、某函数已有输入校验、某函数之调用路径分析结果等。
  - 如此可免重复分析，提高效率，并保持分析之一致性。
- 完成对本批次候选问题之判断后，主输出仅打印结束符 {ot("!!!COMPLETE!!!")}，勿输出其他任何内容。任务总结将于后交互中见询。
""".strip()
