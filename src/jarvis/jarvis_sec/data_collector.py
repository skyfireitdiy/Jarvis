...
class DataCollector:
    ...
    def __init__(self, database: ProjectDatabase):
        ...
        if not TREE_SITTER_AVAILABLE:
            PrettyOutput.auto_print("[DataCollector] tree-sitter不可用，将使用正则表达式回退方案")
            self.c_parser = None
    ...
    def analyze_files_batch(self, file_paths: List[str], language: str) -> Dict[str, Any]:
        """
        批量分析文件
        ...
        for file_path in file_paths:
            try:
                ...
                total_result['failed_files'].append({'file': file_path, 'error': str(e)})
    ...
    def _extract_from_ast(self, node: Node, code: str, file_path: str, result: Dict[str, Any]):
        """
        从AST提取数据
    ...
    def _handle_function_definition(self, node: Node, code: str, file_path: str, result: Dict[str, Any]):
        """
        处理函数定义
    ...
    def _handle_type_definition(self, node: Node, code: str, file_path: str, result: Dict[str, Any]):
        """
        处理类型定义
        ...
        if node.type == 'struct_specifier':
            ...
            identifiers = [child for child in node.children if child.type == 'type_identifier']
            if identifiers:
    ...
    def _handle_declaration(self, node: Node, code: str, file_path: str, result: Dict[str, Any], scope: str):
        """
        处理变量声明
    ...
    def _handle_call_expression(self, node: Node, code: str, file_path: str, result: Dict[str, Any], scope: str):
        """
        处理函数调用
        ...
        if args_node:
            self._extract_call_arguments(args_node, code, file_path, result, scope, line)

    def _handle_assignment(self, node: Node, code: str, file_path: str, result: Dict[str, Any], scope: str):
        """
        处理赋值表达式
    ...
    def _handle_memory_operation(self, node: Node, code: str, file_path: str, result: Dict[str, Any], scope: str):
        """
        处理内存操作（malloc、free等）
    ...
    def _extract_with_regex(self, code: str, file_path: str, result: Dict[str, Any]):
        ...
        # 提取函数定义
        func_pattern = re.compile(r'^(?:static\s+)?(?:inline\s+)?(?:const\s+)?(?:struct\s+)?(?:enum\s+)?(?:union\s+)?(?:\w+\s+\*?\s*)?(\w+)\s*\([^)]*\)\s*\{')
        ...
        for i, line in enumerate(lines, 1):
            ...
            for match in call_pattern.finditer(line):
                ...
                # 排除常见关键字
                if callee_name in ['if', 'while', 'for', 'switch', 'return', 'sizeof', 'typeof']:
                    continue
        ...
        # 提取malloc/free调用
        malloc_pattern = re.compile(r'(\w+)\s*=\s*(?:\([^)]*\))?\s*(malloc|calloc|realloc)\s*\(')
                    ...
                    if child.type == 'parameter_declaration':
                        ...
                        if param_declarator:
                            if param_declarator.type == 'identifier':
                                params.append(self._get_node_text(param_declarator, code))
                            elif param_declarator.type == 'pointer_declarator':
                                # 指针参数
                                for subchild in param_declarator.children:
                                    if subchild.type == 'identifier':
                                        params.append(self._get_node_text(subchild, code))
        return params
                ...
                if declarator:
                    if declarator.type == 'identifier':
                        ...
                        for subchild in declarator.children:
                            if subchild.type == 'identifier':
                                decls.append((self._get_node_text(subchild, code), type_name + '*'))
    ...
    def _extract_call_arguments(self, args_node: Node, code: str, file_path: str, result: Dict[str, Any], scope: str, line: int):
        """
        提取函数调用参数中的变量使用
    ...
    def _extract_struct_members(self, body_node: Node, code: str) -> List[Dict[str, Any]]:
        """
        提取struct/union成员
