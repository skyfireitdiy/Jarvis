...
{_json2.dumps(pending_in_file_with_ids, ensure_ascii=False, indent=2)}
    ...
    # ========== 验证阶段：基于 gid 的断点恢复 ==========
    # 核心原则：从 agent_issues.jsonl 读取已处理的 gid，从 progress.jsonl 读取已完成的批次，基于 gid 列表和批次状态判断批次是否完成
    ...
    # 从 progress.jsonl 中读取已完成的批次（基于 batch_status 事件）
