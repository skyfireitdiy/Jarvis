...
# 格式规则
一、类型: fix(修bug), feat(新功能), docs(文档), style(格式), refactor(重构), test(测试), chore(其他)
二、范围表示变更的模块或组件 (如: auth, database, ui)
三、主题行不超过七十二个字符，不以句号结尾，使用祈使语气
四、若有详述，用空行分隔主题与详述
五、详述应当解释"是什么"与"为什么"，而非"怎么做"
六、只输出提交信息，不要输出其他内容
"""
                )
                base_prompt += f"""
