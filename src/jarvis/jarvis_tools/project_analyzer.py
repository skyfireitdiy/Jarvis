...
## 分析策略
1. 在一切分析开始前，必须先确定项目的主要编程语言和技术栈（直接使用shell执行loc命令）
...
1. **确定项目的编程语言和技术栈**:
   - 使用shell命令(loc或find等)统计各类文件数量
   - 检查项目的构建文件和配置文件(package.json, requirements.txt, pom.xml等)
   - 确定使用的主要框架和依赖
...
5. **研究核心实现**:
   - 深入分析与分析目标相关的关键代码
   - 使用`function_analyzer`和`find_caller`理解复杂逻辑
   - 分析关键算法和业务逻辑的实现方式
   - 根据分析目标调整分析深度
