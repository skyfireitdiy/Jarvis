...
class ReadCodeTool:
    ...
    def _extract_syntax_units(
        ...
        if StructuredCodeExtractor:
            return StructuredCodeExtractor.extract_syntax_units(
    ...
    def _extract_syntax_units_with_split(
        ...
        return self._extract_syntax_units(filepath, content, start_line, end_line)
    ...
    def _extract_blank_line_groups(
        ...
        if StructuredCodeExtractor:
            return StructuredCodeExtractor.extract_blank_line_groups(
    ...
    def _extract_blank_line_groups_with_split(
        ...
        """
        # 先获取空白行分组
        blank_line_groups = self._extract_blank_line_groups(
            content, start_line, end_line
        )
    ...
    def _extract_line_groups(
        ...
        if StructuredCodeExtractor:
            return StructuredCodeExtractor.extract_line_groups(
                content, start_line, end_line, group_size
            )
        return []
    ...
    def _merge_and_split_by_points(
        ...
        for unit in import_units:
            ...
            if unit_start > 0 and unit_end > 0:
                ...
                if range_key not in seen_ranges:
                    ...
                    segment_info[segment_key].append(
                        unit.get("id", f"import_{unit_start}")
                    )
            ...
            if unit_start > 0 and unit_end > 0:
                ...
                if range_key not in seen_ranges:
                    ...
                    if segment_key not in segment_info:
                        segment_info[segment_key] = []
                    segment_info[segment_key].append(
                        unit.get("id", f"{unit_start}-{unit_end}")
                    )
        ...
        if os.environ.get("DEBUG_SPLIT_POINTS") == "1":
            print(
                f"\n[DEBUG] 初始切分点（语法元素边界）: {sorted(split_points)}",
                file=sys.stderr,
            )
            print(
                f"[DEBUG] 最终切分点（包含空白行和固定大小切分）: {final_split_points}",
                file=sys.stderr,
            )
            print(f"[DEBUG] 导入单元数: {len(import_units)}", file=sys.stderr)
            for i, unit in enumerate(import_units, 1):
                print(
                    f"[DEBUG]   导入单元{i}: 行 {unit.get('start_line')}-{unit.get('end_line')} ({unit.get('id', 'N/A')})",
                    file=sys.stderr,
                )
            print(f"[DEBUG] 语法单元数: {len(syntax_units)}", file=sys.stderr)
            for i, unit in enumerate(syntax_units, 1):
                print(
                    f"[DEBUG]   语法单元{i}: 行 {unit.get('start_line')}-{unit.get('end_line')} ({unit.get('id', 'N/A')})",
                    file=sys.stderr,
                )
            print("[DEBUG] 根据最终切分点生成的段:", file=sys.stderr)
            for i in range(len(final_split_points) - 1):
                seg_start = final_split_points[i]
                seg_end = final_split_points[i + 1] - 1
                print(
                    f"[DEBUG]   段{i + 1}: 行 {seg_start}-{seg_end} ({seg_end - seg_start + 1} 行)",
                    file=sys.stderr,
                )
        ...
        for i in range(len(final_split_points) - 1):
            ...
            if range_key in seen_ranges:
                if os.environ.get("DEBUG_SPLIT_POINTS") == "1":
                    print(
                        f"[DEBUG] ⚠️  跳过重复段: ({seg_start}, {seg_end})",
                        file=sys.stderr,
                    )
                continue
            ...
            for existing_start, existing_end in seen_ranges:
                ...
                if not (seg_end < existing_start or seg_start > existing_end):
                    ...
                    if os.environ.get("DEBUG_SPLIT_POINTS") == "1":
                        print(
                            f"[DEBUG] ⚠️  发现重叠段: ({seg_start}, {seg_end}) 与 ({existing_start}, {existing_end})",
                            file=sys.stderr,
                        )
                    # 跳过新段（保留已有的段）
                    overlap_found = True
    ...
    def _find_blank_line_split_points(
        ...
        while current + max_size <= end_line:
            ...
            found_blank = False
            search_start = max(
                current + 1, check_end - max_size // 2
            )  # 只在前半部分查找
    ...
    def _extract_imports(
        ...
        if StructuredCodeExtractor:
            return StructuredCodeExtractor.extract_imports(
                filepath, content, start_line, end_line
            )
        return []
    ...
    def _get_blocks_from_cache(
        self, cache_info: Dict[str, Any], start_line: int, end_line: int
    ) -> List[Dict[str, Any]]:
        """从缓存中获取对应范围的blocks
    ...
    def _convert_units_to_sequential_ids(
        self, units: List[Dict[str, Any]], full_content: str = None
    ) -> Dict[str, Any]:
    ...
    def _estimate_structured_tokens(
        ...
        try:
            if raw_mode:
                # 原始模式：按每20行分组计算token
                line_groups = self._extract_line_groups(
                    content, start_line, end_line, group_size=20
                )
                if line_groups:
                    import_units = self._extract_imports(
                        filepath, content, start_line, end_line
                    )
                    ...
                    sample_output = self._format_structured_output(
                        filepath, all_units, total_lines
                    )
                ...
                syntax_units = self._extract_syntax_units_with_split(
                    filepath, content, start_line, end_line
                )
    ...
    def _handle_single_file(
        ...
        try:
            ...
            content_tokens = self._estimate_structured_tokens(
                abs_path, full_content, start_line, end_line, total_lines, raw_mode
            )
            ...
            # 生成整个文件的结构化信息（用于缓存）
            # 提取整个文件的导入/包含语句
            full_import_units = self._extract_imports(
                abs_path, full_content, 1, total_lines
            )
            ...
            if raw_mode:
                # 原始读取模式：按每20行分组（整个文件）
                full_line_groups = self._extract_line_groups(
                    full_content, 1, total_lines, group_size=20
                )
                # 合并导入单元和行号分组
                ...
                full_syntax_units = self._extract_syntax_units_with_split(
                    abs_path, full_content, 1, total_lines
                )
                ...
                if full_syntax_units:
                    ...
                    full_line_groups = self._extract_blank_line_groups_with_split(
            ...
            if agent:
                ...
                if cache_info and self._is_cache_valid(cache_info, abs_path):
                    # 直接从缓存中获取对应范围的blocks
                    cached_blocks = self._get_blocks_from_cache(
                        cache_info, start_line, end_line
                    )
                    if cached_blocks:
                        ...
                        output = self._format_structured_output(
                            abs_path, structured_units, total_lines, agent
                        )
                    else:
                        output = ""
                else:
                    ...
                    if raw_mode:
                        # 原始读取模式：按每20行分组
                        line_groups = self._extract_line_groups(
                            full_content, start_line, end_line, group_size=20
                        )
                        # 合并导入单元和行号分组
    ...
    def _handle_merged_ranges(
        ...
        try:
            ...
            output = self._format_structured_output(
                filepath, structured_units, total_lines, agent
            )
...
"""
    ...
    with (
        tempfile.NamedTemporaryFile(mode="w", suffix=".c", delete=False) as f1,
