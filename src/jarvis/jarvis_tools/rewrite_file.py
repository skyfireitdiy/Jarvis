...
class RewriteFileTool:
    ...
    description = (
        ...
        "局部修改请优先使用 edit_file_structed（结构化编辑）、edit_file_normal（普通 search/replace）或 edit_file_free（基于上下文的模糊匹配）。\n\n"
        "    ⚠️ 重要提示：\n"
        "    - 不要一次重写太多内容，建议分多次进行，避免超过LLM的上下文窗口大小\n"
        "    - 如果文件内容较长（超过2048字符），建议采用以下策略：\n"
        "      1. 第一次调用 rewrite_file 写入部分内容（如文件的前半部分或关键部分）\n"
        "      2. 然后多次调用 edit_file_structed/edit_file_normal/edit_file_free 工具，使用追加/替换操作补充后续内容\n"
