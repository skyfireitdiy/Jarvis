...
class EditFileNormalTool:
    ...
    name = "edit_file"
    description = (
        "使用行号范围（首选）或 search/replace（备选）编辑文件，支持同时修改多个文件。\n\n"
        "💡 使用方式：\n"
        "1. **首选：行号范围编辑** — 指定 start_line 和 end_line 直接替换（精准、无歧义）。注意：前面 diff 的替换会改变文件行数，影响后续 diff 的行号。多次范围替换时，建议**从后往前**执行，或每次替换后用 `read_code` 重新确认行号。\n"
        "2. **备选：search/replace** — 精确匹配替换（search 文本必须在文件中唯一）。仅在以下情况使用：内容分散多处、无法确定行号、修改简单且 search 唯一。\n\n"
        "🚀 特殊功能：\n"
        '- 当 search 为 "" 时，直接重写整个文件（多个diffs时仅第一个生效）\n'
        "- **支持部分成功**：多个 diffs 中部分失败时，成功的修改仍会保留\n\n"
        "⚠️ 关键约束：\n"
        "- search 使用精确字符串匹配，不支持正则表达式\n"
        "- **search 必须包含足够上下文**（前后几行、函数签名、唯一标识符），避免匹配错误位置\n"
        "- 行号范围模式：指定 start_line 和 end_line 时忽略 search 参数，直接替换指定范围（从1开始，end_line包含）"
    )
