...
class meta_agent:
    ...
    name = "meta_agent"
    description = (
        "元代理（Meta-Agent）工具：根据自然语言需求自行编写或改进 Jarvis 工具并完成注册。"
        "能力：1）调用 CodeAgent 生成完整可用的新工具代码（含参数定义、错误处理、最佳实践模板）；"
        "2）生成后自动写入 data/tools 目录并注册到 ToolRegistry；"
        "3）支持新工具内调用现有 Agent（通用任务编排、task_list_manager）与 CodeAgent（改码、构建验证、lint、review 等）；"
        "4）支持反复调用 meta_agent 对已有工具做自举式改进（自分析与演化）。"
        "用法：传 tool_name（工具名/文件名）与 function_description（目标功能的清晰描述）；返回值含生成状态与新工具文件的绝对路径。"
    )
    ...
    def _build_enhanced_prompt(
        ...
        return f"""请根据我的需求生成一个新的 Jarvis 工具。
...
- 必须继承自 Tool 基类（参考：{base_tool_file}）
...
- Agent（通用 Agent）：
  ...
  - **ARCHER 工作流说明**：
    - **ANALYZE**（分析）：理解我的需求，明确任务目标，识别约束条件与边界，**主动识别可能需要的规则支撑**。
    - **RULE**（加载规则）：加载相关的专业规则与最佳实践，为后续阶段提供指导。
    - **COLLECT**（收集信息）：只读收集必要信息，定位相关文件与上下文，为方案设计做准备。
    - **HYPOTHESIZE**（制定方案）：基于收集的信息，设计最优的技术方案，制定可执行的计划。**重要：此阶段完成后，必须经过我的确认，才能进入 EXECUTE 阶段**。
    - **EXECUTE**（执行）：按计划精确实施，确保修改正确且可回退。
    - **REVIEW**（复盘）：全面反思工作成果：审查代码质量（语法/功能/风格），核对功能是否完成，检查受影响的代码是否都已考虑到，确认是否有配套的修改（如文档、测试、配置等），评估影响面与潜在风险，确保质量。
  - **关键特性**：
    - 各阶段可穿插与回退，形成迭代式闭环
    - 在 ANALYZE 阶段主动考虑规则，避免跑偏
    - 严格按流程顺序推进，禁止跳步或乱序（除非我明确要求并说明风险）
...
- 使用 CodeAgent 负责具体代码层面的修改、重构与验证。
生成的工具必须具备以下特性：
