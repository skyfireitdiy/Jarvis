...
from jarvis.jarvis_code_agent.code_analyzer.db import GraphTraverser, SymbolKind, EdgeKind
...
class SymbolDependencyTool:
    ...
    parameters = {
        ...
        "properties": {
            "action": {
                "type": "string",
                "description": "操作类型",
                "enum": ["find_symbol", "get_file_symbols", "find_references", "find_dependencies", "traverse"]
            },
            "project_path": {
                "type": "string",
                "description": "项目根目录路径"
            ...
            "kind": {
                ...
                "enum": ["function", "class", "method", "variable", "constant", "module", "interface", "type", "enum", "struct", "trait", "impl", "namespace", "import"]
            },
            "direction": {
                "type": "string",
    ...
    def _find_symbol(self, db: SymbolTableDB, symbol_name: str, file_path: Optional[str] = None, kind: Optional[str] = None) -> List[Dict[str, Any]]:
        """查找符号"""
        results = []
        ...
        if file_path:
            ...
            for sym in symbols:
                if sym.name == symbol_name:
                    if kind is None or (hasattr(sym.kind, 'value') and sym.kind.value == kind) or str(sym.kind) == kind:
    ...
    def _get_file_symbols(self, db: SymbolTableDB, file_path: str, kind: Optional[str] = None) -> List[Dict[str, Any]]:
        """获取文件的符号列表"""
        symbols = db.get_file_symbols(file_path)
        results = []
    ...
    def _find_references(self, db: SymbolTableDB, symbol_name: str, file_path: Optional[str] = None) -> List[Dict[str, Any]]:
        """查找符号引用"""
        # 先找到符号
    ...
    def _find_dependencies(self, db: SymbolTableDB, symbol_name: str, file_path: Optional[str] = None) -> List[Dict[str, Any]]:
        """查找符号依赖"""
        # 先找到符号
        symbols = self._find_symbol(db, symbol_name, file_path)
        if not symbols:
    ...
    def _traverse(self, db: SymbolTableDB, symbol_name: str, file_path: Optional[str] = None, direction: str = "forward", max_depth: int = 10) -> Dict[str, Any]:
        """图遍历"""
        # 先找到符号
        symbols = self._find_symbol(db, symbol_name, file_path)
        if not symbols:
    ...
    def execute(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not project_path:
                return {"success": False, "stdout": "", "stderr": "缺少 project_path 参数"}
            
            if not os.path.isdir(project_path):
                return {"success": False, "stdout": "", "stderr": f"项目路径不存在: {project_path}"}
            ...
            if action == "find_symbol":
                if not symbol_name and not file_path:
                    return {"success": False, "stdout": "", "stderr": "find_symbol 需要 symbol_name 或 file_path 参数"}
                results = self._find_symbol(db, symbol_name or "", file_path, kind)
                return {"success": True, "stdout": json.dumps(results, ensure_ascii=False, indent=2), "stderr": ""}
            
            elif action == "get_file_symbols":
                if not file_path:
                    return {"success": False, "stdout": "", "stderr": "get_file_symbols 需要 file_path 参数"}
                results = self._get_file_symbols(db, file_path, kind)
                return {"success": True, "stdout": json.dumps(results, ensure_ascii=False, indent=2), "stderr": ""}
            
            elif action == "find_references":
                if not symbol_name:
                    return {"success": False, "stdout": "", "stderr": "find_references 需要 symbol_name 参数"}
                results = self._find_references(db, symbol_name, file_path)
                return {"success": True, "stdout": json.dumps(results, ensure_ascii=False, indent=2), "stderr": ""}
            
            elif action == "find_dependencies":
                if not symbol_name:
                    return {"success": False, "stdout": "", "stderr": "find_dependencies 需要 symbol_name 参数"}
                results = self._find_dependencies(db, symbol_name, file_path)
                return {"success": True, "stdout": json.dumps(results, ensure_ascii=False, indent=2), "stderr": ""}
            
            elif action == "traverse":
                if not symbol_name:
                    return {"success": False, "stdout": "", "stderr": "traverse 需要 symbol_name 参数"}
                direction = args.get("direction", "forward")
                max_depth = args.get("max_depth", 10)
                results = self._traverse(db, symbol_name, file_path, direction, max_depth)
                return {"success": True, "stdout": json.dumps(results, ensure_ascii=False, indent=2), "stderr": ""}
            
            else:
                return {"success": False, "stdout": "", "stderr": f"不支持的操作: {action}"}
