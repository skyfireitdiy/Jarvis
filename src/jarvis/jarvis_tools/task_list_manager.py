...
**参数规范：additional_info 参数**

**作用范围：**
- `get_task_detail` 操作：用于验证任务查询请求的合法性，必须提供非空字符串
- `execute_task` 操作：用于为任务执行提供附加上下文信息，增强任务理解

**内容要求：**
1. **格式标准：**
   - 必须为字符串类型，不能为 `None`
   - 必须包含至少一个非空字符（不能是纯空白字符串）
   - 推荐使用有意义的描述性文本
...
3. **技术规范：**
   - 最大长度：建议不超过2000字符以保证可读性
   - 编码格式：UTF-8编码的Unicode字符串
   - 换行符：统一使用 `\n`（Unix格式）
   - 特殊字符：避免使用控制字符（ASCII 0-31，除了 `\t`, `\n`, `\r`）

**使用示例：**
```
# get_task_detail 场景
"需要查看任务详情以了解当前进度，重点关注任务状态和预期输出"

# execute_task 场景
"任务背景：修复用户登录功能的JWT token验证问题
关键信息：
- 影响文件：src/auth/jwt_handler.py 第45-67行
- 错误现象：token过期后未正确刷新
- 测试数据：user_id=12345的测试账户无法登录
- 预期结果：token过期时自动刷新并重定向到首页"
```
...
**错误处理：**
如果 `additional_info` 不符合要求，系统将返回：
- 错误码：缺少 additional_info 参数
- 错误信息："additional_info 参数不能为空"
