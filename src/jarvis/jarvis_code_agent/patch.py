...
# 生成要求（必须严格遵守）
1. 行号精确性规则：
   - 行号范围必须严格限定在变更描述涉及的代码区域
   - 起始行号必须精确匹配变更前代码的起始位置
   - 结束行号必须精确匹配变更前代码的结束位置
   - 绝对不要包含未修改的相邻代码行

2. 行号计算指引：
   a. 在原始代码中找到与变更描述中代码片段完全匹配的连续行
   b. 根据匹配行的起始和结束位置确定行号范围
   c. 新增代码时，行号应指向要插入的位置（如在第5行后插入，使用5,5）
...
3. 格式规范：
   ...
   - 行号必须从1开始的正整数且满足 起始行号 ≤ 结束行号

4. 安全保护机制：
   - 必须保留变更范围前后至少3行未修改的原始代码作为安全边界
   - 如果无法找到精确匹配的行号范围，宁可放弃修改也不要猜测
