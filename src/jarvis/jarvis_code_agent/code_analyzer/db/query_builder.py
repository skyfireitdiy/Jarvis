...
class QueryBuilder:
    ...
    def _prepare_statements(self) -> None:
        ...
        self._get_nodes_by_file_sql = "SELECT * FROM nodes WHERE file_path = ? ORDER BY start_line"
        self._get_nodes_by_kind_sql = "SELECT * FROM nodes WHERE kind = ?"
        self._get_nodes_by_name_sql = "SELECT * FROM nodes WHERE name = ? OR qualified_name = ?"
        ...
        self._get_outgoing_kinds_sql = "SELECT * FROM edges WHERE source = ? AND kind IN ({})"
        self._get_incoming_sql = "SELECT * FROM edges WHERE target = ?"
        self._get_incoming_kinds_sql = "SELECT * FROM edges WHERE target = ? AND kind IN ({})"
    ...
    def insert_node(self, node: Node) -> None:
        ...
        self.db.execute(
            self._insert_node_sql,
            (
                data["id"], data["kind"], data["name"], data["qualified_name"],
                data["file_path"], data["language"],
                data["start_line"], data["end_line"],
                data["start_column"], data["end_column"],
                data["docstring"], data["signature"], data["visibility"],
                data["is_exported"], data["is_async"], data["is_static"],
                data["parent_id"], data["updated_at"],
            ),
        )
    ...
    def delete_nodes_by_file(self, file_path: str) -> None:
        ...
        cursor = self.db.execute("SELECT id FROM nodes WHERE file_path = ?", (file_path,))
    ...
    def upsert_file(self, file_record: FileRecord) -> None:
        ...
        self.db.execute(
            self._upsert_file_sql,
            (
                data["path"], data["content_hash"], data["language"],
                data["size"], data["modified_at"], data["indexed_at"],
                data["node_count"],
            ),
        )
