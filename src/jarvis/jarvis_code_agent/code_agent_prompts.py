...
5. 先读后写，精准执行（工具规范使用）：
   ...
   - 编辑工具选择：
     - 局部修改（改少数行、补代码块）：优先使用 edit_file_structed（结构化块编辑）、edit_file_normal（普通 search/replace）或 edit_file_free（基于上下文的模糊匹配），明确标注"修改范围（行号/代码片段）+ 修改内容"（如"替换第15-20行的循环逻辑为：xxx"）；
     - edit_file_free 适用于代码有轻微格式差异或需要模糊匹配的场景，建议包含前后3行上下文以提高匹配准确性；
...
- edit_file_structed/edit_file_normal/edit_file_free/rewrite_file：修改后必须保持代码缩进、命名规范与原文件一致（如原文件用4空格缩进，不改为2空格），不引入多余空行、注释。
