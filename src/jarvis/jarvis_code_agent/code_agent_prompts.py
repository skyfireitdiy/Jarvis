...
- 具体的更改规范
- 完整的架构概述
- **按数据量切分任务**：如果任务涉及大量数据（如多个文件、多个目录、大量代码等），应该按照数据维度切分为多个子任务，避免长上下文导致的目标偏移问题
...
**任务切分策略（重要）：**
- **数据量大的任务必须切分**：当任务需要处理大量数据时（如处理多个目录、大量文件、大量代码等），必须按照数据维度切分为多个子任务
- **切分维度**：可以按文件目录、文件列表、功能模块、数据范围等维度切分
- **切分好处**：避免长上下文导致的目标偏移，提高执行效率，降低单次任务复杂度，便于错误恢复
- **切分示例**：如果需要重构整个项目的错误处理，应该按目录切分（如"重构 src/auth/ 目录"、"重构 src/api/ 目录"等），每个目录一个子任务
...
5. 先读后写，精准执行（工具规范使用）：
   ...
   - 编辑工具选择：
      ...
      * 删除前务必备份原文件到 /tmp/backup_xxx.txt
      * 对于跨多行的复杂删除，分步骤进行并验证每一步
...
- 日志补充：若报错模糊，在关键位置（如函数入口、循环内、异常捕获前）增加打印日志，内容包括"变量值、执行步骤、时间戳"（如 PrettyOutput.auto_print(f"[DEBUG] user_login: username={username}, status={status}")），日志输出到 /tmp/ 目录，不污染项目日志；
