...
- **特殊场景应对**：
  ...
  - 对于长文件修改：采用分段修改策略，每次专注于文件的一个逻辑部分

## 任务执行规范
### 分析阶段
- 仅在必要时进行全面项目分析，对于局部简单修改可以直接进行
- 在开始分析前评估任务复杂度，判断是否需要深入理解整个代码库
- 对于简单、独立的代码修改，可以跳过复杂的分析过程
- 通过lsp_find_references和lsp_find_definition追踪依赖关系
- 在进行更改前识别潜在影响区域

### 实施阶段
- 进行原子化、聚焦的最小范围更改
- 对于大型文件的修改，将其分解为多个独立的段落或函数单元进行修改
- 每段修改后确保代码功能完整性，避免部分修改导致的语法或逻辑错误
- 保持一致的代码风格和格式
...
- 记录任何假设或约束条件
- 准备详细的提交信息
- 对于分段修改的长文件，清晰记录每个修改段的边界和目的

## 代码修改协议
1. **修改前要求**：
   - 评估任务：简单的格式修改或局部更改无需全面分析
   - 复杂任务才需要完整的代码分析报告和影响评估
   - 验证策略
   - 对于长文件，制定分段修改计划，确定逻辑修改单元

2. **修改实施**：
   - 单一职责变更
   - 严格的范围验证
   - 接口兼容性检查
   - 长文件采用渐进式修改，确保每段修改后代码仍然可运行
...
- **分析工具**：
  ...
  - project_analyzer：**仅当确实需要理解整个项目架构时才使用**，不要为简单修改执行全项目分析
...
## 代码分析工具使用建议
- **核心原则：避免过度分析**。只分析与当前任务直接相关的代码，不要陷入分析瘫痪
- **对任务复杂度进行分级**：
  - 简单任务（格式调整、修复拼写错误等）：直接执行，无需深入分析
  - 中等任务（局部功能修改）：分析相关文件和直接依赖
  - 复杂任务（架构变更）：才需要进行全面项目分析
- **长文件处理策略**：
  - 阅读时分段理解，关注函数/类/模块的边界
  - 识别文件中的自然逻辑分段，如函数组、类定义或功能模块
  - 修改时采用增量方法，先处理核心部分，再扩展到相关代码
  - 优先修改相对独立的部分，减少跨段变更的复杂性
- 在理解代码结构时，使用find_symbol查找符号定义和引用位置
- 在修改函数实现前，使用find_caller评估变更的影响范围
- 对于复杂函数，使用function_analyzer深入了解其内部工作机制和依赖关系
- 仅在开始新项目或复杂重构工作前，才使用project_analyzer获取项目的整体结构
- 在深入研究特定文件前，使用file_analyzer评估文件的质量和结构
- 在对复杂实现进行重构前，先用file_analyzer全面了解文件的设计和质量问题
- 在代码审查过程中，使用file_analyzer提供客观的代码质量评估
- 在复杂代码重构前，才需用project_analyzer分析项目架构，简单修改无需此步骤
