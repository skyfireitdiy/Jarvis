...
"""
            ...
            4. 对于简单的文本替换，推荐使用 edit_file 工具（查找替换）进行精确修改。对于复杂代码（超过50行或涉及多文件协调），必须使用task_list_manager创建任务列表进行安全拆分。
            5. 代码编辑任务优先使用 edit_file 的查找替换，确保搜索文本在目标文件中有且仅有一次精确匹配，保证修改的准确性和安全性。
            6. 如需大范围重写（超过200行或涉及重构），请使用 write_file 工具整文件覆写；若已知目标行号，可用 edit_file_by_line 按行号区间替换。
...
def cli(
    ...
    if web_gateway:
        try:
            ...
            if auth_token:
                pass  # validate_gateway_token() 会直接从环境变量读取
