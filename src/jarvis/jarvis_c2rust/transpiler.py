...
"- 不可测试时：如函数依赖外部环境或调用未实现符号，暂不生成测试，但请在函数文档注释中注明不可测试原因（例如外部依赖/未实现符号）。",
    ...
    def _review_and_optimize(self, rec: FnRecord, module: str, rust_sig: str) -> None:
        ...
        def build_review_prompts() -> Tuple[str, str, str]:
            sys_p = (
                "你是Rust代码审查专家。验收标准：Rust 实现应与原始 C 实现在逻辑上一致，涵盖主路径与边界条件的功能与可观测副作用；允许非功能性差异（如日志、风格、错误信息表述）。"
                "关注点：检查核心输入输出、状态变化以及边界/异常路径的处理是否与 C 实现逻辑一致；资源释放/内存语义需等价（如指针有效性、长度边界等）。"
                "重要说明：\n"
                "- 允许 Rust 实现修复 C 代码中的安全漏洞（如缓冲区溢出、空指针解引用、未初始化内存使用等），这些修复不应被视为逻辑不一致；\n"
                "- 仅关注逻辑一致性（功能逻辑、输入输出、状态变化），不要求与 C 实现完全一致；\n"
            ...
            usr_p = "\n".join([
                ...
                "",
                "审查说明：",
                "- 仅关注逻辑一致性（功能逻辑、输入输出、状态变化），不要求与C实现完全一致；",
                "- 允许Rust实现修复C代码中的安全漏洞（如缓冲区溢出、空指针解引用、未初始化内存使用等），这些修复不应被视为逻辑不一致；",
                "- 若Rust实现修复了安全漏洞但保持了功能逻辑一致，应视为通过审查；",
                "",
                "被引用符号上下文（如已转译则包含Rust模块信息）：",
                json.dumps(callees_ctx, ensure_ascii=False, indent=2),
                "",
            ...
            sum_p = (
                ...
                "- 若满足“关键逻辑一致”（功能逻辑、输入输出、状态变化一致），请输出：OK\n"
                "- 注意：若Rust实现修复了C代码中的安全漏洞但保持了功能逻辑一致，应输出OK\n"
