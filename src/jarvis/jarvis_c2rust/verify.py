...
**核心原则：关注功能一致性，而非函数接口一致性。** Rust 代码的函数签名、参数类型、返回值类型可以与 C 代码不同（例如使用 Result 代替错误码、使用 Option 代替空指针），只要最终的功能行为与原 C 代码一致即可。不要将接口形式的差异误判为功能不一致。
...
3. **使用 task_list_manager.add_tasks 创建任务列表**
   ...
   - tasks_info: **按文件拆分任务**，每个文件一个子任务：
     ...
     * 每个文件任务需要完成以下所有分析维度：
       - 功能行为一致性对比（核心：验证 Rust 代码是否实现了与 C 代码相同的逻辑功能，而非接口形式是否一致）
       - 函数逻辑和算法实现对比
       - 错误处理机制验证（关注错误处理的功能结果是否一致，而非实现方式是否相同）
       - 内存安全性验证
       - 数据结构和布局对齐验证
...
4. **使用 task_list_manager.execute_task 执行每个文件任务**
   - 对于每个文件任务，提供详细的 additional_info：
     ...
     * analysis_dimensions: 需要分析的维度列表（功能行为一致性、逻辑、错误处理、内存安全、布局对齐）
   
   - 每个文件任务执行时：
     * 读取并分析 C 代码文件（如果文件很大，可以分块读取关键部分）
     * 读取并分析对应的 Rust 代码文件
     * **对每个分析维度进行详细对比**：
       - **功能行为一致性对比**（核心维度）：验证 Rust 代码是否实现了与 C 代码相同的逻辑功能。函数签名、参数类型、返回值类型的差异不算不一致，只要功能行为等价即可（例如：C 返回错误码 vs Rust 返回 Result，C 使用指针参数 vs Rust 使用引用，这些是合理的转译适配）
       - 函数逻辑和算法实现对比
       - 错误处理机制验证（关注功能结果：错误场景是否都被覆盖，错误传播是否等价）
       - 内存安全性验证
       - 数据结构和布局对齐验证
...
task_list_manager(
    ...
    tasks_info=[
        {{
            "name": "验证文件: file1.c",
            "description": "分析 file1.c 与 file1.rs 的功能对齐性，重点关注功能行为一致性（而非接口形式一致性），包括功能行为、逻辑、错误处理、内存安全、布局对齐等所有维度",
            "dependencies": []
        }},
        {{
            "name": "验证文件: file2.c",
            "description": "分析 file2.c 与 file2.rs 的功能对齐性，重点关注功能行为一致性（而非接口形式一致性），包括功能行为、逻辑、错误处理、内存安全、布局对齐等所有维度",
            "dependencies": []
        }},
        # ... 为每个文件对创建一个任务
        {{
...
- 每个文件任务需要完成所有分析维度（功能行为一致性、逻辑、错误处理、内存安全、布局对齐）
- 最后一个汇总任务依赖所有文件任务
...
{{
    ...
    "analysis_dimensions": [
        "功能行为一致性对比（核心：验证功能行为等价，而非接口形式一致）",
        "函数逻辑和算法实现对比",
        "错误处理机制验证（关注功能结果是否等价）",
        "内存安全性验证",
        "数据结构和布局对齐验证"
...
2. 读取对应的 Rust 代码文件
3. **对每个分析维度进行详细对比**：
   - **功能行为一致性对比**（核心维度）：验证 Rust 代码是否实现了与 C 代码相同的逻辑功能。函数签名、参数类型、返回值类型的差异不算不一致，只要功能行为等价即可。例如：C 返回 int 错误码 vs Rust 返回 Result<T,E>，C 使用指针输出参数 vs Rust 使用返回值元组，C 使用 NULL 表示失败 vs Rust 使用 Option——这些都是合理的转译适配，不应标记为不一致
   - **函数逻辑和算法实现对比**：对比核心逻辑流程、算法行为、边界条件处理是否等价
   - **错误处理机制验证**：验证错误场景的覆盖和错误传播的功能结果是否等价（C 的错误码/errno 与 Rust 的 Result/Option 只要功能结果一致即可）
...
**严重程度分类：**
- **High**：严重影响功能或安全性的问题（如：逻辑错误、功能行为不一致、内存安全问题、功能缺失）
- **Medium**：中等影响的问题（如：错误处理方式不同但功能结果一致、边界条件处理差异）
- **Low**：轻微或不影响功能的问题（如：函数签名风格差异、命名风格差异、注释缺失）

**重要**：仅因函数签名、参数类型、返回值类型的形式差异（但功能行为等价）不应标记为任何级别的问题。只有当功能行为真正不一致时才应标记为问题。
...
**核心原则：关注功能一致性，而非函数接口一致性。** 修复的目标是让 Rust 代码的功能行为与 C 代码等价，而不是让函数签名、参数类型、返回值类型与 C 代码一致。Rust 的惯用写法（如 Result 代替错误码、Option 代替空指针、引用代替指针）是合理的，不应为了接口一致而破坏 Rust 惯用模式。只有当功能行为真正不一致时才需要修复。
...
### 第一步：分析问题（ANALYZE）
1. 仔细阅读对齐报告，理解每个问题的：
   - 问题类型（功能行为不一致、逻辑、错误处理、内存安全、布局对齐）
   - **过滤误报**：如果问题仅是函数签名、参数类型、返回值类型的形式差异，但功能行为等价，则不需要修复
...
3. **功能对齐**：确保修复后的 Rust 代码与原 C 代码功能行为等价（而非接口形式一致）。Rust 的惯用写法（Result、Option、引用等）是合理的，不需要改为与 C 一致的接口形式
4. **代码质量**：
   - 保持代码风格和项目规范
   - 使用 Rust 最佳实践（如 Result 类型处理错误、所有权系统管理内存），不要为了与 C 接口一致而放弃 Rust 惯用模式
   - 添加必要的注释说明修复原因
