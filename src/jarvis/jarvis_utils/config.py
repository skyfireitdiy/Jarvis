...
def save_exception(
    ...
    try:
        ...
        # 保存文件：时间戳_模块_函数.json
        safe_module = module.replace("/", "_").replace("\\", "_") if module else "unknown"
        safe_function = function.replace("/", "_").replace("\\", "_") if function else "unknown"
        filename = f"{time.strftime('%Y%m%d_%H%M%S')}_{safe_module}_{safe_function}.json"
