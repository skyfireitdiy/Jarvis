...
class RulesManager:
            ...
            for root, dirs, files in os.walk(builtin_rules_dir):
                for filename in files:
                    ...
                    try:
                        ...
                        save_exception(
                            e,
                            module="jarvis_agent.rules_manager",
                            function="_get_builtin_rules_index",
                        )
                        continue
            ...
            if os.path.exists(rules_dir) and os.path.isdir(rules_dir):
                ...
                try:
                    ...
                    save_exception(
                        e,
                        module="jarvis_agent.rules_manager",
                        function="get_all_available_rule_names",
                    )
                    continue
    ...
    def _merge_loaded_rules(self) -> None:
        ...
        for rule_name in sorted(self.loaded_rules):
            if rule_name in self._loaded_rules:
                ...
                # 格式化规则内容（路径已在 _add_path_comment 中添加）
                if description:
                    # 在规则内容前添加描述（路径注释已由 _add_path_comment 添加）
                    rule_content = f"**规则描述**: {description}\n\n{self._loaded_rules[rule_name]}"
                else:
    ...
    def get_all_rules_with_status(self) -> List[Tuple[str, str, bool, str]]:
        ...
        try:
            ...
            save_exception(
                e,
                module="jarvis_agent.rules_manager",
                function="get_all_rules_with_status",
            )
            pass
    ...
    def select_rule_by_task(self, task_description: str) -> Optional[List[str]]:
        ...
        try:
            ...
            # 构造 prompt，要求模型返回编号
            prompt = f"""请根据以下任务描述，从可用规则中选出最合适的规则。
...
七、示例：<NUM>5</NUM> 或 <NUM>2,3,5,7,9</NUM> 或 <NUM>none</NUM> 或 <NUM>__NO_RULES_NEEDED__</NUM>
八、多个序号之间用逗号分隔，不要留空格
九、只返回 <NUM> 标签内的内容，不要有其他任何输出
...
所选编号："""
    ...
    def _filter_rules_by_content(
        ...
        try:
            ...
            prompt = f"""请根据任务描述，从下面的规则中选出真正相关的规则。
...
要求：
一、仔细阅读每条规则的完整内容，判断它是否真的与任务相关
二、**保守策略**：如果不确定某条规则是否相关，倾向保留它
三、只选内容确实与任务匹配的规则
四、如果没有相关规则，返回 "none"
五、**任务不需要规则**：如果任务非常简单（如寒暄、简单计算、查询时间等），不需要任何规则/技能就能完成，请返回 "__NO_RULES_NEEDED__"
六、请按下式返回规则名称：<VALID>规则名称1,规则名称2</VALID>
七、示例：<VALID>builtin:xxx.md</VALID> 或 <VALID>project:yyy.md,global:zzz.md</VALID> 或 <VALID>__NO_RULES_NEEDED__</VALID>
八、多个规则名称之间用逗号分隔，不要留空格
九、只返回 <VALID> 标签内的内容，不要有其他任何输出
