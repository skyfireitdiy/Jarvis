...
class GitHubSkillSource(ISkillSource):
    ...
    async def search(self, query: str, limit: int = 20) -> List[SkillResult]:
        ...
        headers = {
            ...
            "User-Agent": "Jarvis-Skill-Discovery"
        }
                ...
                async with session.get(
                    ...
                    for item in data.get('items', []):
                        ...
                        results.append(SkillResult(
                            name=skill_name,
                            description=repo.get('description', '') or 'GitHub Skill',
                            download_url=item['download_url'],
                            source_url=item.get('html_url', ''),
                            relevance_score=self._calc_relevance(query, f"{skill_name} {repo.get('description', '')}"),
