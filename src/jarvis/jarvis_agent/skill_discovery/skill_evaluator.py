...
请开始评估："""
    ...
    async def evaluate_async(self, task_description: str, skill: SkillResult) -> SkillEvaluation:
        """
        异步评估单个技能
    ...
    async def evaluate_batch_async(
        ...
        evaluations = [
            r for r in results 
            if isinstance(r, SkillEvaluation)
        ]
    ...
    def evaluate_batch(
        ...
        """同步批量评估包装器"""
        return asyncio.run(self.evaluate_batch_async(task_description, skills, max_concurrent))
        ...
        try:
            ...
            if hasattr(self.llm_client, 'chat') and hasattr(self.llm_client.chat, 'completions'):
                response = self.llm_client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "你是一个专业的 AI 技能评估专家，负责评估技能的相关性和价值。"},
            ...
            # 尝试直接调用 create 方法
                ...
                response = self.llm_client.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "你是一个专业的 AI 技能评估专家。"},
                        {"role": "user", "content": prompt}
                    ],
                    temperature=0.3,
                    max_tokens=500
