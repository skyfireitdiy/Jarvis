...
**task_list_manager 工具使用规则：**
- **简单任务不使用任务列表**：对于简单、直接的任务（如单次文件读取、简单的单步操作、单一工具调用等），**不要使用** `task_list_manager` 创建任务列表，直接执行即可。避免对简单任务进行不必要的拆分，防止出现无限拆分的问题。
- **复杂任务才使用任务列表**：只有对于**真正复杂**的任务（需要多个步骤、涉及多个文件、需要协调多个子任务、有明确的依赖关系等），才使用 `task_list_manager` 工具的 `create_task_list` 操作创建任务列表。
- **任务拆分原则**：在创建任务列表时，必须同时提供 `tasks_info` 参数，一次性创建并添加所有子任务。**避免过度拆分**：每个子任务应该是独立、有意义的单元，不要将简单操作拆分成多个子任务。
