...
class Agent:
    ...
    def _build_compression_prompt(self, content: str, context: str = "") -> str:
        """构建详细的压缩提示词，参考 SUMMARY_REQUEST_PROMPT 的结构

        参数:
            content: 需要压缩的对话历史内容
            context: 压缩上下文说明（如"更早的对话"、"低重要性对话"等）

        返回:
            str: 详细的压缩提示词
        """
        context_note = f"（{context}）" if context else ""
        return f"""<compression_request>
<objective>
请对以下对话历史进行精准压缩{context_note}，核心目标是为后续对话提供"无关键信息缺失"的上下文支撑：既要提炼核心逻辑，又要完整保留影响后续决策/操作的关键细节，确保后续对话无需回溯原始历史即可顺畅推进。
...
- **已完成**（✅）：**仅包含已通过验证的交付成果**
  - **验证标准**：必须同时满足以下条件才能标记为"已完成"：
    ...
    3. 达到验收标准（如满足性能指标、通过所有测试用例、满足我的需求）
    4. **代码开发任务额外要求（必须全部满足）**：
...
2. 结构组织：按"核心目标→已确认事实→决策与共识→技术细节→代码变更详情→任务进展→待办事项→我的约束"的逻辑组织（替代"时间顺序"，更贴合后续对话需求）；
...
"""
        ...
        try:
            ...
            try:
                ...
                compression_prompt = self._build_compression_prompt(
                    old_content, context="更早的对话历史"
                )

                # 使用临时模型压缩（避免污染当前对话）
                compressed_summary = self._create_temp_model(
                    "你是一个对话历史压缩助手，擅长提取关键信息并保留影响后续决策的关键细节。"
    ...
    def _score_message_importance(self, message: Dict[str, str]) -> float:
        ...
        # 工具调用相关加分
        if "tool_call" in content_lower or "<tool_call>" in content_lower:
        ...
        # 用户明确标记的重要内容加分
        if any(marker in content for marker in ["<Pin>", "重要", "关键", "必须"]):
            score += 3.0  # 用户标记的重要内容非常重要

        # 长度加分：过短的消息可能不太重要
        if len(content) < 50:
            score -= 0.5  # 过短的消息可能不太重要

        # 确保分数在合理范围内
        return max(0.0, min(10.0, score))

    def _importance_scoring_compression(
        self, score_threshold: Optional[float] = None
    ) -> bool:
        """重要性评分压缩：根据消息重要性评分，保留高分消息，压缩低分消息

        参数:
            score_threshold: 重要性评分阈值（低于此阈值的消息将被压缩），如果为None则使用配置值
        ...
        注意:
            ...
            - 低分消息会被压缩为摘要
        """
        from jarvis.jarvis_utils.config import get_importance_score_threshold

        if score_threshold is None:
            score_threshold = get_importance_score_threshold()
        ...
        try:
            ...
            # 计算每条消息的重要性评分
            scored_messages = [
                (self._score_message_importance(msg), msg) for msg in other_messages
            ]

            # 分离高分消息和低分消息
            high_score_messages = [
                msg for score, msg in scored_messages if score >= score_threshold
            ]
            low_score_messages = [
                msg for score, msg in scored_messages if score < score_threshold
            ]

            if not low_score_messages:
                # 没有低分消息需要压缩
                return False
            ...
            try:
                ...
                # 打印压缩摘要
                self._print_compression_summary(
                    compressed_summary.strip(), "重要性评分压缩"
                )
                ...
                PrettyOutput.auto_print(
                    f"⚠️ 重要性评分压缩失败: {str(e)}，将回退到其他压缩策略"
                )
                return False

        except Exception as e:
            PrettyOutput.auto_print(f"⚠️ 重要性评分压缩出错: {str(e)}")
            return False

    def _incremental_summarization_compression(
        self, chunk_size: Optional[int] = None
    ) -> bool:
        """增量摘要压缩：将对话历史分成多个chunk，对前面的chunk进行摘要，保留最后一个chunk的完整内容

        参数:
            chunk_size: 每个chunk包含的对话轮数，如果为None则使用配置值
        ...
        注意:
            ...
            - 避免一次性压缩大量内容，保持摘要质量
        """
        from jarvis.jarvis_utils.config import get_incremental_summary_chunk_size

        if chunk_size is None:
            chunk_size = get_incremental_summary_chunk_size()
        ...
        try:
            ...
            # 如果消息数量不足2个chunk，不需要压缩
            if len(other_messages) <= chunk_size * 2:
                return False

            # 将消息分成chunks
            chunks = []
            for i in range(0, len(other_messages), chunk_size):
                chunk = other_messages[i : i + chunk_size]
                if chunk:
                    chunks.append(chunk)
            ...
            # 最后一个chunk保持完整，前面的chunks需要压缩
            last_chunk = chunks[-1]
            chunks_to_compress = chunks[:-1]

            if not chunks_to_compress:
                return False

            # 压缩前面的chunks
            compressed_summaries = []
            try:
                for idx, chunk in enumerate(chunks_to_compress):
                    # 构建压缩提示
                    chunk_content = "\n\n".join(
                        [
                            f"[{msg.get('role', 'unknown').upper()}]: {msg.get('content', '')}"
                    ...
                    compression_prompt = self._build_compression_prompt(
                        chunk_content, context=f"对话历史片段（Chunk {idx + 1}）"
                    )
                    ...
                    if compressed_summary and compressed_summary.strip():
                        # 打印压缩摘要
                        self._print_compression_summary(
                            compressed_summary.strip(),
                            f"增量摘要压缩（Chunk {idx + 1}/{len(chunks_to_compress)}）",
                        )
                        compressed_summaries.append(compressed_summary.strip())
                    else:
                        PrettyOutput.auto_print(
                            f"⚠️ 增量摘要压缩：chunk {idx + 1} 生成摘要失败，跳过该chunk"
                        )
                        # 如果摘要生成失败，保留原始chunk的部分内容（前3条消息）
                        fallback_content = "\n\n".join(
                            [
                        ...
                        compressed_summaries.append(
                            f"[部分摘要] {fallback_content[:500]}..."
                        )

                if not compressed_summaries:
                    PrettyOutput.auto_print(
                        "⚠️ 增量摘要压缩：所有chunk压缩失败，跳过压缩"
                    )
                    return False
                ...
                if hasattr(self.model, "messages"):
                    ...
                    PrettyOutput.auto_print(
                        ...
                        f"保留了最后 {len(last_chunk)} 条消息的完整内容"
                    )
                    return True
                else:
                ...
                PrettyOutput.auto_print(
                    f"⚠️ 增量摘要压缩失败: {str(e)}，将回退到其他压缩策略"
                )
                return False

        except Exception as e:
            PrettyOutput.auto_print(f"⚠️ 增量摘要压缩出错: {str(e)}")
            return False

    def _is_key_event(self, message: Dict[str, str]) -> bool:
        """判断消息是否为关键事件
        ...
        返回:
            bool: 如果是关键事件返回True，否则返回False

        关键事件类型:
        - 任务开始/完成标记
        - 重要决策点（包含"决定"、"选择"、"方案"等）
        - 错误和修复（包含"错误"、"失败"、"修复"、"解决"等）
        - 关键工具调用结果（包含工具调用和重要结果）
        - 用户明确标记的重要内容（<Pin>标记）
        """
        ...
        if any(keyword in content for keyword in decision_keywords):
            # 如果包含决策关键词且内容较长（可能是重要决策），标记为关键事件
    ...
    def _key_event_extraction_compression(self) -> bool:
        """关键事件提取压缩：提取对话中的关键事件，压缩其他内容
        ...
        try:
            ...
            for msg in other_messages:
                if self._is_key_event(msg):
                    ...
                    non_key_events.append(msg)

            if not non_key_events:
                # 没有非关键事件需要压缩
                return False
            ...
            try:
                ...
                non_key_content = "\n\n".join(
                    [
                        f"[{msg.get('role', 'unknown').upper()}]: {msg.get('content', '')[:500]}"
                        for msg in non_key_events
                    ]
                )

                compression_prompt = self._build_compression_prompt(
                    non_key_content, context="非关键事件对话历史"
                )
                ...
                if not compressed_summary or not compressed_summary.strip():
                    PrettyOutput.auto_print(
                        "⚠️ 关键事件提取压缩：生成摘要失败，跳过压缩"
                    )
                    return False

                # 打印压缩摘要
                self._print_compression_summary(
                    compressed_summary.strip(), "关键事件提取压缩"
                )
                ...
                # 保留最近5条非关键消息作为上下文
                recent_non_key = (
                    non_key_events[-5:] if len(non_key_events) > 5 else non_key_events
                )
                ...
                PrettyOutput.auto_print(
                    f"⚠️ 关键事件提取压缩失败: {str(e)}，将回退到其他压缩策略"
                )
                return False

        except Exception as e:
            PrettyOutput.auto_print(f"⚠️ 关键事件提取压缩出错: {str(e)}")
            return False
    ...
    def _detect_task_type(self) -> str:
        ...
        返回:
            str: 任务类型，可能的值：
            - "code": 代码任务（涉及代码修改、文件操作等）
            - "analysis": 分析任务（数据分析、信息检索等）
            - "conversation": 对话任务（问答、讨论等）
            - "mixed": 混合任务（包含多种类型）
            - "unknown": 未知类型
        """
        try:
            # 检查Agent类型
            agent_type = getattr(self, "_agent_type", "normal")
            if agent_type == "code" or hasattr(self, "start_commit"):
                return "code"
            ...
            # 根据工具调用统计判断任务类型
            total_indicators = code_tools + analysis_tools + conversation_indicators
            ...
            PrettyOutput.auto_print(f"⚠️ 检测任务类型出错: {str(e)}")
            return "unknown"
