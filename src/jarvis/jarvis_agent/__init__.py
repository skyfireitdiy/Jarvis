    ...
    def _sliding_window_compression(self, window_size: Optional[int] = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                # 发送简短的压缩指令
                compression_prompt = """请对以上对话历史进行压缩摘要。

**压缩要求**：
1. 保留核心目标和关键决策
2. 保留已完成的任务和验证结果
3. 保留重要的代码变更信息（文件路径、函数名、变更原因）
4. 保留错误信息和调试过程
5. 删除重复内容和无关闲聊

**输出格式**：使用简洁的列表形式，按"目标→已完成→进行中→待完成"的顺序组织。"""

                # 使用临时模型压缩（避免污染当前对话）
                compressed_summary = temp_model.chat_until_success(compression_prompt)
