...
def optimize_system_prompt(
    ...
    try:
        ...
        # 构建优化提示词
        optimization_prompt = f"""你是专业的系统提示词优化专家。请根据当前的系统提示词与用户需求，有针对性地优化系统提示词。
...
【优化要求】
1. 保持原有系统提示词的核心功能与架构不变
2. 根据用户需求，有针对性地增强或调整相关部分的描述
3. 确保优化后的提示词更加贴合用户的具体任务场景
4. 保持提示词的结构清晰、逻辑完整
5. 若用户需求涉及特定领域（如代码开发、数据分析等），可适当强调相关的最佳实践与注意事项
6. 优化后的提示词应能帮助 AI 更好地理解并执行用户的具体需求
