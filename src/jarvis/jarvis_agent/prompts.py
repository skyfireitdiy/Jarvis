...
- **已完成**（✅）：**仅包含已通过验证的交付成果**
  ...
  - 格式："[优先级] [交付物] - [验证方式] - [验证证据]"
  - 示例："P0 用户表结构设计 - 已通过代码审查 - 审查记录：PR#123已合并，测试用例test_user_table通过"
  - **禁止行为**：不能仅因为"计划执行"或"开始执行"就标记为已完成；不能因为"看起来应该完成"就标记为已完成
...
- **部分完成**（⚠️）：已完成部分但未达到完整验收标准
  - 格式："[优先级] [任务描述] - [已完成部分] - [缺失部分]"
  - 示例："P2 用户登录功能 - 已完成前端UI和后端接口，待补充错误处理和日志记录"
...
- **已完成**（✅）：**仅包含已通过验证的交付成果**
  ...
  - 格式："[优先级] [交付物] - [验证方式] - [验证证据]"
  - 示例："P0 用户表结构设计 - 已通过代码审查 - 审查记录：PR#123已合并，测试用例test_user_table通过"
  - **代码开发场景**：必须包含修改的文件路径、函数名、变更类型（新增/修改/删除）、测试结果
  - **禁止行为**：
    - ❌ 不能仅因为"计划执行"或"开始执行"就标记为已完成
    - ❌ 不能因为"看起来应该完成"或"理论上应该完成"就标记为已完成
    - ❌ 不能因为"代码已编写"但未测试就标记为已完成
    - ❌ 不能因为"功能已实现"但仍有错误就标记为已完成
    - ✅ 必须提供具体的验证证据（测试结果、运行日志、审查记录等）
...
- **部分完成**（⚠️）：已完成部分但未达到完整验收标准
  - 格式："[优先级] [任务描述] - [已完成部分] - [缺失部分] - [未通过验证项]"
  - 示例："P2 用户登录功能 - 已完成前端UI和后端接口 - 待补充错误处理和日志记录 - 测试用例test_error_handling未通过"
  - **代码开发场景**：必须明确说明哪些功能已实现、哪些测试未通过、哪些需求未满足
...
4. **任务状态呈现要求（关键）**：
   ...
   - 如果任务未完成，必须如实标记为"进行中"或"部分完成"，不能错误标记为"已完成"
   - 对于"部分完成"的任务，必须明确说明已完成部分和未完成部分，以及未通过验证的具体项
5. **代码变更呈现**：使用结构化格式，例如：
   - 文件：`src/xxx.py`（修改）
