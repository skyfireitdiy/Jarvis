...
<content>
#### 1. 目标层次结构（必须清晰区分，且必须识别目标变化）
- **整体目标**（顶层愿景）：
  - 格式："[目标类型] 实现[具体成果]，达到[量化指标]"
  - 示例："[架构升级] 完成用户认证系统重构，达到QPS≥1000，99.9%可用性"
  - 必须包含：业务意义、成功标准
  - **[WARNING] 目标变化识别**：如果整体目标发生变化，必须明确说明变化原因和影响

- **阶段目标**（当前阶段交付）：
  - 格式："阶段[序号]：[具体目标] → [验收标准]"
  - 示例："阶段1：数据库表设计完成 → user/auth表结构定义+索引优化方案"
  - 必须包含：当前阶段边界、可验证的交付物
  - **[WARNING] 阶段边界**：必须明确当前阶段的开始和结束条件，避免范围蔓延

#### 2. 任务状态矩阵（精确跟踪，严格验证）
** 关键原则：必须基于实际执行结果和验证证据，禁止推测或假设任务已完成**
...
2. 结构组织：按"核心目标→已确认事实→决策与共识→技术细节→任务进展→待办事项→我的约束"的逻辑组织；
...
6. **必须包含的关键词（至少5个）**：你的输出中必须包含以下关键词中的至少5个，否则验证将失败：整体目标、阶段目标、目标变化、已完成、进行中、部分完成、待完成、关键信息位置、关键文件路径、代码变更、错误与调试、测试与验证、技术决策、未完成工作、完整性检查、检查清单、技术栈、架构决策、配置参数、代码位置、调试信息、接口定义、数据模型、代码变更历史、偏好与约束、禁忌项、代码风格。
</format>
</report>"""
...
<format>
**[WARNING] 目标一致性要求**：
- **目标更新规则**：如果对话历史中显示目标已发生偏移或更新，必须以最新目标为准
- **旧目标清理**：在总结中只保留最新的目标信息，不体现旧的目标内容
- **目标变化说明**：如果目标发生变化，必须明确说明变化情况和原因
...
7. **必须包含的关键词（至少5个）**：你的输出中必须包含以下关键词中的至少5个，否则验证将失败：整体目标、阶段目标、目标变化、已完成、进行中、部分完成、待完成、关键信息位置、关键文件路径、代码变更、错误与调试、测试与验证、技术决策、未完成工作、完整性检查、检查清单、技术栈、架构决策、配置参数、代码位置、调试信息、接口定义、数据模型、代码变更历史、偏好与约束、禁忌项、代码风格。
</format>
</summary_request>
"""
