...
- **已完成**（✅）：按优先级排序的已交付成果
  ...
  - 示例："P0 用户表结构设计 - 已通过代码审查"
  - 必须包含：具体交付物、验证标准
  - **代码开发场景**：必须包含修改的文件路径、函数名、变更类型（新增/修改/删除）
...
- **代码变更详情**（🔧）：
  - 已修改文件清单：每个文件的完整路径、修改类型（新增/修改/删除/重构）
  - 关键函数/类变更：函数名、类名、修改位置（行号范围）、变更原因
  - 代码变更上下文：为什么这样改、解决了什么问题、替代方案为什么不可行
  - 影响范围分析：哪些模块/文件/函数受到影响，是否需要同步修改
  - 依赖关系：修改依赖哪些前置条件，后续哪些工作依赖此次修改

- **错误与调试信息**（🐛）：
  - 遇到的错误：完整错误信息（堆栈、行号、错误类型）、复现步骤
  - 调试过程：尝试过的解决方案、失败原因、最终解决方案
  - 性能问题：发现的性能瓶颈、优化措施、优化效果
  - 边界情况：发现的边界条件、异常处理、测试用例

- **测试与验证**（✅）：
  - 测试结果：单元测试、集成测试、手动测试的结果
  - 验证方式：如何验证功能正确性、性能指标、兼容性
  - 未通过的测试：哪些测试失败、失败原因、是否需要修复
  - 测试覆盖：新增/修改代码的测试覆盖情况

- **技术决策与权衡**（⚖️）：
  - 技术选型：为什么选择这个方案、其他方案为什么不可行
  - 设计决策：架构设计、接口设计、数据结构设计的理由
  - 权衡考虑：性能vs可维护性、简单vs扩展性等权衡
  - 已知限制：当前方案的局限性、未来改进方向
...
4. **代码位置**：核心函数、配置文件、关键算法实现（**必须包含精确的文件路径和函数签名**）
5. **调试信息**：错误日志、性能数据、测试结果（**必须包含完整的错误堆栈和调试过程**）
6. **接口定义**：API端点、参数格式、响应结构（**必须包含函数签名、参数类型、返回值类型**）
7. **数据模型**：数据库表结构、字段定义、关系映射
8. **代码变更历史**：Git diff摘要、关键变更点、回滚点（**如果系统提供了git diff，必须整合到总结中**）
...
5. 长度限制：严格控制在500词以内，优先压缩冗余，不压缩必保信息（**代码开发任务信息较多时，可适当放宽至800词，确保关键信息不丢失**）。
</format>
</summary_request>
"""
