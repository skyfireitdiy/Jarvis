...
class AgentRunLoop:
                ...
                if json_str:
                    try:
                        ...
                        save_exception(
                            ...
                            function="_filter_tool_calls_from_response",
                        )
                        pass
                i += 1
            elif response[i : i + 3] == "```":
    ...
    def _recover_interrupt_addon(self, ag) -> None:
        """KeyboardInterrupt 恢复。

        文本协议：把用户补充信息并入 addon，置下一轮运行 input handler。
        原生 function calling：补充信息**直接作为下一轮用户输入**交给模型——
        不能放 addon，因为 _call_model 在 run_input_handlers 时若 message 为空会
        `if not message: return ""`，导致消息根本发不出去、又回到输入提示；
        同时清理待执行的工具状态（_pending_native_tool_calls / _native_continue）。
        """
    ...
    def _handle_summary(self, ag, current_response: str) -> tuple[bool, str]:
        ...
        try:
            ...
            save_exception(
                e, module="jarvis_agent.run_loop", function="_handle_summary"
            )
            pass
        # 直接使用全量总结
    ...
    def _collect_auto_complete_context(self, ag) -> str:
        ...
        try:
            ...
            save_exception(
                e,
                module="jarvis_agent.run_loop",
                function="_collect_auto_complete_context",
            )
            pass
    ...
    def _track_no_tool_call(
        ...
        # 注意：无论首轮是否有工具调用，都需重置标志，避免后续轮次误触发
        is_first_run = getattr(ag, "_first_run_occurred", False)
        ag._first_run_occurred = False
        if is_first_run and not has_tool_call:
            try:
