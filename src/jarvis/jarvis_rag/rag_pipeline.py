...
class JarvisRAGPipeline:
    ...
    def query(self, query_text: str, n_results: int = 5) -> str:
        ...
        # 2. Retrieve initial candidates using the rewritten query
        print(f"🔍 正在为查询 '{rewritten_query}' 进行混合检索...")
        candidate_docs = self.retriever.retrieve(
            rewritten_query, n_results=n_results * 4
        )
        ...
        print(f"🔍 正在对 {len(candidate_docs)} 个候选文档进行重排...")
        retrieved_docs = self.reranker.rerank(
            rewritten_query, candidate_docs, top_n=n_results
        )
        ...
        prompt = self._create_prompt(rewritten_query, retrieved_docs)
