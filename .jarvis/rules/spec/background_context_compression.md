...
| 异常类型             | 异常代码 | 描述                       | 处理策略               |
| -------------------- | -------- | -------------------------- | ---------------------- |
| 压缩线程启动失败     | BG-001   | Thread创建/启动异常        | 降级同步压缩           |
| 临时模型创建失败     | BG-002   | _create_temp_model异常     | 设置failed，降级同步   |
| LLM调用失败         | BG-003   | chat_until_success异常     | 设置failed，降级同步   |
| 摘要验证失败         | BG-004   | _validate_summary不通过    | 重试2次后设置failed    |
| 合并时历史不一致     | BG-005   | snapshot_turn不匹配        | 放弃合并，降级同步     |
...
[后台压缩异常]
  -> 捕获异常（不传播到主线程）
  -> 记录日志（logging.error，不输出到用户）
  -> 持锁设置 compression_failed = True
  -> 主线程下一轮检测到failed -> 降级同步压缩
...
- **降级同步压缩**：后台压缩任何环节失败，主线程下一轮自动执行原有同步压缩逻辑
- **线程守护**：后台线程设为 daemon=True，主进程退出时自动终止
- **锁超时保护**：所有锁操作设置超时，避免死锁
...
| 威胁类型         | 风险等级   | 影响描述               | 防护措施               |
| ---------------- | ---------- | ---------------------- | ---------------------- |
| 历史消息丢失     | 高         | 压缩合并时丢失新消息   | snapshot_turn对齐机制  |
| 并发竞态         | 中         | 多线程同时修改历史     | threading.Lock保护     |
| 死锁             | 低         | 锁获取顺序不一致       | 统一锁获取顺序+超时   |
| 临时模型泄露     | 低         | 临时模型状态残留       | 用后即弃，不缓存      |

### 8.2 安全机制

- **线程安全**：所有共享状态通过 `self._lock` 保护
- **原子替换**：合并操作在主线程执行，不在模型调用中途替换
- **快照隔离**：后台线程只读快照，不接触主模型状态
