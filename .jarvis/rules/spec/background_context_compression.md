...
| 指标             | 目标值          | 测量方法                     |
| ---------------- | --------------- | ---------------------------- |
| 压缩启动延迟     | < 10ms          | start_background_compression耗时 |
| 合并操作延迟     | < 50ms          | apply_pending_replacement耗时 |
| 主对话阻塞时间   | 0               | 压缩期间主对话无阻塞         |
| 额外API消耗      | 1次LLM调用/压缩 | 临时模型调用次数             |

### 9.2 性能优化策略

- **快照优化**：仅深拷贝历史消息列表，不拷贝整个模型
- **临时模型复用**：考虑缓存临时模型实例，避免重复创建（需评估线程安全）
- **合并批量化**：如果压缩期间产生多条新消息，一次性合并
...
| 测试类型   | 测试目标                     | 测试方法                     |
| ---------- | ---------------------------- | ---------------------------- |
| 单元测试   | BackgroundCompressionManager | Mock模型，验证状态转换       |
| 单元测试   | 安全合并算法                 | 构造快照+新消息，验证不丢失 |
| 集成测试   | 后台压缩+主对话并行          | 模拟长对话，触发压缩        |
| 异常测试   | 压缩失败降级                 | Mock临时模型失败，验证降级  |
| 并发测试   | 多次触发压缩                 | 快速连续触发，验证仅一个线程|

### 10.2 关键测试用例

#### TC-01: 后台压缩不阻塞主对话
- **步骤**：触发压缩 -> 主对话继续调用模型 -> 验证主对话无等待
- **预期**：主对话响应时间与无压缩时一致

#### TC-02: 压缩结果正确合并
- **步骤**：快照时turn=5 -> 压缩期间产生turn 6-8 -> 合并
- **预期**：新历史 = [摘要] + [turn 6-8的消息]

#### TC-03: 压缩失败降级
- **步骤**：Mock临时模型抛异常 -> 下一轮主循环
- **预期**：执行同步压缩，历史正确压缩

#### TC-04: 并发压缩互斥
- **步骤**：压缩进行中再次触发压缩
- **预期**：第二次触发返回False，不启动新线程

#### TC-05: 全程静默
- **步骤**：执行完整压缩流程
- **预期**：无任何PrettyOutput输出到用户
