...
**YAML Front Matter Header（顶层规则必须）：**
...
---
name: [规则名称，简洁明了的标识]
description: 当需要[使用场景]时使用此规则——[规则名称及核心功能描述]。包括：[主要研究或操作内容概述，使用分号分隔不同阶段或模块]。每当用户提及"[触发关键词1]"、"[触发关键词2]"、"[触发关键词3]"或[其他触发条件描述]时触发[补充说明，如适用范围等]。
license: [可选，开源协议标识，如MIT、Apache-2.0]
---
```
...
- **必须**：使用 `---` 作为起止标记
- **必须**：包含 `name` 字段（必填，规则名称）
- **必须**：包含 `description` 字段（必填，详细描述）
- **可选**：包含 `license` 字段（选填，开源协议）
- **必须**：name字段使用规则文件的简短名称（不含.md后缀）
- **必须**：name字段使用英文或拼音，不建议使用中文
- **必须**：description字段使用中文
- **必须**：描述格式："当需要[场景]时使用此规则——[规则全称]\n. 包括：[内容概览]\n. 每当用户提及"[关键词]"时触发[补充说明]"
- **建议**：description 控制在 200-500 字之间
- **建议**：列出的触发关键词准确且有代表性
