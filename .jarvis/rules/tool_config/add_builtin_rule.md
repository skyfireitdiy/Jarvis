...
- **必须**：描述格式："当需要[触发条件]时触发。每当用户提及"[关键词1]"、"[关键词2]"时触发。不触发：[条件1]；[条件2]；[条件3]。"
- **必须**：description只含触发条件、触发关键词、不触发条件，禁止写功能介绍/技能说明/规则内容/操作列表
- **建议**：description 控制在 80-200 字之间
- **建议**：列出的触发关键词准确且有代表性
...
description: 当需要系统性研究开源项目时触发。每当用户提及"研究开源项目"、"开源项目分析"、"技术调研"、"竞品对比"时触发。不触发：简单查看项目README；仅安装部署项目；代码片段参考。
license: MIT
---
```
...
````markdown
---
description: 当需要[触发条件]时触发。每当用户提及"[关键词1]"、"[关键词2]"时触发。不触发：[条件1]；[条件2]。
---
