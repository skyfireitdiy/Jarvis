...
description: 当需要分析工具调用解析失败数据、补充工具调用解析模式或优化_extract_tool_calls解析器时使用此规范——指导如何阅读tool_parse_errors目录下的错误数据，分析LLM输出中工具调用的格式模式，并在registry.py的_extract_tool_calls方法中补充对应的解析器。每当用户提及"解析失败"、"工具调用解析"、"补充解析模式"、"优化解析器"或需要分析解析错误数据、添加新的解析模式时触发。
---
