---
name: version_release
description: 当需要发布新版本或管理版本号时触发。每当用户提及"版本发布"、"发布新版本"、"版本管理"、"ReleaseNote"、"版本号"时触发。不触发：日常开发提交；功能开发；bug修复。
---
# 版本发布规则

## 规则简介

本规则用于规范Jarvis项目之ReleaseNote更新流程，确保ReleaseNote之格式一致性及代码变更之准确性。
**本规则仅支持单一模式**：

- **仅更新ReleaseNote**：只更新ReleaseNote.md文件，**禁**创建git tag、**禁**push代码、**禁**修改项目版本号（如`__init__.py`、`setup.py`、`pyproject.toml`等文件）

## 汝必遵守之原则

### 1. 语义化版本号

**要求说明：**

- **必**：用语义化版本格式 `vX.Y.Z`（如 `v1.2.0`）
- **必**：版本号格式为 `v` + `主版本号` + `.` + `次版本号` + `.` + `修订号`
- **禁**：跳过版本号或用非标准格式（如 `v1.2`、`1.2.0`）
**版本号含义：**
- **主版本号（X）**：不兼容之API修改或破坏性变更
- **次版本号（Y）**：向下兼容之功能性新增
- **修订号（Z）**：向下兼容之问题修正

### 2. 代码变更获取准确性

**要求说明：**

- **必**：用 `git diff` 获取真实之代码变更
- **禁**：用 `git log` 或 commit 信息（可能不准确）
- **必**：变更范围从最新tag至HEAD
**原因：** commit 信息可能存在不准确、不完整之情况，而 `git diff` 提供者乃真实之代码差异。

### 3. ReleaseNote格式一致性

**要求说明：**

- **必**：保持与现有ReleaseNote.md相同之格式结构
- **必**：于文件头部插入新版本信息
- **必**：用统一之分类（新功能、修复、优化、文档等）
- **必**：遵循用户视角原则（见下方详细说明）

### 3.1 用户视角原则（核心）

**重要说明**：ReleaseNote 乃给用户看者，非给开发者看者。
**禁内容**（技术细节）：

- **禁**：代码行数（如 +583行，-417行）
- **禁**：文件路径（如 src/jarvis/jarvis_sec/cli.py）
- **禁**：方法名（如 \_save_agent_state()）
- **禁**：实现方式细节（如 于\_list_session_files()方法中添加过滤逻辑）
**必内容**（用户价值）：
- **必**：功能带来之好处（如 配置更便捷、使用更简化）
- **必**：用户体验改进（如 中断恢复、快速提交）
- **必**：问题修复说明（如 修复了用户遇到之何问题）
**写作公式**：

1. **功能描述**：功能名称 + 价值 + 使用场景
2. **问题修复**：用户遇到之问题 + 修复效果
3. **优化改进**：改进点 + 用户价值

### 3.2 格式规范

**图标使用**：

- 新功能：🎯（与历史版本保持一致）
- 修复：🔧
- 优化：🎯
- 文档：📖
- 分类标题：🚀📌🔧📚🗑
**子条目格式**：

```markdown
- **🎯 功能名称**
  - **价值标签**：功能带来之好处
  - **场景标签**：何时用到、如何用
```

**内容组织**：

- 每个条目控制在 2-3 个子条目
- 突出核心价值，避冗长描述
- 用简洁之标签式描述

## 汝必执行之操作

### 操作1：获取最新版本号

**执行步骤：**

1. 执行命令获取最新tag：

```bash
git describe --tags --abbrev=0
```

1. 若无tag，用默认版本 `v1.0.0`
**预期输出：** 最新版本号（如 `v1.1.1`）

### 操作2：获取当前日期

**执行步骤：**

1. 执行命令获取当前日期：

```bash
date +%Y-%m-%d
```

**预期输出：** 日期字符串（如 `2026-01-12`）

### 操作3：运行pytest测试

**执行步骤：**

1. 执行pytest命令：

```bash
pytest -v
```

1. 检查测试结果：
   - 若测试全部通过（exit code为0）：继续执行后续操作
   - 若有测试失败（exit code非0）：**禁发布**，提示用户
2. （可选）查看详细测试报告：

```bash
pytest -v --tb=short
```

**注意事项：**

- 必于项目根目录执行pytest命令
- 若有特定测试目录，可指定路径（如 `pytest tests/`）
- 测试失败时禁跳过此步骤继续发布
**预期输出：**
- 测试全部通过：继续流程
- 测试失败：停止流程并禁发布

### 操作4：获取代码变更

**执行步骤：**

1. 首先获取变更统计信息：

```bash
git diff <latest_tag> HEAD --stat
```

1. 然后获取完整之代码差异：

```bash
git diff <latest_tag> HEAD
```

**注意事项：**

- `<latest_tag>` 替换为操作1获取之版本号
- 若输出过大，可限制行数（如 `| head -n 500`）

### 操作5：学习ReleaseNote格式

**执行步骤：**

1. 读取ReleaseNote.md前200行：

```bash
head -n 200 {{ git_root_dir }}/ReleaseNote.md
```

1. 参考历史版本格式（建议查看 v1.2.6 之前之版本）：
   - 标题格式：`### Release Note - v{version} {date}`
   - 分类结构：新功能、修复、优化与重构、文档更新等
   - 图标使用：🚀、📌、🔧、📚、🧪、🗑
   - 子分类格式：`#### **分类名称**`
   - 条目格式：`- **🎯 标题**` + `- **标签**：内容`
2. **特别注意**：参考历史版本时，重点关注用户视角与内容简洁性，勿含技术细节

### 操作6：生成新版本ReleaseNote

**执行步骤：**

1. **计算新版本号**（据用户提供之版本类型）：

```python
# 版本号计算逻辑
major, minor, patch = map(int, latest_version[1:].split('.'))
if version_type == 'major':
    new_version = f"v{major + 1}.0.0"
elif version_type == 'minor':
    new_version = f"v{major}.{minor + 1}.0"
elif version_type == 'patch':
    new_version = f"v{major}.{minor}.{patch + 1}"
else:
    raise ValueError("版本类型必须是 major、minor 或 patch")
```

1. **生成新版本内容**：

```markdown
### Release Note - {new_version} {date}
#### **🚀 新功能**
#### **📌 修复**
#### **🔧 优化与重构**
#### **📚 文档更新**
---
```

1. **于ReleaseNote.md头部插入新版本内容**：
   - 读取现有ReleaseNote.md内容
   - 于文件开头插入新版本内容
   - 保持原有格式与分隔符
**注意事项：**

- 必于用户确认版本类型后再执行插入操作
- 插入位置为文件第一行之前
- 保持原有之 `---` 分隔符格式
- **✅ 正确做法：读取整个文件 → 于开头插入新版本内容 → 写回完整文件（原有内容+新内容）**
- **❌ 错误做法：只写入新版本内容而丢失原有内容**

### 操作7：执行版本发布

**执行步骤：**

1. **执行发布脚本**：

```bash
./scripts/publish.py <发布类型>
```

说明：发布类型据实际情况选择，可选值：

- `patch`：修复问题（修订号+1）
- `minor`：新增功能（次版本号+1）
- `major`：重大变更（主版本号+1）
**注意事项：**
- 发布类型必与操作7中计算之新版本号一致
- 发布脚本会自动处理版本号更新、创建git tag等操作
**预期输出：**
- ReleaseNote.md已提交至git仓库
- 发布脚本执行成功，版本号已更新
- git tag已创建并推送

> ⚠
>
> - **禁**手动创建git tag（由发布脚本自动创建）
> - **禁**手动push代码至远程仓库（由发布脚本自动推送）
> - **禁**手动修改项目版本号（由发布脚本自动更新）
> - **禁**跳过发布脚本手动执行发布流程
>
## 检查清单

完成任务后，汝必确认：

- [ ] 最新版本号获取成功（格式正确：vX.Y.Z）
- [ ] 日期获取成功（格式正确：YYYY-MM-DD）
- [ ] pytest测试全部通过（无失败用例）
- [ ] 代码变更已获取（用了git diff而非git log）
- [ ] ReleaseNote格式已学习（了解分类与图标使用）
- [ ] 新版本号计算正确（据版本类型）
- [ ] 新版本内容已插入至ReleaseNote.md头部
- [ ] ReleaseNote.md文件格式完整（无语法错误）
- [ ] **ReleaseNote原有内容已完整保留（验证：检查文件中是否含所有历史版本记录）**
- [ ] 发布脚本./script/publish.py执行成功
- [ ] git tag已创建并推送

## 常见问题

### Q1：本规则之使用场景为何？

本规则用于完整之版本发布流程，含更新ReleaseNote、执行发布脚本，适用于以下场景：

- 正式版本发布（patch/minor/major）
- 功能迭代后之版本发布
- 定期之版本维护发布
**发布流程包括**：
- 更新ReleaseNote.md文件
- 执行发布脚本自动创建git tag、更新版本号、推送代码

### Q2：若无git tag如何？

用默认版本号 `v1.0.0` 作为基准版本。

### Q3：如何确定版本类型（major/minor/patch）？

必询问用户确认版本类型，不能自行推断。

### Q4：是否需要手动更新版本号？

否。本规则会通过发布脚本自动处理版本号更新，**禁**手动执行以下操作：
**禁事项清单：**

- **禁**手动修改项目中之版本号（如`__init__.py`、`setup.py`、`pyproject.toml`等文件）
- **禁**手动创建git tag（由发布脚本自动创建）
- **禁**手动push代码至远程仓库（由发布脚本自动推送）
- **禁**跳过发布脚本手动执行发布流程
**允许事项：**
- ✅ 更新ReleaseNote.md文件内容
- ✅ 于ReleaseNote.md头部插入新版本说明
- ✅ 添加代码变更之详细描述
- ✅ 执行./script/publish.py脚本完成发布（脚本会自动更新版本号、创建tag、推送代码）

### Q5：ReleaseNote内容如何生成？

据操作3获取之代码变更，结合操作4学习之格式，人工分析并生成ReleaseNote内容。
**关键要点**：

1. 从代码变更中提取**用户价值**，而非技术细节
2. 用**用户视角**重写功能描述（参考 3.1 节）
3. 每个条目控制在 2-3 个子条目，突出核心价值
4. 参考历史版本格式，保持风格一致

## 相关资源

- ReleaseNote模板：`{{ git_root_dir }}/ReleaseNote.md`
- Git命令参考：`git diff --help`
- 语义化版本规范：<https://semver.org/lang/zh-CN/>
