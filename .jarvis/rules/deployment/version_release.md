...
说明：`jgs` 命令用于自动生成提交信息并提交更改，参数为步骤1获取的commit id。

3. **执行发布脚本**：

```bash
./scripts/publish.sh <发布类型>
```
...
> ⚠️ **重要**：此步骤在版本发布完成后执行，用于生成朋友圈推广内容。

**执行步骤：**

1. **分析版本更新内容**：

根据操作7中生成的ReleaseNote内容，提取以下信息：
...
1. 使用 docs/images/wechat.png 作为二维码
2. 截图显示新功能的界面或效果
3. 添加版本号水印 {new_version}
```
...
- [ ] 发布脚本./script/publish.sh执行成功
- [ ] git tag已创建并推送
- [ ] 朋友圈推广文案已生成
- [ ] 文案包含版本号、更新亮点、安装指引
- [ ] 文案格式适合朋友圈发布（150-200字）

## 常见问题

### Q1：本规则的使用场景是什么？

本规则用于完整的版本发布流程，包括更新ReleaseNote、执行发布脚本、生成推广文案，适用于以下场景：
...
- ✅ 执行./script/publish.sh脚本完成发布（脚本会自动更新版本号、创建tag、推送代码）
