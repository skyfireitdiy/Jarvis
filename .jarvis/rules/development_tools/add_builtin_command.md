---
description: 当需要添加@触发的内置快捷命令时使用此规则——添加内置快捷命令规则，说明如何在Jarvis系统中添加新的@触发的内置快捷命令。包括：设计命令的触发名称和别名；定义命令的功能和参数；编写命令的执行逻辑；注册命令到系统配置；测试命令的触发和执行；处理命令的参数和选项；提供命令的帮助文档；优化命令的性能和响应；确保命令的安全性和权限控制；维护命令的向后兼容性。每当用户提及"添加命令"、"内置命令"、"@命令"、"快捷命令"、"命令开发"、"快捷键配置"或需要添加内置命令、命令开发、快捷键配置时触发，无论命令类型和复杂度如何。如果需要为Jarvis系统添加新的@触发的内置快捷命令，请使用此规则。
---
